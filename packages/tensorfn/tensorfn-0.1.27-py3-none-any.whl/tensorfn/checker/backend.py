...
class Storage:
    ...
    def _load_path(self, name, absolute=False):
        if "/" in name:
            if absolute:
                path = name
            ...
            path = os.path.join(self.get_path(), name)
...
class Local(Storage):
    ...
    def save(self, data, name):
        ...
        target_path = os.path.join(self.get_path(), name)
    ...
    def load(self, name, absolute=False, flag="rb"):
        path = self._load_path(name, absolute)
    ...
    def load_checkpoint(self, name, absolute=False):
        buf = io.BytesIO(self.load(name, absolute))
        ckpt = torch.load(buf, map_location=lambda storage, loc: storage)
        buf.close()
...
class S3(Storage):
    ...
    def _remove(self, name):
        target_path = f"{self.get_path()}/{name}"
    ...
    def load(self, name, absolute=False, flag="rb"):
        ...
        if self.show_progress:
            ...
            with tqdm(total=size, unit="B", unit_scale=True, desc=path) as pbar:
