...
class ContainerMap:
    ...
    def write_map_remote(self, map):
        ...
        self.logger(f"Finished writing map to storage account {self.CONTAINER_NAME} at {self.MAP_FILE_NAME}")
    ...
    def write_map_local(self, map):
        ...
        json_file.close()
        self.logger(f"Finished writing at {os.path.abspath(self.MAP_FILE_NAME)}")
...
class MapReporter:
    ...
    def write_csv_local(self):
        ...
        file.write(file.write(csv_data))
        file.close()
        self.logger(f"Finished writing at {os.path.abspath(self.OUTPUT_FILE_NAME)}")
