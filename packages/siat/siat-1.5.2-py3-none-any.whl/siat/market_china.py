...
def market_detail_china(category='price'):
    """
    ...
    if category1=='PRICE':
        ...
        print("信息来源：东方财富，","统计时间:",nowstr)
        print("注：")
        print("①使用动态指标。若今日不开市则指昨日或上一个交易日")
        print("②涨速：平均每分钟股价变化率，表示股价变化速度")
        print("③振幅：最高最低价差绝对值/昨收，表示股价变化活跃程度")
        print("④涨跌幅：(最新价-昨收)/昨收，表示相对昨日的变化程度")
        print("⑤涨跌额：最新价-昨收，，表示相对昨日的变化金额")
        print("⑥5分钟涨跌：最新5分钟内股价的涨跌幅度")
    ...
    if category1=='VOLUME':
        ...
        print("注：")
        print("①使用动态指标。若今日不开市则指昨日或上一个交易日")
        print("②量比：当前平均每分钟成交量与过去5个交易日均值之比，表示当前成交量的变化")
        print("③成交量：当前成交股数，表示交易活跃度")
        print("④换手率：成交量/流通股数，表示成交量占比")
        print("⑤成交额：当前开市后的累计成交金额")
    ...
    if category1=='VALUATION':
        ...
        print("信息来源：东方财富，","统计时间:",nowstr)
        print("注：")
        print("①市净率：这里为静态市盈率，按季/中/年报计算")
        print("②市盈率：这里为动态市盈率，即市盈率TTM，过去12个月的连续变化")
...
def market_detail_china2table(df):
    ...
    专门为函数market_detail_china制作
    """  

    exchnamelist=list(df)
    item='项目'
    df[item]=df.index
    df1=df[[item]+exchnamelist]
    
    from prettytable import PrettyTable
    import sys
    # 传入的字段名相当于表头
    tb = PrettyTable([item]+exchnamelist, encoding=sys.stdout.encoding) 
    
    for i in range(0, len(df1)): 
        tb.add_row(list(df1.iloc[i]))
    
    tb.align[item]='l'
    for e in exchnamelist:
        tb.align[e]='r'
...
#==============================================================================
def pandas2table(df,titletxt,footnote):
    """
    功能：将一个df转换为prettytable格式，打印，在Jupyter Notebook下整齐
    通用，但引入表格的字段不包括索引字段
    注意：py文件最开始处要加上下面的语句
