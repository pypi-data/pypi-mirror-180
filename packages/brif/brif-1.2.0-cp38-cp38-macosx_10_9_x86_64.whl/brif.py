...
class brif:
    ...
    def fit(self, df, target_col):
        ...
        self._model = brifc.fit(colnames, vartypes, 
                  num_ind, df.iloc[:,num_ind].to_numpy(na_value = 0),
                  int_ind, df.iloc[:,int_ind].to_numpy(na_value = 0),
                  fac_ind, df.iloc[:,fac_ind].to_numpy(na_value = 'NA'),
                  n, p, self.get_param())
    ...
    def predict(self, test_df, type = 'score'):
        ...
        brifc.predict(self._model, self.tmp_preddata, test_colnames, test_vartypes,
                             test_num_ind, test_df.iloc[:,[test_num_ind[i] - 1 for i in range(len(test_num_ind))]].to_numpy(na_value = 0),
                             test_int_ind, test_df.iloc[:,[test_int_ind[i] - 1 for i in range(len(test_int_ind))]].to_numpy(na_value = 0),
                             test_fac_ind, test_df.iloc[:,[test_fac_ind[i] - 1 for i in range(len(test_fac_ind))]].to_numpy(na_value = 'NA'),
                             n, p, self.get_param())
