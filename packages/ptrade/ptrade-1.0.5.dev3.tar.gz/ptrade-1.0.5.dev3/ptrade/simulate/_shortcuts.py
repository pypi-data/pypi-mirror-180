...
slLine              int|float|None  止盈价格，默认值为None，slLine与slRate同时存在，优先级：slLine >>> slRate
smLine              int|float|None  爆仓价格，默认值为None，逐仓的爆仓价格会自动算出，无需填写，全仓的爆仓价格需要用户指定，否则不会考虑全仓爆仓
orderType           str|None        订单类型，默认值：None
...
endType                 str|None        平仓类型，止盈：TP 止损：SL 爆仓：SM 按照时间段卖出：PT 超过最大持仓时间：TT 模拟交易后以市场价平仓：MT
