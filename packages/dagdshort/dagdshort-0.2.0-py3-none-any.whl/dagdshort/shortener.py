...
class Shortener:
    ...
    def _shorten_url(self, long_url: str) -> str:  # pylint: disable=too-many-locals,method-hidden
        ...
        long_url = urllib.parse.quote(long_url, safe=string.punctuation)  # Ref: https://github.com/dagd/dagd/issues/51
    ...
    def shorten_urls(self, long_urls: list[str]) -> dict[str, str]:
        ...
        # Note: Preprocessing of long_urls is intentionally done in self._shorten_url, and not here, as long_urls is to be returned without modification as dictionary keys.
        ...
        start_time = time.monotonic()
        url_map = dict(url_mapper(lambda long_url: (long_url, self._shorten_url(long_url)), long_urls))
        time_used = time.monotonic() - start_time
