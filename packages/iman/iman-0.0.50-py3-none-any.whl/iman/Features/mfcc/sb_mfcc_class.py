...
class STFT(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        framed = framing(torch.squeeze(x).cpu().detach().numpy(),  self.win_length, self.hop_length).copy()
...
class MFCC(torch.nn.Module):
    ...
    def forward(self, wav):
        ...
        """
        STFT,log_energy = self.compute_STFT(wav)
        ...
        return mfccs,fbanks,log_energy