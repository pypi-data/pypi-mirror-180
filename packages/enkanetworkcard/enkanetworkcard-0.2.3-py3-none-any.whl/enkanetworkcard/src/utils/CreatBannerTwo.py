...
def create_picture(rezFrame,person,imgs,adapt,splash = None):
    if imgs:
        ...
        if splash:
            banner = imagSize(link = splash,size = (1974,1048))
        else:
            banner = imagSize(link = person.images.banner.url,size = (1974,1048))
...
def weaponAdd(weaponRes,characters,lvlName):
    ...
    image = imagSize(link = characters.detail.icon.url,size = (131,138))
...
def stats(statRes,characters,assets):
    ...
    for key in reversed(list(g)):
        ...
        icon = imagSize(image = iconImg,fixed_width = 26)
...
def constant(rezConstant,characters,person):
    ...
    for key in characters.constellations:
        ...
        imageIcon = imgD(link = key.icon.url).resize((43,48))
        if not key.unlocked:
...
def talants(talatsRes,characters):
    ...
    for key in characters.skills:
        ...
        imagesIconTalants = imgD(link = key.icon.url).resize((52,52))
...
def creatArtifact(artifacResSave,infpart,imageStats):
    ...
    artimg = imagSize(link = infpart.detail.icon.url,size = (120,107))
    ...
    for key in infpart.detail.substats:
        ...
        if not imageStats:
            continue
        imageStats= imagSize(image = imageStats,fixed_width = 24) 
...
def creatUserInfo(hide,uid,player,lang, nameCharter = None, namecard = False):
    if not namecard:
        bannerUserNamecard = imagSize(link = player.namecard.banner.url,size = (601,283))
    else:
        linkImgCard = f"https://enka.network/ui/UI_NameCardPic_{nameCharter}_P.png"
        bannerUserNamecard = imagSize(link = linkImgCard,size = (601,283))
    ...
    try:
        avatar = imagSize(link = player.icon.url.url,size = (150,150)) 
    except:
        avatar = imagSize(link = player.avatar.icon.url,size = (150,150))
