...
class TemporalGitRepository(object):
    ...
    def clone(self, repo_dir: Union[str, PathLike]) -> Repo:
        ...
        """
        try:
            key_file = self._key_file[0] if self._key_file else None
            params = get_git_parameters(self.branch, key_file, self.single_branch, self.depth)
            return Repo.clone_from(self._repository_url, repo_dir, **params)
