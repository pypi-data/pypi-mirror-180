...
def main():
    ...
    The script requires 'pandas' and 'numpy' to be installed within the Python environment
    for the script to run.

    Specifying the file path as the first argument will run the main "Crunch" report.
    ...
    try:
        ...
        # Add TB columns
        v_info_data["vInfo TB"] = round(v_info_data["In Use MiB"] / (1024 ** 2), 2)
        v_partition_data["vPartition TB"] = round(
            (v_partition_data["Capacity MiB"] - v_partition_data["Free MiB"])
            / (1024 ** 2),
            2,
        )
    ...
    if args.rdmcheck:
        ...
        v_disk_data["Capacity TB"] = round(v_disk_data["Capacity MiB"] / (1024 ** 2), 2)
        result = (
