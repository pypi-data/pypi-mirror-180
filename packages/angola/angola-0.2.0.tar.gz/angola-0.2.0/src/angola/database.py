...
class Database(object):
    ...
    def select_collection(self, collection_name:str, indexes=None, immut_keys=None, user_defined=True) -> "Collection":
        """
...
class Collection(object):
    ...
    def item(self, data:dict) -> CollectionItem:
        ...
        if not isinstance(data, CollectionItem):
            if "_key" not in data:
                return CollectionItem.new(data, db=self.db, collection=self.collection, commiter=self._commit, immut_keys=self._immut_keys, custom_ops=self._custom_ops)               
        return CollectionItem(data, db=self.db, collection=self.collection, commiter=self._commit, immut_keys=self._immut_keys, custom_ops=self._custom_ops)
