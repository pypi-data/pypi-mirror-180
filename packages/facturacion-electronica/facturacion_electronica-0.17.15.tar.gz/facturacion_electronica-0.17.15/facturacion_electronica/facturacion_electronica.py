...
def enviar_xml(vals):
    ...
    try:
        ...
        _logger.warning("Error en envíar_xml", exc_info=True)
    del envios[key]
    return respuesta
...
def consulta_estado_dte(vals):
    ...
    for d in vals['Documento']:
        for r in d['documentos']:
            ...
            cesion = r.get('cesion', False)
            if cesion:
                del r['cesion']
                documento = Cesion(r)
                documento._dte_emisor = emisor
            else:
                documento = Documento(
                    r,
                    emisor=emisor,
            ...
            conexion.cesion = cesion
            respuesta[documento.ID] = conexion.consulta_estado_dte(
                documento)
    return respuesta

def consulta_estado_cesion_relac(vals):
    firma = Firma(vals["firma_electronica"])
    emisor = Emisor(vals["Emisor"])
    respuesta = {}
    for d in vals['Documento']:
        for r in d['documentos']:
            ...
            conexion.cesion = cesion
            respuesta[documento.ID] = conexion.consulta_estado_cesion_relac(
                documento)
    return respuesta
...
def recepcion_xml(vals):
    vals.update({'test': True})
    try:
        resp = Envio(vals)
        respuestas = resp.do_receipt_deliver()
    except Exception as e:
        msg = "No se pudo crear recepcion XML: " + str(e)
...
def recepcion_mercaderias(vals):
    ...
    try:
        ...
        msg = "No se pudo crear recepción mercadería: " + str(e)
        _logger.warning(msg, exc_info=True)
        respuesta = {
            "error": msg
        }
...
def validacion_comercial(vals):
    ...
    try:
        ...
        msg = "No se pudo crear validación Comercial: " + str(e)
        _logger.warning(msg, exc_info=True)
        respuesta = {
            "error": msg
        }
