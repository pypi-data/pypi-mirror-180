...
class AMLPipelineHelper:
    ...
    def _get_dc_datastore(self):
        if self.config.compute.dc_datastore:
            return self.config.compute.dc_datastore
        else:
            raise UserErrorException(
                "Please specify `compute.dc_datastore` in your compute yaml."
            )
    ...
    def _apply_windows_runsettings(
        ...
        **custom_runtime_arguments,
        ...
        if self.module_loader.is_local(module_name):
            ...
            if (
                self.config.compute.hdi_dc_target
                or self.config.compute.synapse_dc_target
            ):
                datastore_name = self._get_dc_datastore()
        else:
            target = (
                target
                if target is not None
    ...
    def _update_dc_configs(self, pipeline):
        ...
        for id in sorted_node_ids:
            ...
            if node.type != "ScopeComponent" and (
