...
def default_unknown_op(op: bytes, args: CHINILLACLVMObject) -> Tuple[int, CHINILLACLVMObject]:
...
class OperatorDict(dict):
    ...
    def __call__(self, op: bytes, arguments: CHINILLACLVMObject) -> Tuple[int, CHINILLACLVMObject]:
        f = self.get(op)
        if f is None:
