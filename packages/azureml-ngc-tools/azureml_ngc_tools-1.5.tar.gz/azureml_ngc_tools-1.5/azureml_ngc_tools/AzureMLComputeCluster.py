...
class AzureMLComputeCluster:
    ...
    **kwargs: dict
    ...
    def __create_cluster(self):
        ...
        ws_config_file = os.path.abspath(os.path.join(os.getcwd(), ".azureml/config.json"))
        with open(ws_config_file , 'r') as file:
            ws_config = file.read()
     ...
     #   )
        ...
        src = ScriptRunConfig(source_directory=os.path.join(self.abs_path, "setup"),
        script='start_jupyter.py',
        compute_target=self.compute_target,
        environment=self.environment_definition, 
        arguments = ["--output_path", output, "--ws_config", ws_config, "--folders", " ".join(self.folders)])
