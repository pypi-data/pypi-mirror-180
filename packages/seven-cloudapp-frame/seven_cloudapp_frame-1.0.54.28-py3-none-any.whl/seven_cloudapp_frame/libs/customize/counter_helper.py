...
@LastEditTime: 2022-12-07 14:44:47
...
class CounterHelper:
    """
    :description: 计数帮助类 计数在redis的有效时间默认只有半年  配置字段说明{"expire":计数过期时间，单位秒,"计数key":{"db_connect_key": 上下文,"table_name": 表名,"id_field_name": 主键字段名,"value_field_name": 返回字段名}}
    ...
    def amount(self, key_name, object_id, value, sync_update_db=True):
        ...
        try:
            ...
            if counter_config_dict["is_release"] == 0:
                counter_config_dict = {}
            ...
            if sync_update_db == False or (sync_update_db == True and db_update_count>0):
                ...
                if key_expire > 0:
                    init.expire(counter_key, key_expire)
                return count
            else:
                return 0
