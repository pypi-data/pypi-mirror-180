...
class DataBeaver(Object):
    ...
    def build(self, model=None):
        ...
        for full_file_path in model_files:
            ...
            if file_name.count('.') == 3:
                ...
                if index_number > 1:
                    ...
                    self._logger.info(f"{previous_sql_name} is a dependency for {full_file_path}")
        ...
        if self.FIELD_PROCESSES in self._config[self.SECTION_DATA_MODELS][model]:
            processes = int(self._config[self.SECTION_DATA_MODELS][model][self.FIELD_PROCESSES])
        else:
            processes = int(self._config[self.SECTION_DATA_BEAVER][self.FIELD_PROCESSES])
                    ...
                    if ExecutionStatus.SUCCEEDED.value == execution_status:
                        ...
                        for file_name, file in file_info.items():
                            if filename_executed in file['dependencies']:
                                self._logger.info(f"{file_name} dependency removed ({filename_executed}) ")
