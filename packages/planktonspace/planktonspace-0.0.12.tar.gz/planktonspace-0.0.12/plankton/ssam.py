...
def __run_pca(sdata, exclude_bad_clusters, pca_dims, random_state):
    if exclude_bad_clusters:
        good_vecs = sdata.uns['ssam']['normalized_vectors'][sdata.uns['ssam']['filtered_cluster_labels'] != -1, :]
    else:
        good_vecs = sdata.uns['ssam']['normalized_vectors']
    return PCA(n_components=pca_dims, random_state=random_state).fit_transform(good_vecs)

def generate_umap(sdata,run_umap=False, pca_dims=10, metric="correlation", exclude_bad_clusters=True, random_state=0, umap_kwargs={}):
    """generates UMAP of localax coordinates, stored in sdata.uns['ssam']['umap']"""

    if sdata.uns['ssam']['filtered_cluster_labels'] is None:
        exclude_bad_clusters = False
    if run_umap or not ('umap' in sdata.uns['ssam'].keys()):
        pcs = __run_pca(sdata,exclude_bad_clusters, pca_dims, random_state)
        sdata.uns['ssam']['umap'] = UMAP(metric=metric, random_state=random_state, **umap_kwargs).fit_transform(pcs[:, :pca_dims])

def plot_umap(sdata,colors=[],exclude_bad_clusters=True, s=None, excluded_color="#00000033", cmap="jet",):
    """Plots a umap of localmax expression values generated in the function 'generate_umap' """

    if sdata.uns['ssam']['filtered_cluster_labels'] is not None:
        cols = sdata.uns['ssam']['filtered_cluster_labels'][sdata.uns['ssam']['filtered_cluster_labels'] != -1]
    else:
        cols = None

    if len(colors) > 0:
        cmap = ListedColormap(colors)
    if not exclude_bad_clusters and sdata.uns['ssam']['filtered_cluster_labels'] is not None:
        plt.scatter(sdata.uns['ssam']['umap'][:, 0][sdata.uns['ssam']['filtered_cluster_labels'] == -1], sdata.uns['ssam']['umap'][:, 1][sdata.uns['ssam']['filtered_cluster_labels'] == -1], s=s, c=excluded_color)
        plt.scatter(sdata.uns['ssam']['umap'][:, 0][sdata.uns['ssam']['filtered_cluster_labels'] != -1], sdata.uns['ssam']['umap'][:, 1][sdata.uns['ssam']['filtered_cluster_labels'] != -1], s=s, c=cols, cmap=cmap)
    else:
        plt.scatter(sdata.uns['ssam']['umap'][:, 0], sdata.uns['ssam']['umap'][:, 1], s=s, c=cols, cmap=cmap)
    return
...
def plot_localmax(sdata, c=None, cmap=None, s=1, rotate=0):
    """
    ...
    if rotate == 0 or rotate == 2:
        ...
        dim0, dim1 = 0, 1
    plt.scatter(sdata.uns['ssam']['localmax_coords'][dim0], sdata.uns['ssam']['localmax_coords'][dim1], s=s, c=c, cmap=cmap)
    plt.xlim([0, sdata.uns['ssam']['vf_norm'].shape[dim0]])
    plt.ylim([sdata.uns['ssam']['vf_norm'].shape[dim1], 0])
    if rotate == 1:
...
def plot_celltypes_map(sdata, background="black", centroid_indices=[], colors=None, cmap='jet', rotate=0, min_r=0.6, set_alpha=False):
    """
    ...
    """
    # if z is None:
    #     z = int(sdata.uns['ssam']['filtered_'].shape[2] / 2)
    num_ctmaps = np.max(sdata.uns['ssam']['ct_map_filtered']) + 1
    ...
    celltype_maps_internal = np.array(sdata.uns['ssam']['ct_map_filtered'], copy=True)
    ...
    if set_alpha:
        alpha = np.array(sdata.uns['ssam']['max_correlations'], copy=True)
