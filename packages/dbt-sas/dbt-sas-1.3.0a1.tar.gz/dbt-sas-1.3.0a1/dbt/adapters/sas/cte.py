...
from sqlparse.sql import Comment, Function, Identifier, IdentifierList, Parenthesis, Statement
...
def process_cte_definition(token: Identifier, pre: List[str], post: List[str], ctes: Dict[str, str]) -> None:
    ...
    # Get identifier
    token = get_token_skip_whitespace(tokens)
    identifier = get_identifier(token)
    # identifier = f"{WORK_SCHEMA}.{identifier}"
    # Check 'AS' keyword
    token = get_token_skip_whitespace(tokens)
    if token.ttype != Token.Keyword or token.value.lower() != 'as':
...
def process_cte(statement: Statement, pre: List[str], post: List[str], ctes: Dict[str, str]) -> None:
    token = get_token_skip_whitespace(statement)
    if isinstance(token, IdentifierList):  # Multiple CTE
        with_tokens = list(token.tokens)
        while with_tokens:
            with_token = get_token_skip_whitespace(with_tokens)
    ...
    while i_tokens:
        ...
        if isinstance(token, Parenthesis):
            ...
            if insert_schema and identifier.lower() in ctes and not has_schema:
                identifier = ctes[identifier.lower()]
    ...
    while statement:
        ...
        if isinstance(token, Identifier):
            ...
            while id_tokens:
                ...
                if id_token.ttype not in [Token.Text.Whitespace, Token.Text.Whitespace.Newline]:
                    query.append(str(id_token))
                elif id_token.ttype == Token.Punctuation:
                    query.append(str(id_token))
...
def process_tokens(
    ...
    while statement:
        token = statement.pop(0)
        if token.ttype == Token.Keyword.CTE:
...
def prepare_query(sql: str) -> Query:
    ...
    keep_ctes = pre and "".join(query).lower().strip().startswith('create view')
    if keep_ctes:
        post = []
    return Query(
        pre="\n".join(pre),
