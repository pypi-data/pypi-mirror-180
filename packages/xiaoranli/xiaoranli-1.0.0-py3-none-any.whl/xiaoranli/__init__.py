...
def install_packages():
    scripts_path = os.path.join(pkg_installed_path, "scripts/install_packages.sh")
    os.system(f"bash {scripts_path}")
...
def info():
    ...
    cprint(
        "3. you could also use 'xiaoranli install_zsh' to install zsh,then use 'xiaoranli install_packages' to install packages, then use 'xiaoranli setup_rc_files' to setup rc files",
        "red",
    )
