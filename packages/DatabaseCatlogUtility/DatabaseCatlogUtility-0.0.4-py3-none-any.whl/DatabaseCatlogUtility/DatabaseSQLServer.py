...
class DatabaseSQLServer:
    ...
    def getTables(self,connectObj, dbDetails, userInfo, tableName=""):
        logger.log(f"inside DatabaseSQLServer getTables","0")
        resultStr = ""
        transDB = ""
        isTableFound = False
        ...
        try:
            if (len(userInfo) != 0):
                ...
                if ("" == tableName):	
                    ...
                    logger.log(f"resultStr DatabaseSQLServer getTable: {resultStr}","0")
                    ...
                    logger.log(f"DatabaseSQLServer getTables else","0")
                    tableName = "'%"+tableName+"%'";
                    ...
                    logger.log(f"SelectQuery DatabaseSQLServer getTables: {selectQuery}","0")
                    cursor.execute(selectQuery)
                    resultStr = cursor.fetchall()
                    logger.log(f"DatabaseSQLServer getTables IF resultStr = {resultStr}","0")
                ...
                if(not isTableFound ) : 
                    resultStr = self.getErrorXml("Tables not found in the DatabaseSQLServer Database against the Schema "+transDB+"", "Table not Exist")
                    logger.log(f"error String DatabaseSQLServer getTables {resultStr}", "0")
            ...
            if (connectObj != None):
                connectObj.close()
                logger.log(f"DatabaseSQLServer DB connection closed","0")
        return resultStr
                
    def getColumns(self, connectObj, tableNames, userInfo, dbDetails):
        logger.log(f"inside DatabaseSQLServer getColumns()", "0")
        if "transDB" in userInfo.keys():
            if len(userInfo["transDB"]) != 0: 
                transDB = userInfo["transDB"]
        ...
        try:
            if len(userInfo) != 0 :
                if  ("" != tableNames.upper()) or ("" != tableNames.lower()):	
                    ...
                    for j in range(len(tableArray)):
                        ...
                        selectQuery = '''SELECT column_name from INFORMATION_SCHEMA.COLUMNS where TABLE_NAME ="''' + tableArray[j].upper() +'" ORDER BY ORDINAL_POSITION'  
                        logger.log(f"selectQuery: {selectQuery}","0")
                        
                        cursor = connectObj.cursor()
                        cursor.execute(selectQuery)
                        resultStr = cursor.fetchall()
                        logger.log(f"resultStr getColumns DatabaseSQLServer: {resultStr}", "0")
                    ...
                    logger.log(f"mainDataArray DatabaseSQLServer : {mainDataArray}","0")
                    connectObj.close()
                    connectObj = None
            ...
            if connectObj != None:
                connectObj.close()
                logger.log(f"DatabaseSQLServer getColumns Connection closed. ","0" )
