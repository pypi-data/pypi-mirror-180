...
from qctrl.queries.action_input_export import ActionInputExportQuery
...
def action_export(
    api_root, oidc_url, start_date, end_date, ignore_test_users, file_name, path
):
    """
...
@main.command(help="Export an action's input data into a json file.")
@click.option(
    "--api-root",
    help="Custom Q-CTRL API base URL.",
    default=DEFAULT_API_ROOT,
...
@click.option(
    "--action-id", help="The ID of the Action input to export.", required=True
)
@click.option(
    "--pathfile",
    help="The path and filename to store the file at e.g ~/temp/action.json",
    type=click.Path(file_okay=True, dir_okay=False),
)
def action_input_export(api_root, oidc_url, action_id, pathfile):
    """
    Exports action input data to a json file.
    """
    gql_api = create_gql_client(api_root, create_client_auth(oidc_url))
    action_input_export_query = ActionInputExportQuery(gql_api)
    result = action_input_export_query(action_id)

    if not result:
        click.echo("Unable to retrieve action input")
    else:
        # format and save the action input to file
        json_result = json.loads(result)
        _process_output(json_result, pathfile)
