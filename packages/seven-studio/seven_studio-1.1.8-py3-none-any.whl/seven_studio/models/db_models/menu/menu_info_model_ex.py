...
class MenuInfoModelEx(MenuInfoModel):
    ...
    def add_fast_menu(self, parent_id, suffix_name, button_type_str):
        ...
        for button_Type in button_type_list:
            ...
            elif curr_button_type_enum == ButtonType.Delete.value:
    ...
    def add_copy_menu(self, copy_menu_id, stick_menu_id):
        ...
        if not menu_info_list:
            ...
            invoke_result.ResultMessage = "操作失败，没有需要复制的栏目"
            return invoke_result

        sub_menu_list = [i for i in menu_info_list if i.MenuID != copy_menu_id]
        ...
        copy_menu_info.SortIndex += 1
        # 获取父节点信息
        stick_menu = self.get_entity_by_id(stick_menu_id)
        if not stick_menu:
            invoke_result.ResultCode = "error"
            invoke_result.ResultMessage = "操作失败，没有需要复制的栏目"
            return invoke_result
        # 无法拷贝到操作菜单下
        if stick_menu.MenuType == 3:
            invoke_result.ResultCode = "error"
            invoke_result.ResultMessage = "操作失败，无法拷贝到操作菜单下"
            return invoke_result
        # 不能拷贝到子节点下
        if copy_menu_info.Depth < stick_menu.Depth:
            is_sub_menu = [i for i in sub_menu_list if i.IDPath.__contains__(stick_menu_id)]
            if is_sub_menu:
                invoke_result.ResultCode = "error"
                invoke_result.ResultMessage = "操作失败，无法拷贝到子节点下"
                return invoke_result
        copy_menu_info.IDPath = stick_menu.IDPath + copy_menu_info.MenuID + ","
        copy_menu_info.MenuNamePath = stick_menu.MenuNamePath + copy_menu_info.MenuName + ","
        copy_menu_info.Depth = stick_menu.Depth + 1

        self.add_entity(copy_menu_info)
    ...
    def add_copy_child_menu(self, old_parent_id, curr_parent_id, menu_info_list):
        ...
        for curr_menu_info in curr_menu_info_list:
            ...
            # 获取父节点信息
            parent_menu = self.get_entity_by_id(curr_parent_id)
            if parent_menu:
                curr_menu_info.IDPath = parent_menu.IDPath + curr_menu_info.MenuID + ","
                curr_menu_info.MenuNamePath = parent_menu.MenuNamePath + curr_menu_info.MenuName + ","
                curr_menu_info.Depth = parent_menu.Depth + 1
            self.add_entity(curr_menu_info)
            self.add_copy_child_menu(curr_menu_id, curr_menu_info.MenuID, menu_info_list)
    ...
    def clear_menu(self, parent_id=""):
        ...
        :last_editors: HuangJingCan
        """
        menu_list = self.get_list("ParentID=%s", params=[parent_id])
        for menu in menu_list:
            have_child = 1 if self.get_total("ParentID=%s", params=[menu.MenuID]) > 0 else 0
            menu.HaveChild = have_child
            menu.Depth = len(menu.IDPath.strip(',').split(','))
            p_menu = self.get_entity_by_id(menu.ParentID)
            menu.MenuNamePath = p_menu.MenuNamePath + menu.MenuName + "," if p_menu else menu.MenuNamePath

            MenuInfoModelEx(sub_table="copy").add_entity(menu)

            self.clear_menu(menu.MenuID)