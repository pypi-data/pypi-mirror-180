...
class FactorUCB(AbstractRanker):
    ...
    def update(self, batch_users, sampled_items, rankings, clicks, round=None, user_round=None):
        for i in range(len(batch_users)):
            ...
            _clicks, _XVW_optimal = self.__collect_feedback(clicks[i], rankings[i], XVW_optimal)
            ...
            # XW * A^-1 * XW^T
            xAx = np.dot(_XVW_optimal, np.dot(self.AInv[user], _XVW_optimal.T))
            # (1/sigma I + xAx)^-1
            try:
                tmp_inv = np.linalg.inv(1 / self.sigma * np.eye(len(_XVW_optimal)) + xAx)
            ...
            for i in range(len(ranking)):
                ...
                if self.contextual_var:
                    self.d[item] += Theta_v_W * (_clicks[i] - np.dot(self.X[ranking],np.dot(self.Theta_x.T, self.W[user])))
                else:
    ...
    def __collect_feedback(self, click, ranking, batch_X):
        ...
        if self.config.feedback_model == 'cascade':
            ...
            return click[:first_click + 1], batch_X[:first_click + 1]
