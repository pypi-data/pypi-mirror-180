...
def reexec(step: PipelineSteps = typer.Option(..., "--passo-pipeline", "-p", prompt="Escolha o passo do pipeline para reprocessar"),
    ...
    try:
        ...
        typer.echo("fA seguinte exceção foi encontrada: {e}")

@app.command()
def filtro(id_arq: str):
    if "," in id_arq:
    ...
    clipboard = Clipboard()
    clipboard.copy(f"https://app.intuitivecare.com/glosas/auditar-glosas/protocolo/?filters={base64_filtro}")
