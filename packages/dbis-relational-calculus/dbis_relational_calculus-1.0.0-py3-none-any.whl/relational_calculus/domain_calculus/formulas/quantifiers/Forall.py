...
class Forall(dc.Formula):
    ...
    def _rename_variable(
        ...
        if isinstance(self.variable, set):
            new_variable = set(var for var in self.variable if var != old_variable)
            new_variable.add(new_variable)
        return Forall(
            new_variable, self.children[0]._rename_variable(old_variable, new_variable)
        )
