...
from chinilla.cmds.passphrase_funcs import obtain_current_passphrase
...
from chinilla.util.errors import KeychainException
from chinilla.util.file_keyring import MAX_LABEL_LENGTH
from chinilla.util.ints import uint32
from chinilla.util.keychain import Keychain, bytes_to_mnemonic, generate_mnemonic, mnemonic_to_seed
...
def show_all_key_labels() -> None:
    ...
    if len(keys) == 0:
        sys.exit("No keys are present in the keychain. Generate them with 'chinilla keys generate'")
