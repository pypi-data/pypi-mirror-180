...
cd ../chinilla-blockchain-gui/packages/gui || exit 1
...
PRODUCT_NAME="chinilla"
echo electron-builder build --linux rpm "${OPT_ARCH}" \
  --config.productName="${PRODUCT_NAME}" --config.linux.desktop.Name="Chinilla Blockchain" \
  --config.rpm.packageName="chinilla-blockchain"
...
GUI_RPM_NAME="chinilla-blockchain-${CHINILLA_INSTALLER_VERSION}-1.${REDHAT_PLATFORM}.rpm"
mv "dist/${PRODUCT_NAME}-${CHINILLA_INSTALLER_VERSION}.rpm" "../../../build_scripts/dist/${GUI_RPM_NAME}"
