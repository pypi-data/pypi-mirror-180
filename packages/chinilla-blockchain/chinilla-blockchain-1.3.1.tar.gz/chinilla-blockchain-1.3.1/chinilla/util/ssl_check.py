...
import stat
import sys
from chinilla.util.config import load_config, traverse_dict
from chinilla.util.permissions import octal_mode_string, verify_file_permissions
...
def check_ssl(root_path: Path) -> None:
    ...
    if len(invalid_files):
        ...
        print("Run `chinilla init --fix-ssl-permissions` to fix issues.")
