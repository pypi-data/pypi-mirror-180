...
def run(node):
    ...
    * Add the main Maya file and references.
    ...
    paths = evaluate_file_node_frames(node, paths)
...
def evaluate_file_node_frames(submitter_node, paths):
    """
    ...
    for obj_num, path_obj in enumerate(paths):
        ...
        result += _generate_padded_filename_sequence(submitter_node, path_obj, padding_info)
    return result
...
def _generate_padded_filename_sequence(submitter_node, path_obj, padding_info):
    """
    ...
    sequence = Sequence.create(pm.PyNode(submitter_node).attr("frameSpec").get())
    print(sequence)
