...
def get_relocate_data(baremetal_cidr='192.168.7.0/24', overrides={}):
    ...
    if overrides.get('relocate_switch', False) and new_api_vip is not None and new_ingress_vip is not None:
        ...
        if registry:
            ...
            waitcommand = f'[ "$({waitcommand})" != "" ] && kubectl patch storageclass ocs-storagecluster-ceph-rbd '
            ...
            waitcommand = f'[ "$({waitcommand})" == "Completed" ] && exit 0'
