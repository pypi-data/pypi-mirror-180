...
async def create_picture(rezFrame,person,imgs,adapt,splash = None):
    if imgs:
...
async def weaponAdd(weaponRes,characters,lvlName):
    ...
    position,font = centrText(name, witshRam = 290, razmer = 24, start = 177, Yram = 38, y = 17)
    d.text(position, str(name), font= font, fill=coloring) 
    d.text((195 ,111), str(lvlUp), font= t24, fill=(248,199,135,255)) 
    position,font = centrText(f"{lvlName['lvl']}: {lvl}/90", witshRam = 211, razmer = 24, start = 251, Yram = 38, y = 107)
    d.text(position, f"{lvlName['lvl']}: {lvl}/90", font= font, fill=coloring) 

    position,font = centrText(baseAtt, witshRam = 131, razmer = 24, start = 167, Yram = 38, y = 56)    
    d.text(position, str(baseAtt), font= font, fill=coloring)
    if proc:
        position,font = centrText(f'{dopStat}%', witshRam = 131, razmer = 24, start = 333, Yram = 38, y = 56)
        d.text(position, f'{dopStat}%', font= font, fill=coloring)
    else:
        position,font = centrText(str(dopStat), witshRam = 131, razmer = 24, start = 333, Yram = 38, y = 56)
        d.text(position, str(dopStat), font= font, fill=coloring) 
    WeaponBg.paste(stars,(10,135),stars)
    weaponRes.put_nowait(WeaponBg)

async def nameBanner(nameRes,characters,lvlName):
    NameBg = openFile.NameBgTeampleTwo.copy()
    d = ImageDraw.Draw(NameBg)
    centrName,fonts = centrText(characters.name, witshRam = 244, razmer = 24,start = 19, Yram = 29, y = 0)
    ...
    centrName,fonts = centrText(f"{lvlName['lvl']}: {characters.level}/90", witshRam = 209, razmer = 24, start = 77)
...
async def stats(statRes,characters,assets):
    postion = (15,10)
    ...
    for key in reversed(list(g)):
        ...
        pX,fnt = centrText(value, witshRam = 151, razmer = 24, start = 410)
...
async def constant(rezConstant,characters,person):
    constantRes = []  
...
async def talants(talatsRes,characters):
    count = 0
    tallantsRes = []
    for key in characters.skills:
        ...
        px,fnt = centrText(key.level, witshRam = 25, razmer = 18, start = 1, Yram = 25, y = 1) 
...
def naborArtifact(rezArtSet,info,ArtifactNameBg):
    ...
    for key in info:
        if info[key] > 1:
            ...
            centrName,fonts = centrText(key, witshRam = 289, razmer = 24, start = 2, Yram = 28, y = 1) 
...
async def creatArtifact(artifacResSave,infpart,imageStats):
    ...
    centrName,fonts = centrText(val, witshRam = 62, razmer = 17, start = 53)
...
async def artifacAdd(rezArt,rezArtSet,characters):
    artifactRes = {
        "art1": None,
        "art2": None,
        "art3": None,
    ...
    for key in characters.equipments:
        ...
        await creatArtifact(artifactRes[f"art{count}"],key,imageStats)
...
async def creatUserInfo(hide,uid,player,lang, nameCharter = None, namecard = False):
    ...
    try:
        avatar = await imagSize(link = player.icon.url.url,size = (150,150)) 
    except:
    ...
    centrName,fonts = centrText(player.nickname, witshRam = 150, razmer = 19, start = 0)
...
async def appedFrame(frame,weaponRes,nameRes,statRes,constantRes,talatsRes,artifacRes,artifactSet,signatureRes,elementRes,starsRes):
...
async def generationTwo(characters,assets,img,adapt,signatureRes,lvl, splash):
    ...
    try:
        if splash:
            pic = asyncio.create_task(create_picture(frame,person,img,adapt,characters.image.banner.url))
        else:
            pic = asyncio.create_task(create_picture(frame,person,img,adapt))
        wep = asyncio.create_task(weaponAdd(weaponRes,characters.equipments[-1],lvl))
        name = asyncio.create_task(nameBanner(nameRes,characters,lvl))
        state = asyncio.create_task(stats(statRes,characters,assets))
        const = asyncio.create_task(constant(constantRes,characters,person))
        tal = asyncio.create_task(talants(talatsRes,characters))
        artif = asyncio.create_task(artifacAdd(artifacRes,artifactSet,characters))
        await asyncio.gather(pic,wep,name,state,const,tal,artif)
        starsRes = starsAdd(person)
        elementRes = elementIconPanel(person.element.value)
        
        result = await appedFrame(frame.get(),weaponRes.get(),nameRes.get(),statRes.get(),constantRes.get(),talatsRes.get(),artifacRes.get(),artifactSet.get(),signatureRes,elementRes,starsRes)
        return result
