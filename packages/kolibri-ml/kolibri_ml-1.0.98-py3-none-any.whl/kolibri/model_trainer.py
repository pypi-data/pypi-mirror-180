...
#        self.config
    ...
    #            self.y_pred=list(predictions[:,0])
            ...
            if self.config["register-model"]:
                ...
                self.experiment_logger.register_model(model_name, registered_model_version_stage=self.config["model-stage"], archive_existing_versions=self.config["archive-exiting-versions"])
    ...
    def objective(self, X, y):
        ...
        objective=PipelineObjective(X, y, self.pipeline, None, eval_metric=eval_metric,n_jobs=-1, random_state=42, direction=direction)
        return objective

    def _get_optimize_metric(self):
        if self.config['opt-metric-name'] is not None:
            return self.config['opt-metric-name']
