...
class PipelineObjective(Objective):
    ...
    def __call__(self, trial):
        try:
            ...
            if not self.eval_metric.minimize_direction:
                score *= -1.0
    ...
    def update_parameters(self, best):
        if best:
            for key in best:
                for component in self.parameters.items():

                    p = nested_dict_get_key_path(key, component["tunable"])
                    if p is not None:
                        if 'tunable' in p:
                            p.append('value')
                        nested_dict_set_key_value(p, component, best[key])
