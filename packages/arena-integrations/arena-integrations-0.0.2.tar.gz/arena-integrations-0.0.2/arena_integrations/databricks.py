...
class ArenaDatabricks:
    def __init__(
        ...
        self.alias_map = S3JsonInput(access_key, secret_key, read_bucket, "arena-integration/aliases").read()
    ...
    def load_dataset(self, dataset: str) -> pd.DataFrame:
        return spark.read.parquet(f"s3a://{self.read_bucket}/{self.resolve_alias(dataset)}")
