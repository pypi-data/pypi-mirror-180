    ...
    def __init__(self,AppId,AppSecret,token):
        ...
        if os.path.exists(self.path):
            ...
            if not os.path.exists(data_path):
                print(os.path.exists(data_path),1)
                os.makedirs(data_path)
            self.list = {}  # 在获取卡种列表时会自动更新内容
    ...
    def get_list(self,
        ...
        for pageIndex in range(start,count):
            ...
            for item in res:
                ...
                item = [CardType, Title, RemainingCount, UsedCount, TotalCount, CreateTime]
                res_data[IdNo] = item
                item.insert(IdNo)
                # 为避免短号[inx] 重复，或覆盖原值 ，所以
                for inx in range(1000):
                    if not self.list.get(inx):
                        self.list[inx] = item
                        break

            if len(res_data) % pageSize != 0:
                break
        # 更新self.list
        if count == 10000:
            for k,v in self.list.items():
                idNo = v[0]
                if not res_data.get(idNo):
                    self.list.pop(k)
