...
class obj:
    def __init__(
        ...
        self.url = f'http://127.0.0.1:{port1}/DaenWxHook/client/'
    ...
    def getMsg(self,dicData):
        ...
        # 这是名片
            ...
            rs = self.getXmlTxt(xmlCode, "msg", ["username", 'nickname', "antispamticket", 'province', 'city', 'openimdesc'])
    ...
    def get_d0002(self,data, tm):
        ...
        self.wxid2nick_group, self.nick2wxid_group = self.fetch_group(True)
        # 获取所有好友    {wxid:(昵称,微信号),...}
        self.wxid2nick = wx.fetch_firends(True)
        ...
        dic = {
            "type": "D02",
            "info": f"{tm} 登录成功，微信账号[{self.wxNum}]上线，昵称[{self.nick}]",
            "data": {
                'wxid': data['wxid'],  # wxid
    ...
    def get_d0003(self,data, tm):
        ...
        msgType = data["msgType"]  # 消息类型：1|文本 3|图片 34|语音 42|名片 43|视频 47|动态表情 48|地理位置 49|分享链接或附件 2001|红包 2002|小程序 2003|群邀请 10000|系统消息
        ...
        dic来源 = {1: f"好友[{fromWxid}]", 2: f"群[{fromWxid}]->成员[{finalFromWxid}]", 3: f'公众号[{fromWxid}]'}
        ...
        dic["info"] = f'{tm} 收到 @{来源}的 {消息类型}：[{dicMsg["内容"]}]'
    ...
    def get_d0004(self,data, tm):
        ...
        info = f'{tm} {ls1[msgSource-1]}|{ls2[transType-1]}->{nick}[{wxnum}]：{money}元，id:{transferid} 备注：{memo}'
        dic = {
