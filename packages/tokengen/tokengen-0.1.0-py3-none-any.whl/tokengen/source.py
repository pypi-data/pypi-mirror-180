...
def generate_oasis_api_token_for_retailer_token(username=None, password=None, email=None, registry=None, admin=None, vault_url=None, secret_name=None):
    ...
    try:
        ...
        image_name = "registry.gitlab.com/aifi-ml/production/cloud-api/utils:fix-token-generation"
        environment_variables = ["type=aifi", "PEPPER={}".format(os.getenv("PEPPER")), "ADMIN={}".format(admin)]
        ...
        token = subprocess.check_output(('grep', 'token'), stdin=proc.stdout, text=True).strip(" '").replace("token: '", "").replace("'", "")
