...
def download_deployment_folder(model_name: str, env: str, **kwargs) -> None:
    ...
    if latest_version < version:
        ...
        if len(files_to_ignore) > 0:
            for file in os.listdir(final_target_path):
                if file in files_to_ignore:
                    file_to_remove = os.path.join(final_target_path, file)
                    logger.info(
                        f"Removing file {file_to_remove} from deployment folder"
                    )
                    os.remove(file_to_remove)
        else:
            logger.info("No files to remove, keeping entire deployment folder")
