...
def robust_create(create_function: partial) -> None:
    ...
    for trial in range(max_errors):
        logging.info(f"Creating function (trial {trial+1}/{max_errors})")
        ...
        if function.status == "Failed" and trial < max_errors - 1:
            ...
            logging.info(f"Error was: {function.error=}")
            logging.info("Try to create function again")
        else:
            raise Exception(
...
def download_file(
    ...
    if external_id is not None:
        logging.info(f"Download file with {external_id=} to {path}")
        client.files.download_to_path(path, **external_id)
    else:
        logging.info(f"Download file with {id=} to {path}")
...
def delete_file(id: Dict) -> None:
    ...
    logging.info(f"Deleted file with {id=}")
