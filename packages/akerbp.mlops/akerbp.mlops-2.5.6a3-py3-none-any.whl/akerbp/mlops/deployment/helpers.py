...
def copy_to_deployment_folder(lst: Dict, deployment_folder: Path) -> Dict:
    """
...
def rm_deployment_folder(model: str) -> None:
    logging.info("Deleting deployment folder")
...
def run_tests(c: ServiceSettings, **kwargs) -> Any:
    ...
    if c.test_file:
        ...
        logging.info(f"Running tests for model {c.model_name}")
        try:
        ...
        if setup_venv:
            ...
            logging.info("Ephemeral virtual environment deleted")
...
def deploy_model(
    ...
    logging.info(f"Starting deployment of model {c.model_name} to {env}")
    latest_artifact_version = cdf.get_latest_artifact_version(external_id=function_name)
    if c.artifact_version is None:
        logging.info(f"Latest artifact version in {env} is {latest_artifact_version}")
...
def redeploy_model_with_predictable_external_id(
    ...
    if c.test_file:
        ...
        logging.warning("No test file was specified in the settings, skipping tests")
...
def garbage_collection(
    ...
    if models_to_keep is None:
        if keep_all_models is True:
            logging.info("Keeping all models alive as specified in the settings file")
        else:
            logging.info(
                "Number of models to keep alive not specified in settings file"
            )
            logging.info("Setting number of models to keep to 3 (default)")
            models_to_keep = 3
    else:
        logging.info(
    ...
    if models_to_keep >= len(external_ids):
        ...
        for external_id in external_ids_to_delete:
            ...
            logging.info(f"Deleting version {version} of model {c.model_name} in {env}")
