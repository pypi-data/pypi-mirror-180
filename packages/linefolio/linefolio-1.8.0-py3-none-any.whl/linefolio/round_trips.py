...
PNL_STATS = OrderedDict(
    [
        ...
        (
            "Profit factor",
            lambda x: x[x > 0].sum() / x[x < 0].abs().sum() if x[x < 0].abs().sum() != 0 else np.nan,
        ),
...
def agg_all_long_short(round_trips, col, stats_dict):
    stats_all = round_trips.assign(ones=1).groupby("ones")[col].agg(**stats_dict).T.rename(columns={1.0: "All trades"})
    stats_long_short = (
...
TIMEDELTA_8H = pd.Timedelta("8 hours")


def _groupby_consecutive(txn, max_delta=TIMEDELTA_8H):
...
def extract_round_trips(transactions, portfolio_value=None):
    ...
    if portfolio_value is not None:
        ...
        pv = pd.DataFrame(portfolio_value, columns=["portfolio_value"]).assign(date=portfolio_value.index)
...
def print_round_trip_stats(round_trips, hide_pos=False):
    ...
    print_table(stats["returns"] * 100, float_format="{:.2f}%".format, name="Return stats")

    if not hide_pos:
        stats["symbols"].columns = stats["symbols"].columns.map(format_asset)
        print_table(stats["symbols"] * 100, float_format="{:.2f}%".format, name="Symbol stats")
