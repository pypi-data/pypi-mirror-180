...
from . import capacity, plotting, pos, round_trips, timeseries, txn, utils
...
def create_full_tear_sheet(
    ...
    create_interesting_times_tear_sheet(returns, benchmark_rets=benchmark_rets, set_context=set_context)
...
def create_simple_tear_sheet(
    ...
    if positions is not None:
        ...
        plotting.plot_long_short_holdings(returns, positions_alloc, ax=ax_long_short_holdings)
...
def create_returns_tear_sheet(
    ...
    plotting.plot_return_quantiles(returns, live_start_date=live_start_date, ax=ax_return_quantiles)
...
def create_txn_tear_sheet(
    ...
    plotting.plot_turnover(returns, transactions, positions, turnover_denom=turnover_denom, ax=ax_turnover)

    plotting.plot_daily_volume(returns, transactions, ax=ax_daily_volume)

    try:
        plotting.plot_daily_turnover_hist(transactions, positions, turnover_denom=turnover_denom, ax=ax_turnover_hist)
    except ValueError:
    ...
    if unadjusted_returns is not None:
        ...
        plotting.plot_slippage_sweep(unadjusted_returns, positions, transactions, ax=ax_slippage_sweep)
        ax_slippage_sensitivity = plt.subplot(gs[5, :])
        plotting.plot_slippage_sensitivity(unadjusted_returns, positions, transactions, ax=ax_slippage_sensitivity)
    for ax in fig.axes:
        # Matplotlib 2
        plt.setp(ax.get_xticklabels(), visible=True)
...
def create_interesting_times_tear_sheet(
    ...
    if benchmark_rets is not None:
        ...
        bmark_interesting = timeseries.extract_interesting_date_ranges(benchmark_rets, periods)
    ...
    for i, (name, rets_period) in enumerate(rets_interesting.items()):
        ...
        ep.cum_returns(rets_period).plot(ax=ax, color="forestgreen", label="algo", alpha=0.7, lw=2)

        if benchmark_rets is not None:
            ep.cum_returns(bmark_interesting[name]).plot(ax=ax, color="gray", label="benchmark", alpha=0.6)
