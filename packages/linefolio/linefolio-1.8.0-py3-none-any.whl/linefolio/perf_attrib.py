...
def create_perf_attrib_stats(perf_attrib, risk_exposures):
    ...
    annualized_returns_by_factor = [ep.annual_return(perf_attrib[c]) for c in risk_exposures.columns]
    cumulative_returns_by_factor = [ep.cum_returns_final(perf_attrib[c]) for c in risk_exposures.columns]
...
def show_perf_attrib_stats(
    ...
    perf_attrib_stats, risk_exposure_stats = create_perf_attrib_stats(perf_attrib_data, risk_exposures)
...
def plot_returns(perf_attrib_data, cost=None, ax=None):
    ...
    ax.plot(ep.cum_returns(specific_returns), color="g", label="Cumulative specific returns")
...
def _align_and_warn(
    ...
    if len(missing_stocks) > 0:
        ...
        if len(missing_stocks) > 5:
            ...
            missing_stocks_displayed = ("The following assets were missing factor loadings: {}.").format(
                list(missing_stocks)
            )
    ...
    missing_factor_loadings_index = positions.index.difference(factor_loadings.index.get_level_values(0).unique())

    if len(missing_factor_loadings_index) > 0:

        if len(missing_factor_loadings_index) > 5:
            missing_dates_displayed = ("(first missing is {}, last missing is {})").format(
        ...
        warning_msg = (
            "Could not find factor loadings for {} dates: {}. " "Truncating date range for performance attribution. "
