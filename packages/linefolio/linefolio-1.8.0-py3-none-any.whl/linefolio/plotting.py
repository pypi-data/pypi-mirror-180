...
def plot_long_short_holdings(returns, positions, legend_loc="upper left", ax=None, **kwargs):
    """
...
def plot_perf_stats(returns, factor_returns, ax=None):
    ...
    bootstrap_values = timeseries.perf_stats_bootstrap(returns, factor_returns, return_stats=False)
...
def plot_rolling_returns(
    ...
    **kwargs, optional
    ...
    if factor_returns is not None:
        cum_factor_returns = ep.cum_returns(factor_returns.loc[factor_returns.index.intersection(cum_rets.index)], 1.0)
...
def plot_rolling_beta(returns, factor_returns, legend_loc="best", ax=None, **kwargs):
    ...
    **kwargs, optional
    ...
    ax.set_ylabel("Beta")
    rb_1 = timeseries.rolling_beta(returns, factor_returns, rolling_window=APPROX_BDAYS_PER_MONTH * 6)
    rb_1.plot(color="steelblue", lw=3, alpha=0.6, ax=ax, **kwargs)
    rb_2 = timeseries.rolling_beta(returns, factor_returns, rolling_window=APPROX_BDAYS_PER_MONTH * 12)
...
def plot_rolling_volatility(
    returns, factor_returns=None, rolling_window=APPROX_BDAYS_PER_MONTH * 6, legend_loc="best", ax=None, **kwargs
):
    """
    ...
    **kwargs, optional
    ...
    if factor_returns is not None:
        rolling_vol_ts_factor = timeseries.rolling_volatility(factor_returns, rolling_window)
...
def plot_rolling_sharpe(
    ...
    **kwargs, optional
    ...
    if factor_returns is not None:
        rolling_sharpe_ts_factor = timeseries.rolling_sharpe(factor_returns, rolling_window)
...
def show_and_plot_top_positions(
    returns, positions_alloc, show_and_plot=2, hide_positions=False, legend_loc="real_best", ax=None, **kwargs
):
    """
...
def plot_return_quantiles(returns, live_start_date=None, ax=None, **kwargs):
    ...
    **kwargs, optional
    ...
    is_returns = returns if live_start_date is None else returns.loc[returns.index < live_start_date]
    is_weekly = ep.aggregate_returns(is_returns, "weekly")
    is_monthly = ep.aggregate_returns(is_returns, "monthly")
    sns.boxplot(data=[is_returns, is_weekly, is_monthly], palette=["#4c72B0", "#55A868", "#CCB974"], ax=ax, **kwargs)
...
def plot_turnover(returns, transactions, positions, turnover_denom="AGB", legend_loc="best", ax=None, **kwargs):
    """
...
def plot_slippage_sweep(
    returns, positions, transactions, slippage_params=(3, 8, 10, 12, 15, 20, 50), ax=None, **kwargs
):
    """
    ...
    **kwargs, optional
    ...
    for bps in slippage_params:
        adj_returns = txn.adjust_returns_for_slippage(returns, positions, transactions, bps)
        label = str(bps) + " bps"
...
def plot_capacity_sweep(
    ...
    for start_pv in range(min_pv, max_pv, step_size):
        adj_ret = capacity.apply_slippage_penalty(returns, txn_daily_w_bar, start_pv, bt_starting_capital)
...
def plot_daily_turnover_hist(transactions, positions, turnover_denom="AGB", ax=None, **kwargs):
    """
