...
from linefolio.perf_attrib import _cumulative_returns_less_costs, create_perf_attrib_stats, perf_attrib
...
class PerfAttribTestCase(unittest.TestCase):
    ...
    def test_perf_attrib_regression(self):
        ...
        positions = pd.read_csv("linefolio/tests/test_data/positions.csv", index_col=0, parse_dates=True)

        positions.columns = [int(col) if col != "cash" else col for col in positions.columns]

        returns = pd.read_csv(
            "linefolio/tests/test_data/returns.csv",
            index_col=0,
            parse_dates=True,
            header=None,
            squeeze=True,
        ...
        residuals = pd.read_csv("linefolio/tests/test_data/residuals.csv", index_col=0, parse_dates=True)

        residuals.columns = [int(col) for col in residuals.columns]

        intercepts = pd.read_csv(
            "linefolio/tests/test_data/intercepts.csv",
            index_col=0,
            header=None,
            squeeze=True,
        )
        ...
        pd.util.testing.assert_series_equal(returns, common_returns, check_names=False)

        self.assertTrue(np.isclose(specific_returns, 0).all())

        # specific and common returns combined should equal total returns
        pd.util.testing.assert_series_equal(returns, combined_returns, check_names=False)
        ...
        pd.util.testing.assert_series_equal(expected_common_returns, common_returns, check_names=False)
        ...
        self.assertEqual(ep.cum_returns_final(combined_returns), perf_attrib_summary["Total Returns"])

        avg_factor_exposure = risk_exposures_portfolio.mean().rename("Average Risk Factor Exposure")
        pd.util.testing.assert_series_equal(avg_factor_exposure, exposures_summary["Average Risk Factor Exposure"])

        cumulative_returns_by_factor = pd.Series(
            [ep.cum_returns_final(perf_attrib_output[c]) for c in risk_exposures_portfolio.columns],
            name="Cumulative Return",
            index=risk_exposures_portfolio.columns,
        )

        pd.util.testing.assert_series_equal(cumulative_returns_by_factor, exposures_summary["Cumulative Return"])

        annualized_returns_by_factor = pd.Series(
            [ep.annual_return(perf_attrib_output[c]) for c in risk_exposures_portfolio.columns],
            name="Annualized Return",
            index=risk_exposures_portfolio.columns,
        )

        pd.util.testing.assert_series_equal(annualized_returns_by_factor, exposures_summary["Annualized Return"])
    ...
    def test_missing_stocks_and_dates(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            self.assertIn(
                "Could not find factor loadings for " "{} dates".format(len(missing_dates)),
                str(w[-1].message),
            )
    ...
    def test_cumulative_returns_less_costs(self):
        ...
        pd.util.testing.assert_series_equal(expected_returns, _cumulative_returns_less_costs(returns, None))

        expected_returns = pd.Series([0.099000, 0.207801, 0.327373], index=returns.index)
        pd.util.testing.assert_series_equal(expected_returns, _cumulative_returns_less_costs(returns, cost))
