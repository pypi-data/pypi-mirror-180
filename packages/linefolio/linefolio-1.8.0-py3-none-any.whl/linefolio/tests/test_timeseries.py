...
from linefolio.utils import to_series, to_utc
...
class TestDrawdown(TestCase):
    ...
    def test_gen_drawdown_table_relative(
        ...
        self.assertEqual(np.round(drawdowns.loc[0, "Net drawdown in %"]), first_net_drawdown)
        ...
        self.assertEqual(np.round(drawdowns.loc[1, "Net drawdown in %"]), second_net_drawdown)
    ...
    px_list_2 = np.array([100, 120, 100, 80, 70, 80, 90, 90]) / 100.0  # Ends in drawdown
    ...
    def test_get_max_drawdown(self, px, expected_peak, expected_valley, expected_recovery):
        ...
        self.assertTrue(pd.isnull(recovery)) if expected_recovery is None else self.assertEqual(
    ...
    def test_gen_drawdown_table(self, px, expected_peak, expected_valley, expected_recovery, expected_duration):
        rets = px.pct_change().iloc[1:]

        drawdowns = timeseries.gen_drawdown_table(rets, top=1)
        self.assertTrue(pd.isnull(drawdowns.loc[0, "Peak date"])) if expected_peak is None else self.assertEqual(
            drawdowns.loc[0, "Peak date"], expected_peak
        )
        self.assertTrue(pd.isnull(drawdowns.loc[0, "Valley date"])) if expected_valley is None else self.assertEqual(
            drawdowns.loc[0, "Valley date"], expected_valley
        )
        self.assertTrue(
            pd.isnull(drawdowns.loc[0, "Recovery date"])
        ) if expected_recovery is None else self.assertEqual(drawdowns.loc[0, "Recovery date"], expected_recovery)
        self.assertTrue(pd.isnull(drawdowns.loc[0, "Duration"])) if expected_duration is None else self.assertEqual(
    ...
    def test_drawdown_overlaps(self):
        ...
        spy_drawdowns = timeseries.gen_drawdown_table(spy_returns, top=20).sort_values(by="Peak date")
...
class TestNormalize(TestCase):
    ...
    @parameterized.expand([(pd.Series(np.array(px_list) * 100, index=dt), pd.Series(px_list, index=dt))])
...
class TestStats(TestCase):
    ...
    simple_week_rets = pd.Series([0.1] * 3 + [0] * 497, pd.date_range("2000-1-31", periods=500, freq="W"))

    simple_month_rets = pd.Series([0.1] * 3 + [0] * 497, pd.date_range("2000-1-31", periods=500, freq="M"))
    ...
    @parameterized.expand([(simple_rets[:5], 2, [0, np.inf, np.inf, 11.224972160321, 0])])
    def test_sharpe_2(self, returns, rolling_sharpe_window, expected):
        np.testing.assert_array_almost_equal(
            timeseries.rolling_sharpe(returns, rolling_sharpe_window).fillna(0).values,
            np.asarray(expected),
        )
...
class TestBootstrap(TestCase):
    ...
    def test_calc_bootstrap(self, true_mean, true_sd, n):
        ...
        assert_almost_equal(
            ...
            mean_of_mean,
            3,
            "Mean of bootstrap does not match theoretical mean of" "sampling distribution",
        )
