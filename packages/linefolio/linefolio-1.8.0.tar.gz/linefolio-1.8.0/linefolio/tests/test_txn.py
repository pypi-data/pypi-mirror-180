...
from linefolio.txn import adjust_returns_for_slippage, get_turnover
...
class TransactionsTestCase(TestCase):
    def test_get_turnover(self):
        ...
        transactions = DataFrame(data=[], columns=["sid", "amount", "price", "symbol"], index=dates)
        ...
        result = get_turnover(positions, transactions, denominator="portfolio_value")
    ...
    def test_adjust_returns_for_slippage(self):
        ...
        positions = DataFrame([[0.0, 10.0]] * len(dates), columns=[0, "cash"], index=dates)
        ...
        result = adjust_returns_for_slippage(returns, positions, transactions, slippage_bps)
