...
from linefolio.round_trips import _groupby_consecutive, add_closing_transactions, extract_round_trips
...
class RoundTripTestCase(TestCase):
    ...
    def test_groupby_consecutive(self, transactions, expected):
        ...
        assert_frame_equal(grouped_txn.sort_index(axis="columns"), expected.sort_index(axis="columns"))
    ...
    @parameterized.expand(
        [
            ...
            (
                ...
                DataFrame(
                    data=[[dates[0], dates[1], Timedelta(days=1), 10.0, 0.5, True, "A"]],
                    columns=[
                        "open_dt",
                        "close_dt",
                        "duration",
            ...
            (
                ...
                DataFrame(
                    data=[[dates[0], dates[2], Timedelta(days=2), -10.0, -0.2, True, "A"]],
                    columns=[
                        "open_dt",
                        "close_dt",
                        "duration",
            ...
            (
                ...
                DataFrame(
                    data=[
                        [dates[0], dates[1], Timedelta(days=1), 10.0, 0.5, True, "A"],
                        [
                            dates[1],
                            dates[2],
                            Timedelta(days=1),
    ...
    def test_extract_round_trips(self, transactions, expected, portfolio_value=None):
        ...
        assert_frame_equal(round_trips.sort_index(axis="columns"), expected.sort_index(axis="columns"))
    ...
    def test_add_closing_trades(self):
        ...
        expected = DataFrame(
            data=[["A", 2, 10], ["A", -5, 10], ["B", -1, 10.0], ["A", 3, 20.0]],
    ...
    def test_txn_pnl_matches_round_trip_pnl(self):
        ...
        transactions_closed["txn_dollars"] = transactions_closed.amount * -1.0 * transactions_closed.price
        round_trips = extract_round_trips(transactions_closed)

        self.assertAlmostEqual(round_trips.pnl.sum(), transactions_closed.txn_dollars.sum())
