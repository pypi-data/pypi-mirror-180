...
def test_wait_for_code(wait_for_code_tester):
    def do_test():
        r = requests.get("http://localhost:11001/callback?code=test_wait_for_code&state=foo")
        assert r.status_code == 200
        assert "Login Successful!" in r.text

    wait_for_code_tester(expected_state="foo", expected_code="test_wait_for_code", do_test=do_test)
...
def test_wait_for_code_invalid_state(wait_for_code_tester):
    def do_test():
        r = requests.get("http://localhost:11001/callback?code=test_wait_for_code&state=bar")
        assert r.status_code == 401
...
def test_get_token_from_code_ok(mocker, browser_redirect_flow, global_options, test_org):
    ...
    token = browser_redirect_flow.get_token_from_code(global_options, test_org, "1234XYZ", "test-verifier")
...
def test_get_token_from_code_missing_refresh_token(mocker, browser_redirect_flow, global_options, test_org):
    with pytest.raises(InvalidTokenError):
        ...
        mocker.patch.object(requests, "post", return_value=response)
        mocker.patch.object(response, "json", return_value=response_json)
        browser_redirect_flow.get_token_from_code(global_options, test_org, "1234XYZ", "test-verifier")


def test_get_token_from_code_error_response(mocker, browser_redirect_flow, global_options, test_org):
...
def test_login(mocker, global_options, browser_redirect_flow, auth_token, test_org):
    ...
    mocker.patch.object(browser_redirect_flow, "get_token_from_code", return_value=auth_token)
...
@patch(
    "sym.flow.cli.helpers.login.login_flow.BrowserRedirectFlow._new_server",
)
def test_login_no_auth_code(_, browser_redirect_flow, global_options, test_org):
