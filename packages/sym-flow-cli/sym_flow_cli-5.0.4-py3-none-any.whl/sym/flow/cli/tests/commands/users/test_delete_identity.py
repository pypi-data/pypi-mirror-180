...
class TestUsersDeleteIdentity:
    ...
    def patchypatch(self, users, services):
        with ExitStack() as stack:
            stack.enter_context(
                patch(
                    "sym.flow.cli.helpers.api.SymAPI.get_users", return_value=[users[0]]
                )
            )
            stack.enter_context(
                patch(
            ...
            stack.enter_context(
                patch(
                    "sym.flow.cli.helpers.api_operations.OperationHelper.handle_delete_identities"
                )
            )
            yield

    def test_delete_user_identity(self, click_setup, users):
    ...
    def test_delete_user_identity_prompt_both(self, click_setup, users, services):
        with click_setup() as runner:
    ...
    def test_delete_user_identity_prompt_external_id(self, click_setup, users, services):
        with click_setup() as runner:
            # Prompts external ID
            slack_external_ids = sorted(
                [
            ...
            with patch(
                "inquirer.list_input", side_effect=[slack_external_ids[0]]
            ) as mock_inquire:
                result = runner.invoke(
                    click_command,
                    [
    ...
    def test_delete_user_identity_prompt_service_type(self, click_setup, users, services):
        with click_setup() as runner:
    ...
    def test_delete_user_identity_does_not_prompt_external_id(
        self, click_setup, users, services
    ):
        with click_setup() as runner:
