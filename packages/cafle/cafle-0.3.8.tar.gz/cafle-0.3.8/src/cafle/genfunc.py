...
__all__ = ['PY', 'Area', 'EmptyClass', 'is_iterable', 'limited', 'rounding', 
           'print_rounding', 'round_up', 'log10', 'extnddct', 'margindf', 'showdf']
...
def margindf(iptval):
    """
    This method returns DataFrame or Series with sum value on the margin of it.

    Parameters
    ----------
    iptval : DataFrame, Series

    Returns
    -------
    DataFrame, Series
    """
    if type(iptval) == DataFrame:
        rsltdf = iptval.copy()
        sumdf = DataFrame(columns=rsltdf.columns, index=['Sum'])

        for key, item in rsltdf.items():
            if is_numeric_dtype(item):
                dtype = item.dtype
                sumval = item.sum()
                sumdf.loc['Sum', key] = sumval
                sumdf[key] = sumdf[key].astype(dtype)
            else:
                sumdf.loc['Sum', key] = '-'
        rsltdf = pd.concat([rsltdf, sumdf])
        return rsltdf

    if type(iptval) == Series:
        rsltsrs = iptval.copy()
        if is_numeric_dtype(rsltsrs):
            dtype = rsltsrs.dtype
            sumval = rsltsrs.sum()
        else:
            sumval = '-'
        sumsrs = Series(sumval, index=['Sum']).astype(dtype)
        rsltsrs = pd.concat([rsltsrs, sumsrs])
        return rsltsrs


def showdf(iptval, margins=False):
    """
    This method shows DataFrame or Series more easily understood.

    Parameters
    ----------
    iptval : DataFrame or Series
    margins : True or False

    Returns
    -------
    DataFrame or Series rounded and applied a separator to the number.
    And margins option makes sum row on the margin.
    ...
    """
    if margins is True:
        iptval = margindf(iptval).copy()

    if type(iptval) == DataFrame:
        rsltdf = iptval.copy()
        for key, item in rsltdf.items():
            if is_numeric_dtype(item):
                rsltdf[key] = rsltdf[key].fillna(0).apply(lambda x: f"{x:,.0f}")
        return rsltdf

    if type(iptval) == Series:
        rsltsrs = iptval.copy()
        if is_numeric_dtype(rsltsrs):
            rsltsrs = rsltsrs.fillna(0).apply(lambda x: f"{x:,.0f}")
        return rsltsrs
