...
class TableElement(ScreenElement):
    ...
    class TableRowModel(object):
        ...
        @property
        def course(self):
            return self.driver.find_element_by_css_selector(self.WORK).text

        @property
        def salary(self):
            return self.driver.find_element_by_css_selector(self.SALARY).text
    """
    ...
    def __get__(self, obj, owner):
        ...
        """
        self._refresh(obj)
        table_rows = [self._table_entry_cls(table_row) for table_row in self.driver]
        header = table_rows[1]
        rows = table_rows[1:]
        return {'table_rows': table_rows, 'header': header, 'rows': rows}
