...
class ScreenElement(object):
    ...
    @aspect.log
    def locator_is_none(self, obj):
        """
        When inheriting the screen element and overriding the __get__ method, it is preferable to call this method first.
    ...
    def __get__(self, obj, owner):
        ...
        When overriding the ScreenElement "if" block is desirable to handle cases when locator is not set in constructor
        """
        if self.locator_is_none(obj):
            return self
        else:
            self._refresh(obj)
