    ...
    def _query_handler(self, start_date: str, end_date: str, channel_ids: list) -> list:
        ...
        if endpoint_name == "content_metrics":
            ...
            for channel_id in channel_ids:
                ...
                results = self._published_posts_by_channel_id(start_date,end_date, [channel_id])
                dataset.extend(results)
                self.start += 1
        else:
...
class CustomFalconReaderV2:
    ...
    def _get_content_ids_by_channel_id(
        ...
        endpoint_url=f"publish/items?apikey={self._creds['api_key']}"\
        ...
        while endpoint_url is not None:
            ...
            for item in response_data.get("items"):
                ...
                if channel_id is not None and channel_id[0] in channel_ids:
                    content_ids_by_channel_id.setdefault(channel_id[0], []).append(content_id)
    ...
    def _get_insights_request_id(
                                    ...
                                    start_date: str,
                                    end_date: str,
                                    content_ids_by_channel_id: dict
                                ) -> dict:
    ...
    def _get_content_insights(self,insights_request_id: str):
        ...
        endpoint_url = f"measure/v2/insights/{insights_request_id}?apikey={self._creds['api_key']}"
        url=f"{self.base_url}{endpoint_url}"
    ...
    def _normalize_data(self,dataset: dict, network: str):
        normalized_data = []
    ...
    def _query_handler(
        ...
        return self._normalize_data(dataset,network)
