...
from .rbasic import check_parm, get_first_notnull, if_iterable
    ...
    def execute(
        ...
        **kwargs: "object"
        ...
        if self.conn:
            ...
            conn = self.create_conn(only_once=only_once)
        if parms != None or kwargs != {}:
            if parms != None and kwargs != {}:
                if type(parms) == list:
                    for parm in parms:
                        parm.update(kwargs)
                else:
        ...
        if limit != None:
            ...
            if list_type in [str, int]:
                ...
                if len(limit) in [1, 2]:
                    ...
                    error = ValueError("The length of the limit parameter value must be 1 or 2")
                    raise error
            sqls.append(limit_sql)
        sql = "\n".join(sqls)
        if print_sql:
            rprint(sql, title="SQL", format_module=None)
