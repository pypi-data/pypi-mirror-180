...
def get_driver_path():
    ...
    if not v:
        print("can not detect chrome version, use latest")
        return ChromeDriverManager(cache_valid_range=1000).install()
...
class Driver:
    ...
    def set_chrome(self, keep_open: str = "Close", download_dir: str = None, load_cookies: bool = False, proxy=""):
        ...
        try:
            self.driver_ = webdriver.Chrome(service=Service(driver_path.path), options=chrome_options)
