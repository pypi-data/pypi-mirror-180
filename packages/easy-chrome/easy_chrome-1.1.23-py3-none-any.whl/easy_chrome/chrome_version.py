...
def get_driver_path():
    # WDM call a Window system cmd that be blocked, so use an custom validator collector instead
    _os_name = os_name()
    v = _custom_get_win_chrome_version() if _os_name == OSType.WIN else None
    if not v:
        v = get_browser_version_from_os(ChromeType.GOOGLE)

    print(f"found chromedriver version: {v}")
