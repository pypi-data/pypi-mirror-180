...
class LzConcurrentImpThreading(LzConcurrent):
...
class LzConcurrentImpThreadingPool(LzConcurrent):
...
class LzConcurrentImpProcessing(LzConcurrent):
...
class LzConcurrentImpProcessingPool(LzConcurrent):
...
class LzConcurrentAsync(LzConcurrent):
    ...
    def callback_function(self, fun: Callable, args, future):
        return fun(*args)

    def add_async_function(self, task_name: str, task: Callable, *args, **kwargs):
    ...
    def run_task(self, task_name: str, callback: Callable = None, callback_arg=None):
        ...
        if callback is not None:
            ...
            async_task.add_done_callback(partial(self.callback_function, callback, callback_arg))

        # async_task.add_done_callback(partial(callback, *callback_arg))
        self.__future_task_dict[task_name] = async_task
        return self

    def add_and_run_async_function(self, task_name: str, task: Callable, *args, **kwargs):
        self.add_async_function(task_name, task, *args, **kwargs)
    ...
    def wait_all_finished_and_get_result(self):
