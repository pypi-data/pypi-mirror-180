...
def check_index_path_option(ctx: Any, param: Any, value: Union[Path, None]) -> Path:
    if value:
        print('Value is already a path.')
        return value
...
@click.option('--compression', type=click.Choice(('bz2', 'gzip')), default='bz2', show_default=True,
...
@click.option('--residue_type_policy', default='strict', callback=check_residue_type_policy_option,
               help='''\b\nOption to control whether mutated versions of the query motif should also be tested. There are 4 possibilities:\n 1) strict (DEFAULT): only similar motifs with strictly identical residues to the query motif are searched.\n 2) relaxed: finds PDBs with motifs that are similar but with potential mutated residues by allowing residues to mutate according to the residue group they are part of, such as polar, positively charged, aromatic, etcetc (see our Github for the exact grouping of the residues).\n 3) fully_relaxed: similar to n° 2), but allows residues to mutate to any of the 19 other possible residues.\n 4) custom position specific exchange: specify possible residue mutations for the desired positions in your motif using a JSON formated string (e.g: '{"A1":["K", "L"], "A2":["Y", "W"], "A5":["E"]}'. Note the use of single and double quotes).\n To control the maximum number of simultaneous mutations, see the max_n_mutated_residues option.''')
@click.option('--max_n_mutated_residues', type=click.IntRange(min=0), default=0, show_default=True,
               help='Maximum number of simultaneous mutations that should be allowed when generating mutated motifs through the residue_type_policy option. For example, when set to 1, versions of the query motif with 1 residue mutated according to the given residue_type_policy will be generated and checked, in addition to the query motif itself. Note that very high values for this option will result in longer computation time given all the possible mutated versions of your motif are searched for in the index. For most use cases, we recommend to use a value between 1 and 3.')
