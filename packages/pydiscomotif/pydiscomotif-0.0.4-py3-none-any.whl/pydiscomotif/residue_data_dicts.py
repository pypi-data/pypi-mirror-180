...
def decompress_gzip_PDB_file(PDB_file_path: Path) -> Path:
    """
    Decompresses the PDB file in the same directory and returns the path of the decompressed file.
    Taken from https://stackoverflow.com/a/44712152/12397370.
    """
    decompressed_PDB_file_path = PDB_file_path.with_suffix('') # Removes .gz suffix
    with gzip.open(PDB_file_path, 'rb') as input_file_handle, open(decompressed_PDB_file_path, 'wb') as output_file_handle:
        shutil.copyfileobj(input_file_handle, output_file_handle)

    return decompressed_PDB_file_path

def parse_PDB_with_biopython(PDB_file_path: Path) -> Structure:
    """
    """
    decompressed_flag = False
    try:    
        if len(PDB_file_path.suffixes) == 1: # PDB file in any of the valid formats (PDB, mmCIF, etc) so Biopython should be able to parse it.
            pass
        elif len(PDB_file_path.suffixes) == 2 and PDB_file_path.suffixes[-1] == '.gz': # Compressed PDB file, first need to decompress
            PDB_file_path = decompress_gzip_PDB_file(PDB_file_path)
        ...
        if decompressed_flag:
            PDB_file_path.unlink() # Always delete the decompressed file
