...
class JobListService:
    ...
    def jobs_by(self, name: str):
        if name is None:
            return self.all_jobs
        return [job for job in self.all_jobs if name.lower() in job.name.lower()]
...
# job_service = JobListService(api_client)
# jobs = job_service.filter_by('gam')
# print(len(jobs))
