...
class IdentificationOnly(TSCBaseEvaluationMethod):
    ...
    def __init__(self, output_folder_name: str = "evaluation_pairwise", perform_windowslicing: bool = False,
    ...
    def evaluate(self, dataset: Dataset, prediction_model: TSCBasePredictionModel):
        _, _, y_test = dataset.view_one_hot_encoded_labels()
        if self.perform_windowslicing:
            x_test, y_test, _ = create_window_slices(dataset.testset_data,
                                                     y_test,
                                                     self.window_stride,
                                                     self.window_size,
                                                     shuffle=self.shuffle_window_slices)

            do_cm(x_test, y_test, prediction_model, self.output_folder_name)
            do_cm(x_test, y_test, prediction_model, self.output_folder_name, perform_per_sample_majority_vote=True)
        else:  # no window slicing here
            x_test = dataset.testset_data
            do_cm(x_test, y_test, prediction_model, self.output_folder_name)
