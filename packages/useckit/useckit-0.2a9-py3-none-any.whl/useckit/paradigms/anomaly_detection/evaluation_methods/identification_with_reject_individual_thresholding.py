...
class IdentificationRejectIndividualThreshold(AnomalyBaseEvaluationMethod):

    def __init__(self, output_folder_name: str = "evaluation_identification_with_reject"):
        super().__init__(output_folder_name)

    def evaluate(self, dataset: Dataset, prediction_model: AnomalyBasePredictionModel):
        test_data, test_labels = dataset.testset_data, dataset.testset_labels
        thresholding_data, thresholding_labels = dataset.validationset_data, dataset.validationset_labels
        ...
        plot_confusion_matrix(contingency_table, target_names=label_names, path=self.output_folder_name,
