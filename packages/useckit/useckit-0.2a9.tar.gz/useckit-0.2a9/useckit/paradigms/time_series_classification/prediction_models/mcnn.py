...
class Classifier_MCNN(TSCBasePredictionModel):

    def __init__(self, nb_classes, verbose=False, nb_epochs=None):
        super().__init__()
    ...
    def train(self, x_train, y_train, x_test, y_test, y_true, pool_factor=None, filter_size=None, do_train=True):
                ...
                for minibatch_index in range(n_train_batches):
                    ...
                    if (iteration + 1) % validation_frequency == 0:
                        ...
                        if valid_loss <= best_validation_loss:
                            ...
                            model.save(os.path.join(self.output_directory, 'best_model.hdf5'))
        ...
        self.y_pred = y_pred = np.array(y_predicted)
        ...
        df_metrics.to_csv(os.path.join(self.output_directory, 'df_metrics.csv'), index=False)
    ...
    def predict(self, x_test, y_true, x_train, y_train, y_test, return_df_metrics=True):
        if return_df_metrics:
            ...
            assert self.y_pred is not None, 'Classifier_MCNN was called for prediction before training. ' \
                                            'self.y_pred is None!'
            return self.y_pred
