...
class Classifier_TWIESN(TSCBasePredictionModel):
    ...
    def train(self):
        ...
        gc.collect()
        # save the model
        np.savetxt(os.path.join(self.output_directory, 'W_in.txt'), self.W_in)
        np.savetxt(os.path.join(self.output_directory, 'W.txt'), self.W)
        np.savetxt(os.path.join(self.output_directory, 'W_out.txt'), self.W_out)
    ...
    def predict(self, x_test, y_true, x_train, y_train, y_test, return_df_metrics=True):
        return self.y_pred_reshaped_best
