...
from useckit.paradigms.time_series_classification.prediction_models.tsc_prediction_model_base import \
    TSCBasePredictionModel


def plot_roc_curve(labels_test, predictions, output_directory):
    fpr, tpr, threshold = metrics.roc_curve(labels_test, predictions)
    ...
    precision, recall, threshold = metrics.precision_recall_curve(labels_test, predictions)
...
def do_cm(x_val: np.ndarray,
    ...
    with open(os.path.join(output_dir, 'classification-report.txt'), 'w') as f:
