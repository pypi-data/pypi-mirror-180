...
from ....util.utils import save_logs_t_leNet as save_logs


class Classifier_TLENET(TSCBasePredictionModel):

    def __init__(self, verbose, nb_epochs=None):
        super().__init__()
    ...
    def fit(self, x_train, y_train, x_val, y_val, y_true, input_shape, nb_classes):
        if not tf.test.is_gpu_available:
            print('error')
            exit()
        nb_epochs = 1000 if self.nb_epochs is None else self.nb_epochs
        batch_size = 256
        nb_classes = y_train.shape[1]

        # limit the number of augmented time series if series too long or too many 
        if x_train.shape[1] > 500 or x_train.shape[0] > 2000 or x_val.shape[0] > 2000:
        ...
        for i in range(test_num_batch):
            unique_value, sub_ind, correspond_ind, count = np.unique(
                y_pred[i * tot_increase_num:(i + 1) * tot_increase_num], True, True, True)
    ...
    def predict(self, x_test, y_true, x_train, y_train, y_test, return_df_metrics=True):
        ...
        y_pred_model = model.predict(new_x_test, batch_size=batch_size)
        # convert the predicted from binary to integer
        y_pred_argmax = np.argmax(y_pred_model, axis=1)

        # get the true predictions of the test set
        y_predicted = []
        test_num_batch = int(new_x_test.shape[0] / tot_increase_num)
        for i in range(test_num_batch):
            unique_value, sub_ind, correspond_ind, count = np.unique(y_pred_argmax, True, True, True)
