...
def activate_venv(name: str = "") -> str:
    ...
    if out and "\0" in out[0]:      # TODO: fix error for APP_PRJ (e.g. kivy_lisz)
...
def _check_types_linting_tests(pdv: PdvType):
    ...
    with _in_prj_dir_venv(project_path):
        ...
        _cl(63, "python -m pytest --cov-report html "
                f"{''.join([' --cov=' + opt_val for opt_val in pdv_val(pdv, 'project_packages') or ['main']])}"
                f"{' -v' if '-v' in args else ''}"
...
def check_integrity(ini_pdv: PdvType):
    ...
    if 'CI_PROJECT_ID' not in os.environ:   # skip template checks on gitlab CI for aedev portions and for ROOT_PRJ:
        _check_templates(ini_pdv)           # .. find_extra_modules() can't find extra modules, e.g. enaml_app.functions
