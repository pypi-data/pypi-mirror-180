...
def ADMM(prox_f,prox_g,τ,xpu,maxiter=1000,
	rtol=1e-2,rtol_period=5):
	"""
	Applies the ADMM algorithm, for the minimization of f(x)+g(x), 
	knowing the proximal operators of f and g. (Does not use automatic differentiation.)
	
	The method is based on the successive updates
	z' = prox_g(x+u), u' = x+u-z', x' = prox_f(z'-u')
	which amounts to non-expansive iterations applied to xpu := x+u.

	At the limit, x=z is a minimizer of f+g, and u/τ in (-df(x)) n (dg(z)).
	(I.e. -u/τ is a subgradient of f, and u/τ is a subgradient of g, at the minimizer.)
	Input : 
	- prof_f, prox_g : proximal operators. Separable operators can be provided as tuples.
	- τ : proximal time step
	- xpu : initial guess for x+u. Can be provided as a tuple of variables.
	- rtol (optional) : the convergence criterion is |xpu(n+1)-xpu(n)| <= rtol |xpu(1)-xpu(0)|.
		Note that |xpu(n+1)-xpu(n)| is by design a non-increasing sequence converging to 0.
	- rtol_period (optional) : how often to test for the convergence criterion
	Output : 
	 - (x,z,u),(xpu,norm2_hist) : last iterates, and history of |xpu(n+1)-xpu(n)|^2
	"""

	# Deal with variables provided as tuples
	tuples = isinstance(xpu,tuple) 
	if not tuples : xpu = (xpu,)
	def add(a,b): return tuple(ai+bi for ai,bi in zip(a,b))
	def sub(a,b): return tuple(ai-bi for ai,bi in zip(a,b))
	def norm2(a): return sum(np.sum(ai**2) for ai in a)
	norm2_hist = []

	# Deal with separable functions provided as tuples
	if isinstance(prox_f,tuple): 
		prox_f_ = prox_f
		prox_f = lambda *xτ: tuple(prox_fi(xi,xτ[-1]) for (prox_fi,xi) in zip(prox_f_,xτ))
	if isinstance(prox_g,tuple): 
		prox_g_ = prox_g
		prox_g = lambda *xτ: tuple(prox_gi(xi,xτ[-1]) for (prox_gi,xi) in zip(prox_g_,xτ))
	...
	for i in range(maxiter):
		...
		xpu = add(x,u)
		if i%rtol_period==0: 
			norm2_hist.append(norm2(sub(xpu,xpu_old)))
			if norm2_hist[-1]<=rtol**2*norm2_hist[0]: break
	else: 
		...
		f"|xpu(n+1)-xpu(n)| / |xpu(1)-xpu(0)| = {np.sqrt(norm2_hist[-1]/norm2_hist[0])}.")

	return ((x,z,u),(xpu,norm2_hist)) if tuples else ((*x,*z,*u),(*xpu,norm2_hist))
