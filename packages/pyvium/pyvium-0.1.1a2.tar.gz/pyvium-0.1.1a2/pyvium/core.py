...
class Core:
    ...
    def IV_VersionHost(self,version_host):
        '''REVISE!!! Returns the version Host'''
        version_host_ptr = ffi.new("long *",version_host)
        result_code = self._lib.IV_VersionHost(version_host_ptr)
    ...
    def IV_HostHandle(self):
        ...
        return self._lib.IV_HostHandle()

    def IV_VersionDllFile(self):
        '''REVISE!!! Returns DLL file version'''
        return self._lib.IV_VersionDllFile()

    def IV_VersionDllFileStr(self):
        '''REVISE!!! Returns DLL file version str'''
        return self._lib.IV_VersionDllFileStr()

    def IV_SelectChannel(self, chanel_number):
        ...
        result_code = self._lib.IV_SelectChannel(chanel_number_ptr)
        return result_code
    ...
    def IV_setcellon(self,cell_on_mode_number):
        ...
        result_code = self._lib.IV_setcellon(
            cell_on_mode_number_ptr)
        return result_code
    
    def IV_setpotential(self, potential_value):
        '''Set cell potential'''
        potential_value_ptr = ffi.new("double *",potential_value)
        result_code = self._lib.IV_setpotential(potential_value_ptr)
        return result_code
    
    def IV_setpotentialWE2(self,potential_we2_value):
        '''Set BiStat offset potential'''
        potential_we2_value_ptr = ffi.new("double *", potential_we2_value)
        result_code = self._lib.IV_setpotentialWE2(potential_we2_value_ptr)
        return result_code
    
    def IV_setcurrent(self,current_value):
        '''Set cell current (galvanostatic mode)'''
        current_value_ptr = ffi.new("double *", current_value)
        result_code = self._lib.IV_setpotentialWE2(current_value_ptr)
        return result_code
    
    def IV_getpotential(self):
        '''Returns measured potential'''
        potential_value_ptr = ffi.new("double *")
        result_code = self._lib.IV_getpotential(potential_value_ptr)
        return result_code, potential_value_ptr[0]
    
    def IV_setcurrentrange(self,current_range_number):
        '''Set current range, 0=10A, 1=1A, etc,'''
        current_range_number_ptr = ffi.new("long *", current_range_number)
        result_code = self._lib.IV_setcurrentrange(current_range_number_ptr)
        return result_code
    
    def IV_setcurrentrangeWE2(self,current_range_number):
        '''Set current range for BiStat, 0=10mA, 1=1mA, etc,'''
        current_range_number_ptr = ffi.new("long *", current_range_number)
        result_code = self._lib.IV_setcurrentrangeWE2(current_range_number_ptr)
        return result_code

    def IV_getcurrent(self):
        '''Returns measured current'''
        current_value_ptr = ffi.new("double *")
        result_code = self._lib.IV_getcurrent(current_value_ptr)
        return result_code, current_value_ptr[0]

    def IV_getcurrentWE2(self):
        '''Returns measured current from WE2 (bipotentiostat)'''
        current_value_ptr = ffi.new("double *")
        result_code = self._lib.IV_getcurrentWE2(current_value_ptr)
        return result_code, current_value_ptr[0]
    
    def IV_setfilter(self, filter_number):
        '''Set filter, for int :0=1MHz, 1=100kHz, 2=10kHz, 3=1kHz, 4=10Hz'''
        filter_number_ptr = ffi.new("long *", filter_number)
        result_code = self._lib.IV_setfilter(filter_number_ptr)
        return result_code
    
    def IV_setstability(self, stability_number):
        '''Set stability, for int 0=HighSpeed, 1=Standard, 2=HighStability'''
        stability_number_ptr = ffi.new("long *", stability_number)
        result_code = self._lib.IV_setstability(stability_number_ptr)
        return result_code
    
    def IV_setbistatmode(self, value):
        ...
        result_code = self._lib.IV_setstability(value_ptr)
        return result_code

    def IV_setdac(self, channel_number, value):
        ...
        result_code = self._lib.IV_setdac(channel_number_ptr, value_ptr)
        return result_code

    def IV_getadc(self, channel_number):
        '''REVISE! Returns measured voltage on external ADC port, int=channelnr. 0-7'''
        channel_number_ptr = ffi.new("long *", channel_number)
        measured_voltage_ptr = ffi.new("double *")
        result_code = self._lib.IV_getdac(channel_number_ptr, measured_voltage_ptr)
        return result_code, measured_voltage_ptr[0]
    
    def IV_setmuxchannel(self, channel_number=0):
        '''Set channel of multiplexer, int=channelnr. starting from 0(default)'''
        channel_number_ptr = ffi.new("long *", channel_number)
        result_code = self._lib.IV_setmuxchannel(channel_number_ptr)
        return result_code

    def IV_setdigout(self, value):
        '''REVISE! Set digital lines on external port, int is bitmask'''
        value_ptr = ffi.new("long *",value)
        result_code = self._lib.IV_setdigout(value_ptr)
        return result_code

    def IV_getdigin(self):
        '''REVISE! Returns status of digital inputs from external port, int is bitmask'''
        value_ptr = ffi.new("long *")
        result_code = self._lib.IV_getdigin(value_ptr)
        return result_code, value_ptr[0]
    
    def IV_setfrequency(self, frequency):
        frequency_ptr = ffi.new("double *", frequency)
        result_code = self._lib.IV_setfrequency(frequency_ptr)
        return result_code

    def IV_setamplitude(self, amplitude):
        amplitude_ptr = ffi.new("double *", amplitude)
        result_code = self._lib.IV_setamplitude(amplitude_ptr)
        return result_code
    ...
    # WE32 functions
    def IV_we32setchannel(self, channel_index):
        '''Select active WE32 channel (chan)'''
        channel_index_ptr = ffi.new("long *", channel_index)
        result_code = self._lib.IV_we32setchannel(channel_index_ptr)
        return result_code

    def IV_we32setoffset(self, channel_index, value):
        '''Set WE32 offset (chan,value), value -2 to +2V.
        Use chan=0 to apply the same offset to all channels.'''
        channel_index_ptr = ffi.new("long *", channel_index)
        value_ptr = ffi.new("double *", value)
        result_code = self._lib.IV_we32setoffset(channel_index_ptr, value_ptr)
        return result_code

    def IV_we32setoffsets(self, number_of_channels, value):
        ...
        value_ptr = ffi.new("double *", value)
        result_code = self._lib.IV_we32setoffsets(number_of_channels_index_ptr, value_ptr)
        return result_code

    def IV_we32getoffsets(self, number_of_channels):
        '''REVISE! Returns actual WE32 offset values (Nchan,values),
        with Nchan the number of channels (1..32)'''
        number_of_channels_index_ptr = ffi.new("long *", number_of_channels)
        values_ptr = ffi.new("double *")
        result_code = self._lib.IV_we32getoffsets(number_of_channels_index_ptr, values_ptr)
        return result_code, values_ptr[0]

    def IV_we32readcurrents(self):
        '''REVISE! Returns array with 32 WE32 current values,
        that are measured simultaneously'''
        current_values_ptr = ffi.new("double *")
        result_code = self._lib.IV_we32readcurrents(current_values_ptr)
