...
def ssh_download_file(remote_path, local_path, ip, user_name, password, port=22, callback=None):
    ...
    try:
        ...
        if 'scp:' in before_str:
            process_thread.stop(status=False)
            yield False
        else:
            process_thread.stop(status=True)
        yield True
    except pexpect.EOF:
        print('EOF')
        ssh.close()
...
if __name__ == '__main__':
    ...
    res = ssh_download_file('/root/frp.tar.gz', './', ip, user_name, password, callback=process_callback)
    print(res)
    res = ssh_download_file('/root/frp.tar.gz1', './', ip, user_name, password, callback=process_callback)
    print(res)
    res = ssh_download_file('/root/frp.tar.gz', './', ip, user_name, password, callback=process_callback)
    print(res)
