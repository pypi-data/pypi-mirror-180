...
class MdbExomol(object):
    ...
    def __init__(self,
        ...
        self.QTref = np.array(self.QT_interp(Tref_original))
        ...
        if numinf is None:
            ...
            if self.trans_file.with_suffix('.hdf5').exists():
                ...
                self.Sij_typ = self.Sij0 * self.QTref / self.QTtyp \
                    * np.exp(-hcperk*self._elower * (1./self.Ttyp - 1./Tref_original)) \
                    * np.expm1(-hcperk*self.nu_lines/self.Ttyp) / np.expm1(-hcperk*self.nu_lines/Tref_original)
    ...
    def get_Sij_typ(self, Sij0_in, elower_in, nu_in):
        ...
        return Sij0_in * self.QTref / self.QTtyp \
            * np.exp(-hcperk*elower_in * (1./self.Ttyp - 1./Tref_original)) \
            * np.expm1(-hcperk*nu_in/self.Ttyp) / np.expm1(-hcperk*nu_in/Tref_original)
...
class MdbHit(object):
    ...
    def Qr_HAPI(self, Tarr):
        ...
        """
        allT = list(np.concatenate([[Tref_original], Tarr]))
        Qrx = []
...
class AdbVald(object):
    ...
    def qr_interp_Irwin_Fe(self, T, atomspecies='Fe 1'):
        ...
        return self.QT_interp_Irwin_Fe(
            T, atomspecies) / self.QT_interp_Irwin_Fe(Tref_original, atomspecies)
