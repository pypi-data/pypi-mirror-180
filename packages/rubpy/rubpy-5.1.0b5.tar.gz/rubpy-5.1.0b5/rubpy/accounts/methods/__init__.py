...
class Methods:
    ...
    async def __aexit__(self, *excepts):
        exit(excepts)
    ...
    async def _baseSendFiles(self, file_type, file_bytes, **kwargs):
        ...
        metadata = self.utils.textParser(caption)
        if metadata[0] != []:
            data['metadata'] = {'meta_data_parts': metadata[0]}
            data['text'] = metadata[1].strip()
        ...
        if 'file_inline' in list(message_info.keys()):
            ...
            if size <= 131072:
                if save:
                    ...
                    return await self.make.connections._download(session=self.session, url=makeURL, headers=headers)

            else:
                for start_index in range(0, size, 131072):
                    headers["start-index"], headers["last-index"] = str(start_index), str(start_index + 131072 if start_index + 131072 <= size else size)
                    if save:
                        pass
                    else:
                        data += await self.make.connections._download(session=self.session, url=makeURL, headers=headers)
                return data
        else:
            return None