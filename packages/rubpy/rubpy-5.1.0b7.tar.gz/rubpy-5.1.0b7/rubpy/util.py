    ...
    def textParser(self, text):
        ...
        for markdown in finditer(pattern, text):
            ...
            if markdown.group(0).startswith('``'):
                value = sub(pattern, r'\1', text, count=1)
                ...
                value = sub(pattern, r'\2', text, count=1)
                meta_data_parts.append(
                    {
                        'type': 'Bold',
                        'from_index': span[0] - conflict,
                ...
                value = sub(pattern, r'\3', text, count=1)
                meta_data_parts.append(
                    {
                        'type': 'Italic',
                        'from_index': span[0] - conflict,
                ...
                value = sub(pattern, r'\4', text, count=1)
        ...
        result = {'text': value.replace('**', '').replace('__', '').replace('``', '')}
        if meta_data_parts:
            result['metadata'] = {
                'meta_data_parts': meta_data_parts
            }

        return (result.get('metadata'), result.get('text'))
