...
def try_replacing_hashtags_mit_birdname(text, all_birds_list):
    status = False
...
def get_bird_name_from_hashtag_4levels(hashtag_, all_birds_list):
            ...
            while n_ < len(hashtag_[m_:]):
                ...
                while o_ < len(hashtag_[m_:][n_:]):
                    ...
                    while p_ < len(hashtag_[m_:][n_:][o_:]):
                        ...
                        prob_birdname = segments[0] + " " + segments[1] + " " + segments[2] + " " + segments[
                            3] + " " + part4
...
def basic_preprocess(sentence):
    ...
    for key in spelling_corrections:
        if sentence.find(key) > -1:
            sentence = sentence.replace(key, spelling_corrections[key])
    return sentence


def plural_nn_to_singular(sentence, birdnames_words):
    doc = nlp(sentence)
    for token in doc:
        if token.pos_ == "NOUN":
