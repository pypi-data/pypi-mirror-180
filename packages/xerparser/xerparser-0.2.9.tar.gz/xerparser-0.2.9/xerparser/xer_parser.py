...
def _parse_file_to_list_of_tables(file: bytes | str) -> list[str]:
    ...
    def _return_file(_file: str) -> list[str]:
        if not _file.startswith("ERMHDR"):
            raise ValueError(f"ValueError: invalid XER file")
        return _file.split("%T\t")

    if isinstance(file, bytes):
        file_as_str = file.decode(CODEC)
    elif isinstance(file, str):
        with open(file, encoding=CODEC) as f:
            file_as_str = f.read()
    else:
        # File is type fastapi UploadFile
        try:
            file_as_str = file.file.read().decode(CODEC)
        except:
            pass

        # File is type ???
        try:
            file_as_str = file.read().decode(CODEC)
        except:
            raise ValueError("Cannot Read File")

    _return_file(file_as_str)
