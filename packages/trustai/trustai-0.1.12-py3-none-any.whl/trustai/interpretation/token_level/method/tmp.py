...
class IntGradInterpreter(TokenInterpreter):
    ...
    def __init__(self,
        ...
        Args:
            ...
            ret_error(bool, optional): Whether to return error. Default: True
    ...
    def _ig_interpret_instance(self, instance, label, steps) -> tuple:
        ...
        if label is None:
            _, label, _, proba = self.predict_fn(instance,
                                                 label,
                                                 paddle_model=self.paddle_model,
        ...
        if isinstance(pred_proba, np.ndarray) and self.ret_error:
        ...
        return ig_gradients, error_percentage, label[0], pred_proba[0]
