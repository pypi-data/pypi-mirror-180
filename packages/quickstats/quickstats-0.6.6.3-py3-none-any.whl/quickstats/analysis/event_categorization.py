...
class EventCategorization(DataLoader):
    ...
    def scan_class_bounds(self, class_score_hist, n_bins:int, n_boundaries:int,
        ...
        with Timer() as t:
            ...
            signal_yields     = self.get_region_yields(signal_hists, boundary_indices)
            background_yields = self.get_region_yields(background_hists, boundary_indices)
            if is_binary:
                signal_yields     = signal_yields[:, 1:]
                background_yields = background_yields[:, 1:]
            ...
            if minimum_yields is not None:
                for sample in minimum_yields:
                    ...
                    proc_yield = self.get_region_yields(all_hists[sample], boundary_indices)
                    valid_boundaries &= np.all(proc_yield > min_yield, axis=1)
