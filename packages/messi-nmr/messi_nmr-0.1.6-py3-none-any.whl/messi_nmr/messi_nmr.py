...
def GUI():
    ...
    #-----------------------------------------------------------------
    button_example = tk.Button(root, text='User Guide', 
...
def user_guide():
    path = (Path(__file__).parent /'UserGuide' / "UserGuide.pdf").as_posix()
...
def DP4_filter_calculation(i, cant_comp, tens_matrix_all_isomers, wtl_C, wtl_H, d_exp_C, d_exp_H):
...
def main():
    ...
    for i in range(len(matrix_tensors)):
        DP4_results, isom_sin_conf = DP4_filter_calculation(i, cant_comp, tens_matrix_all_isomers, wtl_C, wtl_H, d_exp_C, d_exp_H)
