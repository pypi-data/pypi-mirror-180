...
#                     return pd.DataFrame(columns=results)
    ...
    async def get_picklist_values_by_object_record_type(self, object: str):
        ...
        def record_type_picklist_retriever(self, object, record_type_id, record_type_name):
            ...
            if final_result_pd['defaultValue'].isnull().all():
                final_result_pd[['defaultValueLabel', 'defaultValueValue']] = np.nan
            else:
    ...
    async def retrieve_metadata(self, metadata_type: list[str]):
        
        """Providing a list of metadata type in accordance with the Salesforce Metadata API
        https://developer.salesforce.com/docs/atlas.en-us.api_meta.meta/api_meta/meta_types_list.htm

        Raises:
            Exception: Metadata Name Mismatch, this may be caused by async operation returning metadata in unexpected order, or one of the metadata types requestd cannot be retreived properly

        Returns:
            pd.DataFrame: Returns the full detailed metadata dataframe of all the metadata types requested
        """
        metadata_list_async = async_wrap(self.metadata_list)
        task_list = []
        for group in self.grouper(metadata_type, 3):
            meta_list = [value for value in group if value is not None]
            task_list.append(meta_list)
        result_list = await asyncio.gather(*[metadata_list_async(metadata_type_list=task ) for task in task_list])
        result_list = [item for sublist in result_list for item in sublist]
        metadata_info = pd.DataFrame(zeep.helpers.serialize_object(result_list))
        
        
        metadata_read_async = async_wrap(self.metadata_read)
        
        detailed_metadata = await asyncio.gather(*[metadata_read_async(metadata_type=result['type'], fullName=result['fullName']) for result in result_list])
        
        
        combined_metadata = []
        for item in zip(detailed_metadata, metadata_info.iterrows()):
            
            x_data = item[0].loc[0]
            x_info = item[1][1]


            if x_info['fullName'] == x_data['fullName']:
                x_data.drop('fullName', inplace=True)
            else:
                raise Exception('Metadata Name Mismatch, this may be caused by async operation returning metadata in unexpected order, or one of the metadata types requestd cannot be retreived properly')
            x_info.name= 0
            
            combined_row = pd.DataFrame(pd.concat([x_info, x_data], axis=0)).T
            combined_metadata.append(combined_row)
        if len(combined_result_df) > 1:
            combined_result_df = pd.concat(combined_metadata, axis=0).reset_index(drop=True)
        else:
