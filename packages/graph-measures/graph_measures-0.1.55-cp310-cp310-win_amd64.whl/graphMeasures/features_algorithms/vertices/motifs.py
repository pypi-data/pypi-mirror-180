...
########################################################################################################################
########################################################################################################################
class MotifsEdgesCalculator1(NodeFeatureCalculator):
    def __init__(self, *args, level=3, calc_edges=True
                 , **kwargs):
        super(MotifsEdgesCalculator1, self).__init__(*args, **kwargs)
        assert level in [3, 4], "Unsupported motif level %d" % (level,)
        self._level = level
        self._node_variations = {}
        self._all_motifs = None
    ...
    @classmethod
    def print_name(cls, level=None):
        print_name = super(MotifsEdgesCalculator1, cls).print_name()
        if level is None:
            return print_name
        return "%s_%d" % (print_name, level)
...
class MotifsEdgeCalculator(MotifsNodeCalculator):
    ...
    def _calculate(self, include=None):
        # !!!!!! I added it. Probably shouldn't be here...
