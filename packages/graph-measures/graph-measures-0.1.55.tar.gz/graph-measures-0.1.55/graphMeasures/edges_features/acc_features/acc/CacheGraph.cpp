...
int CacheGraph::GetEdgeNum(unsigned int node1, unsigned int node2) const {
	if (node1 >= m_NumberOfNodes || node2 >= m_NumberOfNodes)
		return -1;
	
	for (int i = m_Offsets[node1]; i < m_Offsets[node1 + 1]; i++)
		if (m_Graph[i] == node2)
			return i;
