...
class CacheGraph {
public:
	CacheGraph(bool directed, unsigned int numOfNodes, unsigned int numOfEdges,
                 unsigned int* neighbors, int64* offests):
			m_NumberOfNodes(numOfNodes), m_NumberOfEdges(numOfEdges), m_Graph(neighbors), m_Offsets(
			offests), m_Weights(NULL), weighted(false), directed(directed) {
	}
	...
	int GetEdgeNum(unsigned int node1, unsigned int node2) const;
