...
#include "MotifVariationConstants.h"
#include <algorithm>
...
MotifCalculator::MotifCalculator(int level, bool directed, string motif_path) :
		...
		NULL), sortedNodesByDegree(NULL), fullGraph(false), calcEdges(true) {
...
void MotifCalculator::InitFeatureCounters() {
	// TODO - the differce between the two condition is basiclly the number of edges\nodes.
	// We should write it better!! 
	if (calcEdges) {
		for (int edge = 0; edge < mGraph->GetNumberOfEdges(); edge++) {
...
vector<vector<unsigned int>*>* MotifCalculator::Calculate() {
	...
	// we should change this->features!
...
void MotifCalculator::GroupUpdater(std::vector<unsigned int> group) {
	...
	if (motifNumber == -1) return;

	if(calcEdges) {
		for (auto node1 : group)
			for (auto node2 : group)
				if (node1 != node2) {
					// function GetEdgeNum will return the edge number in the offsets array.
					int edgeNum = this->mGraph->GetEdgeNum(node1, node2);
					if (edgeNum != -1) {
						(*(*features)[edgeNum])[motifNumber]++;
						// std::cout << motifNumber << " " << node1 << " motif number: " << node2 << " " << std::endl;
					}
				}
	} else {
		for (auto node : group)
