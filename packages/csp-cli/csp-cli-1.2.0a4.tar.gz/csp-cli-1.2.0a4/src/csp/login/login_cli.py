...
@click.option("-p", "--passwd", type=click.STRING, prompt="密码", help="密码", required=True, hide_input=True)
@click.option("-n", "--nickname", type=click.STRING, prompt="显示昵称，默认为空", help="显示昵称，默认为空", default='')
def register(username, passwd, nickname):
    """
    用户注册命令

    \b
    使用示例：csp register -u "用户名" -p "用户密码" -n "显示昵称"
    """
    if not nickname:
        nickname = None
    try:
