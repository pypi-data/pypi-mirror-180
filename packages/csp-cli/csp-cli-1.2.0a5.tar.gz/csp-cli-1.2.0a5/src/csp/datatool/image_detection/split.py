    ...
    def split(self, mode=None):
        ...
        for item in os.listdir(imgsavepath):
            if os.path.isfile(item):
                ...
                if item_type.lower() in ["jpg", "jpeg", "png"]:
                    if item_name in out_train_xml_l:
        ...
        logger.info("the split result files saved in {}".format(self.output))
...
class CocoSplit:
    ...
    def split(self, mode=None):
        ...
        out_eval_json = os.path.join(out_train_annotations, "val.json")
