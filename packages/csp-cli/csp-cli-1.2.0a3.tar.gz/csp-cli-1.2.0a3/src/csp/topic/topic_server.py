...
def topic_list(model_repository=None, classify=None, dataset=None, show=True):
    ...
    if dataset:
        ...
        if msg == "Invalid access token":
            # raise ValueError("token 过期，请重新登录 csp login")
            print("token无效，查询失败，请登录 csp login，重新查询")
            # 跳转至登录
    ...
    if image_url:
        ...
        if model_source == "local":
            ...
            if not out_login.rstrip("\n").endswith("Login Succeeded"):
                raise ConnectionError("私服登录失败，请检查docker服务是否已启动或是否能连接docker私服仓库")
            # 镜像打tag，并判断本地镜像存在性
        ...
        if msg == "Invalid access token":
            # print('模型注册失败')
            # raise ValueError("token无效，模型注册失败，请登录 csp login，重新上传")
            print("token无效，模型注册失败，请登录 csp login，重新上传")
            # 跳转至登录
