...
def parse_submit(submit_file):
    ...
    for result in rs_l:
        ...
        filename = result['file_name'].split('.')[0]
        category = result['category']
        confidence = result['score']
        xmax = result["bbox"][0] + result["bbox"][2]
        ymax = result["bbox"][1] + result["bbox"][3]
        ymin = result["bbox"][1]
        xmin = result["bbox"][0]
...
def eva_voc(submit_file, ori_dir, output=None):
    ...
    for label in label_l:
        ...
        res_list.append({"category": label, "recall": rec, "precision": prec, "ap": ap})
        # rec_m += rec[-1]
        # prec_m += prec[-1]
        rec_m += rec
        prec_m += prec
