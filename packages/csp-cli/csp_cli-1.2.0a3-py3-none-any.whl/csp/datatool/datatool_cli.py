...
def det_voc(submit_file, gold_folder, output):
    ...
    try:
        ...
        if os.path.exists(os.path.join(output, "image")):
            print("badcase分析结果保存至{}".format(os.path.join(output, "image")))
        if os.path.exists(os.path.join(output, "voc")):
            print("badcase分析结果保存至{}".format(os.path.join(output, "voc")))
    except Exception as ae:
        print(ae)
...
@click.option("-c", "--aug_file", prompt="增强配置yml文件，可在csp resource中的config类别中下载", help="增强配置yml文件", required=True,
              type=click.Path(exists=True, file_okay=True, dir_okay=False))
...
@click.option("-c", "--aug_file", prompt="增强配置yml文件，可在csp resource中的config类别中下载", help="增强配置yml文件", required=True,
              type=click.Path(exists=True, file_okay=True, dir_okay=False))
# @click.option("-m", "--mode", type=click.Choice(['train', 'trainval']), help="增强依据，当同时存在train.txt、trainval.txt文件时，必指定其一;若只存在一个，则默认以存在的为依据")
def det_coco(dataset, aug_file, mode):
    """
    目标检测任务COCO数据增强命令
...
@click.option("-c", "--aug_file", prompt="增强配置yml文件", help="增强配置yml文件", required=True, type=click.Path(exists=True, file_okay=True, dir_okay=False))
