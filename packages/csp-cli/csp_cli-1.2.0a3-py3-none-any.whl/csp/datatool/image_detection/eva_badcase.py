        ...
        for item in old_data:
            new_item = {"image_id": item["image_id"],
                        ...
                        "bndbox": {"xmin": str(item["bbox"][0]),
                                   ...
                                   "xmax": str(item["bbox"][0] + item["bbox"][2]),
                                   "ymax": str(item["bbox"][1] + item["bbox"][3])}}
    ...
    def evaluate_image_score(self, iou_threshold: float, false_detection_weight: float,
        ...
        try:
            ...
            ap_table.append({"category": "total",
                             "false detection rate": round(sum_false_detection_rate / len(class_name_list), 4),
                             "missed detection rate": round(sum_missed_detection_rate / len(class_name_list), 4),
                             "correct rate": round(sum_object_detection_correct_rate / len(class_name_list), 4),
                             "image score": round(sum_image_score / len(class_name_list), 4)})
            if show:
