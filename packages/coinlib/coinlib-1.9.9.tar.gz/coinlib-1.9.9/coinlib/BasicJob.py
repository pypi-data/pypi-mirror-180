...
class BasicJob:
    ...
    def getTrendlines(self, name, history_in_sec=None):
        ...
        trendlines = [to_trendline(x) for x in trendlines if x is not None]
    ...
    def get(self, name, index=None, filterNone = False, replaceNone=None):
        ...
        try:
                ...
                if maybe_col_name + ":y" in self.table.columns:
                    data = self.table.column(maybe_col_name + ":y", index)
                if maybe_col_name + ":marker" in self.table.columns:
                    data = self.table.column(maybe_col_name + ":marker", index)
