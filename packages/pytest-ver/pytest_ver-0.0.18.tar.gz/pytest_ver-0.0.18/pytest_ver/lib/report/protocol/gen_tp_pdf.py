...
class GenTpPdf(GenBasePdf):
    ...
    def _gen_step(self, tbl, stepno, step, requirements):
        ...
        if rs.expected_formatted is None:
            v = str(rs.expected_formatted)
        else:
            v = str(rs.expected_formatted).replace('<', '&lt;')
            v = v.replace('>', '&gt;')
        expected = Paragraph(v, self._stmono)

        if self._do_results:
            if rs.actual_formatted is None:
                v = str(rs.actual_formatted)
            else:
                v = str(rs.actual_formatted).replace('<', '&lt;')
                v = v.replace('>', '&gt;')
            actual = Paragraph(v, self._stmono)
        ...
        if rs.reqids is None or rs.reqids == {}:
            ...
            if len(sorted_reqids) == 1:
                reqids_str = Paragraph(sorted_reqids[0], self._strow)
            else:
                reqids_str = Paragraph('<br/>'.join(sorted_reqids), self._strow)
