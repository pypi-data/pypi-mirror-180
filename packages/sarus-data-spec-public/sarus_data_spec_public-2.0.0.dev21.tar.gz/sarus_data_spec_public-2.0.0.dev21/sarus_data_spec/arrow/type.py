...
def to_arrow(
    ...
    class ToArrow(st.TypeVisitor):
        ...
        def Struct(
            self, fields: t.Mapping[str, st.Type], name: t.Optional[str] = None
        ) -> None:
        ...
        def List(
            self, type: st.Type, max_size: int, name: t.Optional[str] = None
        ) -> None:
        ...
        def Datetime(
            self, format: str, min: str, max: str, base: st.DatetimeBase
        ) -> None:
            self.pa_type = DATETIMEBASE_TO_ARROW[base]

        def Time(
            self, format: str, min: str, max: str, base: st.TimeBase
        ) -> None:
            self.pa_type = TIMEBASE_TO_ARROW[base]

        def Date(
            self, format: str, min: str, max: str, base: st.DateBase
        ) -> None:
...
def from_arrow(type: pa.DataType) -> sdt.Type:
    ...
    if pa.types.is_timestamp(type):
        if type.unit in ['ns', 'us']:
            ...
            return sdt.Datetime(base=st.DatetimeBase.INT64_MS)
    ...
    if pa.types.is_temporal(type):
        ...
        # timestamp or duration. Only duration is left.
