def genDemo():	
	...
	f.write("nin=512\n")
	f.write("\n")
	f.write("tab=[97,90,80,60,40]\n")
	f.write("\n")
	f.write("mask=np.ones([len(tab),12*nout**2])\n")
	f.write("for imask in range(len(tab)):\n")
	f.write("    try:\n")
	f.write("        mask[imask]=dodown(np.load(scratch_path+'/MASK_GAL%03d_%d.npy'%(tab[imask],nin)),nout)\n")
	f.write("    except:\n")
	f.write("        print('==========================================================')\n")
	f.write("        print('')\n")
	f.write("        print('Compute Planck Galactic masks...')\n")
	f.write("        print('')\n")
	f.write("        print('==========================================================')\n")
	f.write("        sky=np.load(scratch_path+'/dust_512.npy')\n")
	f.write("        osky=np.argsort(sky)\n")
	f.write("        idx=hp.ring2nest(512,np.arange(12*512*512))\n")
	f.write("        idx2=hp.nest2ring(512,np.arange(12*512*512))\n")
	f.write("        for jmask in range(len(tab)):\n")
	f.write("            themask=hp.smoothing((sky<sky[osky[(12*512*512*tab[jmask])//100]])[idx],10/180.*np.pi)\n")
	f.write("            themask[themask<0.0]=0.0\n")
	f.write("            themask[themask>1.0]=1.0\n")
	f.write("            np.save(scratch_path+'/MASK_GAL%03d_512.npy'%(tab[jmask]),themask[idx2])\n")
	f.write("            print('Save '+scratch_path+'/MASK_GAL%03d_512.npy'%(tab[jmask]))\n")
	f.write("        del sky\n")
	f.write("        del osky\n")
	f.write("        del idx\n")
	f.write("        del idx2\n")
	f.write("    \n")
	f.write("    mask[imask]=dodown(np.load(scratch_path+'/MASK_GAL%03d_512.npy'%(tab[imask])),nout)\n")
	f.write("\n")
	f.write("mask[0,:]=1.0\n")
	...
	f.write("\n")
	f.write("mask=dodown(np.load(scratch_path+'/MASK_GAL060_%d.npy'%(nin)),nout)\n")
	...
	f.write("plt.ylabel(r'C($\ell$) $f_{sky}=%.2f$'%(mask.mean()))\n")
