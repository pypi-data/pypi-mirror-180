...
class Predictor(BasePredictor, AnomalyDetector):
    ...
    def postprocess(self, img, anomaly_map, path, is_vid=False):
        if isinstance(self.input_size, int):
            self.input_size = [self.input_size, self.input_size]
    ...
    def run(self, img_meta):
        ...
        results = self.postprocess(resized_img, anomaly_map, path)
    ...
    def plot_fig(self, resize_img, anomaly_map,coordinates, img_drawBox, path):
        ...
        ax4.axis('off')
        ax4.imshow(img_drawBox[:,:,::-1], cmap='gray', interpolation='none')
