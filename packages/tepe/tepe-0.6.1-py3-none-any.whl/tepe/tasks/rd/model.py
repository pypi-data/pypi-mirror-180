...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            if not self.hard_sample_mining:
                return self.cal_anomaly_map(x1, x2, amap_mode='add')
            else:
                return self.find_hard_sample_in_batch(x1, x2)
    ...
    def find_hard_sample_in_batch(self, fs_list, ft_list):
        '''
        fs_list：[[B,1024,16,16]]
        '''
        a_map_list = []
        for item in range(len(ft_list)):
            a_map = 1 - self.cos_loss(fs_list[item], ft_list[item])  # [B,16,16]
            a_map = torch.unsqueeze(a_map, dim=1)  # [B,1,16,16]
            a_map = F.interpolate(a_map, size=self.input_size, mode='bilinear', align_corners=False)
            a_map_list.append(a_map)
