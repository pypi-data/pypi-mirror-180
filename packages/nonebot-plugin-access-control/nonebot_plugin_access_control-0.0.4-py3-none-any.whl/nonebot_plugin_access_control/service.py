...
class Service(ABC):
    ...
    async def _fire_service_change_permission(self, subject: str, old_allow: bool):
        for node in self.travel():
            allow = await node.get_permission(subject)
            if allow != old_allow:
    ...
    async def set_permission(self, subject: str, allow: bool):
        async with data_source.start_session() as session:
            ...
            if old_allow != allow:
                ...
                await self._fire_service_change_permission(subject, old_allow)
                return True
            else:
                return False
    ...
    async def remove_permission(self, subject: str) -> bool:
        async with data_source.start_session() as session:
            stmt = (select(PermissionOrm)
                    .where(PermissionOrm.service == self.qualified_name,
                           PermissionOrm.subject == subject))
            p = (await session.execute(stmt)).scalars_one_or_none()
            if p is None:
                return False
            ...
            await self._fire_service_change_permission(subject, p.allow)
