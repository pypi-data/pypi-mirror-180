...
class Auth:
    ...
    def get_user_id_from_cookies(
        ...
        return Auth.get_jwt_field_from_cookies(
            cookies, jwt_name, verifying_key, audience, "user_id"
        )
    ...
    def create_user_folder(self, uid: str) -> Union[bool, CustomError]:
        try:
            s3_upload_maybe = s3_put_object_bytes(
                s3_client=Auth.clients.s3_client,
                s3_bucket=self.config.s3_bucket,
                s3_key=f"api/{uid}/.keep",
                s3_body_bytes=b" ",
            )

            if isinstance(s3_upload_maybe, HttpError):
                LOG.error(s3_upload_maybe)
                return CustomError(
                    message=s3_upload_maybe.error.message,
                    reasons=s3_upload_maybe.error.reasons,
                )

            return True

        except Exception as ex:
            LOG.error(f"Error while creating user folder for {uid}")
            return CustomError(
                message=f"Error while creating user folder for {uid}",
                reasons=[f"{ex}"],
            )

    def create_user(
        self, uid: str, user_email: EmailStr, user_role: RoleEnum, users: Users
    ) -> Union[bool, CustomError]:
        try:
            new_user = User(userId=uid, userEmail=user_email, userRole=user_role)
            users.__root__.append(new_user)
            body = users.json().encode("utf-8")
            s3_upload_maybe = s3_put_object_bytes(
                s3_client=Auth.clients.s3_client,
                s3_bucket=self.config.s3_bucket,
                s3_key="api/users.json",
                s3_body_bytes=body,
            )
            ...
            user_folder_maybe = self.create_user_folder(uid)
            if isinstance(user_folder_maybe, CustomError):
                LOG.error(user_folder_maybe)
                return user_folder_maybe

            return True

        except Exception as ex:
            LOG.error(f"Error while creating user {user_email}")
            return CustomError(
                message=f"Error while creating user {user_email}",
                reasons=[f"{ex}"],
            )

    def get_uid_for_user(self, user_email: EmailStr) -> Union[str, CustomError]:
        try:
            s3_object_content_maybe = s3_get_object_bytes(
                s3_client=Auth.clients.s3_client,
                s3_bucket=self.config.s3_bucket,
                s3_key="api/users.json",
            )
            if isinstance(s3_object_content_maybe, HttpError):
                LOG.error(s3_object_content_maybe)
                return CustomError(
                    message=s3_object_content_maybe.error.message,
                    reasons=s3_object_content_maybe.error.reasons,
                )

            users = Users.parse_raw(s3_object_content_maybe)
            user_maybe = list(
                filter(lambda user: user.userEmail == user_email, users.__root__)
            )

            if len(user_maybe) == 0:
                uid = get_user_eid()
                user_created_maybe = self.create_user(
                    uid, user_email, RoleEnum.user, users
                )
                if isinstance(user_created_maybe, HttpError):
                    LOG.error(user_created_maybe)
                    return CustomError(
                        message=user_created_maybe.error.message,
                        reasons=user_created_maybe.error.reasons,
                    )

                return uid

            elif len(user_maybe) == 1:
                return user_maybe[0].userId

            else:
                LOG.error(f"There are multiple users with email {user_email}")
                return CustomError(
                    message=f"There are multiple users with email {user_email}",
                    reasons=["Duplicate users"],
                )
    ...
    async def login(self, login_hash: LoginHash) -> JSONResponse:
        try:
            ...
            uid_maybe = self.get_uid_for_user(mli_maybe.magic_link.email)
            if isinstance(uid_maybe, CustomError):
                LOG.error(uid_maybe)
                return http_403_json(
                    message=uid_maybe.message, reasons=uid_maybe.reasons
                )
    ...
    async def get_who_am_i(self, request: Request) -> JSONResponse:
        try:
            ...
            email, first_name, role, user_id = token_fields_maybe
