...
class Auth:
    ...
    def create_user(
        self, user_email: EmailStr, user_role: RoleEnum
    ) -> Union[User, CustomError]:
        try:
            uid = get_user_eid()
            new_user = User(userId=uid, userEmail=user_email, userRole=user_role)
            body = new_user.json().encode("utf-8")
            s3_upload_maybe = s3_put_object_bytes(
                s3_client=Auth.clients.s3_client,
                s3_bucket=self.config.s3_bucket,
                s3_key=f"api/users/{user_email}.json",
                s3_body_bytes=body,
            )
    ...
    def get_uid_for_user(self, user_email: EmailStr) -> Union[str, CustomError]:
        try:
            ...
            if (
                isinstance(s3_object_content_maybe, HttpError)
                and s3_object_content_maybe.status_code == 404
            ):
                user_created_maybe = self.create_user(user_email, RoleEnum.user)
                if isinstance(user_created_maybe, CustomError):
                    LOG.error(user_created_maybe)
                    return user_created_maybe

                return user_created_maybe.userId
            ...
            user = User.parse_raw(s3_object_content_maybe)
            return user.userId

        except Exception as ex:
            LOG.error(f"Could not get user id for {user_email}")
            return CustomError(
                message=f"Could not get user id for {user_email}",
                reasons=[f"{ex}"],
            )
