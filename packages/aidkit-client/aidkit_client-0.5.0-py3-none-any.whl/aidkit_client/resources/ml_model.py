...
class MLModelVersion:
    ...
    def model_to_file_object(model: Any) -> Tuple[BinaryIO, str]:
        ...
        try:
            ...
            if isinstance(model, tf.keras.Model):
                ...
                bio.seek(0, os.SEEK_SET)
                return bio, "temp.h5"
            from tensorflow.python.training.tracking.base import (  # pylint:disable=C0415
                Trackable,
            )
    ...
    async def upload(
        ...
        model_file_object, file_name = cls.model_to_file_object(model)
    ...
    @classmethod
    async def get_by_name(cls, model_name: str, model_version_name: str) -> "MLModelVersion":
        """
        Get an ML model version by its name.

        :param model_name: Name of the model to get the version from.
        :param model_version_name: Name of the model version to get.
        ...
        try:
            (model_version_response,) = [
                ...
                if model_version_response.name == model_version_name
            ]
        except ValueError as wrong_entry_number:
            available_names = ", ".join(
                model_version_response.name
                for model_version_response in model_version_response_list
            )
            raise ResourceWithNameNotFoundError(
                f"Ml Model Version with name '{model_version_name}' not found. "
                f"Existing Ml Model Versions: '[{available_names}]'."
    ...
    @classmethod
    async def delete(cls, model_name: str, model_version_name: str) -> None:
        """
        Delete a model version.

        :param model_name: Name of the model to delete a version from.
        :param model_version_name: Name of the model version.

        :return: None.
        """
        api_service = get_api_client()
        await MLModelsAPI(api_service).delete_model_version(model_name, model_version_name)
...
class MLModel:
    ...
    @classmethod
    async def delete(cls, model_name: str) -> None:
        """
        Delete an ML model.
        ...
        :return: None.
        """
        await MLModelsAPI(get_api_client()).delete(name=model_name)
...
class ImageModel(ClassificationOutputMixin, MLModel):
    ...
    def _build_detection_output_specs(
        ...
        if output_format.format == "dictionary":
            parsed_output_format = DetectionDictOutputFormat(
                scores_key=output_format.scores_key,
                boxes_key=output_format.boxes.key,
    ...
    async def create(
        ...
        :param batch: Optional string indicating the batch dimension.
        :param image_scaler: Optional image scaler.
        ...
        input_specs = cls._build_input_specs(
            batch=batch, image_input_config=image_input_config, image_scaler=image_scaler
        )
    ...
    @classmethod
    async def get_latest_version_by_name(cls, model_name: str) -> MLModelVersion:
        """
        Get the latest model version for a given model.

        :param model_name: Name of the model for which to retrieve the latest version.
        ...
        try:
            model_version_response: MLModelVersionResponse = [  # type: ignore
                ...
                if model_response.name == model_name
            ][0]
            return MLModelVersion(
                api_service=api_service, model_version_response=model_version_response
            )
...
class TextModel(ClassificationOutputMixin, MLModel):
    ...
    async def create(
        ...
        input_specs = cls._build_input_specs(
            batch=batch, pre_tokenizer=pre_tokenizer, tokenizer=tokenizer
        )
