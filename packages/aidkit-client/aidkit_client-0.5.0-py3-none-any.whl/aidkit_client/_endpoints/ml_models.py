...
class MLModelsAPI:
    ...
    async def get_all_model_versions(self, model_name: str) -> List[MLModelVersionResponse]:
        result = await self.api.get(
            path=f"{Constants.ML_MODELS_PATH}/{model_name}/versions", parameters=None
        )
    ...
    async def upload_model_version(
        self, model_name: str, model_version_name: str, model_file_content: BinaryIO, file_name: str
    ) -> MLModelVersionResponse:
        ...
        if result.is_bad:
            raise ResourceWithIdNotFoundError(
                f"Could not upload version '{model_version_name}' of MLModel '{model_name}'."
            )
        return MLModelVersionResponse(
            **result.body_dict_or_error(
                f"Error uploading version '{model_version_name}' of MLModel '{model_name}'."
            )
        )
    ...
    async def delete_model_version(self, model_name: str, model_version: str) -> bool:
        response = await self.api.delete(
            path=f"{Constants.ML_MODELS_PATH}/{model_name}/versions/{model_version}"
        )

        if response.is_not_found:
            raise ResourceWithIdNotFoundError(
                f"Could not delete MLModel version'{model_version}' from Ml Model '{model_name}'"
            )
