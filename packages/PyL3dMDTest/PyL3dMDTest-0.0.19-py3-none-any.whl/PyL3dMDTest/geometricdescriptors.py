# -*- coding: utf-8 -*-
"""
Created on Sun Oct 16 17:56:33 2022
...
def calgeometriccenter(xyz):
    ...
    k3 = np.sum((xyz-gc)**3)/(nA * s2v**3)
    k4 = np.sum((xyz-gc)**4)/(nA * s2v**4)
    return (gc, s2v, k3, k4)
...
def interatomicdistnacebetweenallatoms(xyz):
    ...
    Ghinv = np.zeros(np.shape(Gh))
    Ghinv2 = np.zeros(np.shape(Gh))
...
def calendtoenddistance(disMat,disMat3D):
    ...
    Ree = np.max(disMat3D[tuple(np.unique(idxarr, axis=0).astype(int).tolist())])
    return Ree
