...
def save_depp_dist(model, args):
    ...
    jc_dist = pd.read_csv(args.hamming_distance_file, sep='\t', index_col=0)
    jc_dist[jc_dist > 0.75] = 0.75 - 1e-2
    jc_dist = - 3 / 4 * np.log(1 - 4 / 3 * jc_dist)
    ...
    for i in range(math.ceil(len(backbone_seq_tensor) / 2000.0)):
        jc_d = torch.from_numpy(jc_dist.loc[backbone_seq_names[i * 2000: (i + 1) * 2000]].values).float()
        encodings_tmp = model(backbone_seq_tensor[i * 2000: (i + 1) * 2000].float(), jc_d).detach()
    ...
    for i in range(math.ceil(len(query_seq_tensor) / 2000.0)):
        jc_d = torch.from_numpy(jc_dist.loc[query_seq_names[i * 2000: (i + 1) * 2000]].values).float()
        encodings_tmp = model(query_seq_tensor[i * 2000: (i + 1) * 2000].float(), jc_d).detach()
        query_encodings.append(encodings_tmp)
    query_encodings = torch.cat(query_encodings, dim=0)
    print(f'finish embedding calculation!')

    query_dist = torch.zeros(len(query_seq_tensor), len(backbone_seq_tensor))
    for i in range(math.ceil(len(query_seq_tensor) / 1000.0)):
        for j in range(math.ceil(len(backbone_seq_tensor) / 10000.0)):
            query_dist[i * 1000: (i + 1) * 1000, j * 10_000: (j + 1) * 10_000] = \
                distance(query_encodings[i * 1000: (i + 1) * 1000],
                         backbone_encodings[j * 10_000: (j + 1) * 10_000], args.distance_mode) * args.distance_ratio
    query_dist = np.array(query_dist)
    query_dist[query_dist < 1e-3] = 0
