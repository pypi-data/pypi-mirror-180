import datetime
import RubiDark.encoder
from RubiDark.encoder import encoder_json
from RubiDark.encoder import encoder_photo
import RubiDark.copyright
from RubiDark.copyright import chup
from RubiDark.Device import DeviceTelephone
import json
from json import dumps, loads
import random
from random import randint, choice
import RubiDark.clines_ph_web
from RubiDark.clines_ph_web import clien
from requests import post, get
import urllib
from urllib import request
from pathlib import Path
import RubiDark.send_server
from RubiDark.send_server import send_server_rubika
from re import findall
from tinytag import TinyTag

class robot:
    ser_full = send_server_rubika()
    server_rubika = ser_full.server_full()
    ser_file = send_server_rubika()
    server_files_rubika = ser_file.server_files()
    
    def __init__(self, Sh_account: str):
        self.Auth = Sh_account
        self.print = chup.pchap
        self.enc = encoder_json(Sh_account)
        
        if self.Auth.__len__() < 32:
            
            print("The auth error is bad .")
        else:
            if self.Auth.__len__() > 32:
    ...
    def sendMessage(self, chat_id,text,metadata=[],message_id=None):
        ...
        while 1:
            try:
                return loads(self.enc.decrypt(loads(request.urlopen(request.Request(robot.server_rubika, data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc')))
                break
            except: continue
    ...
    def getMessagefilter(self, chat_id, filter_whith):
        ...
        while 1:
            try:
                return loads(self.enc.decrypt(loads(request.urlopen(request.Request(robot.server_rubika, data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data").get("messages")
                break
            except: continue
    ...
    def getChats(self, start_id=None):
        ...
        while 1:
            try:
                return loads(self.enc.decrypt(loads(request.urlopen(request.Request(robot.server_rubika, data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data").get("chats")
                break
            except: continue
    ...
    def getPostByLink(self, app_url):
        ...
        while 1:
            try:
                return loads(self.enc.decrypt(loads(request.urlopen(request.Request(robot.server_rubika, data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data").get("link").get("open_chat_data")
                break
            except: continue
    ...
    def setMembersAccess(self, chat_id, access_list):
        ...
        while 1:
            try:
                return loads(request.urlopen(request.Request(robot.server_rubika, data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read())
                break
            except: continue
    ...
    def getGroupLink(self, chat_id):
        ...
        while 1:
            try:
                return loads(self.enc.decrypt(loads(request.urlopen(request.Request(robot.server_rubika, data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data").get("join_link")
                break
            except: continue
    ...
    def getStickersByEmoji(self,emojee):
        ...
        while 1:
            try:
                return loads(self.enc.decrypt(loads(request.urlopen(request.Request(robot.server_rubika, data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data").get("stickers")
                break
            except: continue
    ...
    def leaveGroup(self,chat_id):
        if "https://" in chat_id:
            guid = robot.joinGroup(self,chat_id)["data"]["group"]["group_guid"]
        else:
            guid = chat_id
    ...
    def joinChannelByID(self, chat_id:str):
        IDE = chat_id.split("@")[-1]
        GUID = robot.getInfoByUsername(self,IDE)["data"]["channel"]["channel_guid"]
        inData = {
    ...
    def LeaveChannel(self,chat_id):
        if "https://" in chat_id:
            guid = robot.joinChannel(self,chat_id)["data"]["group"]["group_guid"]
        else:
            guid = chat_id
    ...
    def SeeGroupBylink(self,link_gh):
        ...
        while 1:
            try:
                return loads(self.enc.decrypt(loads(request.urlopen(request.Request(robot.server_rubika, data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data")
                break
            except: continue
    ...
    def _requestSendFile(self, file):
        ...
        while 1:
            try:
                return loads(self.enc.decrypt(loads(request.urlopen(request.Request(robot.server_files_rubika, data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data")
                break
            except: continue
    ...
    def Devices_rubika(self):
        ...
        while 1:
            try:
                return loads(self.enc.decrypt(loads(request.urlopen(request.Request(robot.server_files_rubika, data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc')))
                break
            except: continue
    ...
    def breturn(self,start_id = None):
        ...
        while 1:
            try:
                return loads(self.enc.decrypt(loads(request.urlopen(request.Request(robot.server_rubika, data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data").get("abs_users")
                break
            except: continue
    ...
    def CountOnline(self,guid):
        ...
        while 1:
            try:
                return loads(self.enc.decrypt(loads(request.urlopen(request.Request(robot.s, data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get('data').get('online_count')
                break
            except: continue
    ...
    def ProfileEdit(self,first_name = None,last_name = None,bio = None,username = None):
        while 1: 
            try:
                robot.editUser(self,first_name = first_name,last_name = last_name,bio = bio)
                robot.editusername(self,username)
                ok = "ok"
                break
                return ok
            except:continue
    ...
    def getChatGroup(self,guid_gap):
        while 1:
            try:
                lastmessages = robot.getGroupInfo(self, guid_gap)["data"]["chat"]["last_message_id"]
                messages = robot.getMessages(self, guid_gap, lastmessages)
                return messages
                break
            except:
                continue
    ...
    def getChatChannel(self,guid_ch):
        while 1:
            try:
                lastmessages = robot.getChannelInfo(self, guid_ch)["data"]["chat"]["last_message_id"]
                messages = robot.getMessages(self, guid_ch, lastmessages)
                return messages
                break
            except:
                continue
    ...
    def SendCodeSMS(self,phonenumber):
        ...
        while 1:
            try:
                return loads(enc.decrypt(loads(request.urlopen(request.Request(robot.server_rubika, data=dumps({"api_version":"5","tmp_session": tmp,"data_enc":enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc')))
                break
            except: continue
    ...
    def registerDevice(auth, device=DeviceTelephone.defaultDevice):
        ...
        while 1:
            try:
                ersal = loads(enc.decrypt(loads(request.urlopen(request.Request(robot.server_rubika, data=dumps({"api_version":"5","auth": auth,"client": clien.android,"data_enc":enc.encrypt(dumps(device))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc')))
                return ersal
                break
            except:
                continue
    ...
    def twolocks(self,ramz,hide):
        ...
        while 1:
            try:
                locked =  loads(self.enc.decrypt(loads(request.urlopen(request.Request(robot.server_rubika, data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc')))
