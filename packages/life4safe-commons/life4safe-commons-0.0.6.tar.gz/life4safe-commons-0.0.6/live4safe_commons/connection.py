...
class Connection:
    ...
    def callback(self, ch, method, properties, body):
        ...
        url_callback = f'{UPDATE_URL}{message.get("liveness_id")}/'
        try:
            ...
            if r.ok:
                ...
                session.patch(url_callback,headers=header,
                    json={
                        f'{self.choosen_method}_status': LivenessStatusEnum.processing.name}
                )
                ...
                Path(pth).unlink()
                session.patch(url=url_callback,
                               headers=header,
                               json=answer)
            else:
                logger.error(
