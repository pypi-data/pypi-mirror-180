...
logger = make_logger("datamanager", "utils")
...
class Utils:
    ...
    def sklearn_wrapper(self,
        ...
        if "X" in kwargs:
            ...
            logger.info("Launch %s", func_name)
            res = func(**kwargs)
            ...
            if save:
                logger.info("Saved %s in local vars", var_name)
            ...
            if isinstance(res, np.ndarray) and len(df_) == res.shape[0]:
                ...
                logger.info("Output of %s:%s -> %s (saved=%s)",
                            func_name,
                            str(type(res))[8:][:-2],
                            res,
    ...
    def describe(self, df_: pd.DataFrame) -> pd.DataFrame:
        ...
        if is_interactive():
            logger.info("NOTEBOOK - Description")
    ...
    def columns(self, df_: pd.DataFrame) -> pd.DataFrame:
        ...
        Returns:
            pd.DataFrame: Returns original DataFrame to keep chaining
        """
        logger.info("Columns: %s", df_.columns.values)
