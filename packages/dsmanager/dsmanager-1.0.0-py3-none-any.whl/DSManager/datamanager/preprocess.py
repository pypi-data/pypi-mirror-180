...
logger = make_logger("datamanager", "preprocess")
...
class Preprocesser:
    ...
    def to_category(self,
        ...
        if load and category_name in self.local_vars:
            ...
            logger.info("Load category in preprocesser local vars at: `%s` for column `%s`",
                    category_name,
                    column)
        else:
            col_type = pd.api.types.CategoricalDtype(**kwargs)
            self.local_vars[category_name] = col_type
            logger.info("Add category in preprocesser local vars at: `%s` for column `%s`",
                    category_name,
                    column)
        return (
            df_
