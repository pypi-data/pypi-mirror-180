...
class Paginator:
    ...
    async def _send_pages(self, channel: Union[TextChannel, DMChannel], pages: list, type: int = 2, timeout: Optional[int] = None, author: Optional[Union[Member, User]] = None, disable_on_timeout: bool = True, timeout_callback: Optional[Callable[[], Awaitable[None]]] = None):
        """
        ...
        if len(pages) == 0:
            raise TypeError("The pages list is empty")

        for page in pages:
            if not isinstance(page, Page):
                raise TypeError(f"Found {_type(page)} in the pages list. Only Page objects are allowed.")
        ...
        if type == 1:
            ...
            btns = [
                ...
                Button(emoji=self.page_emojis.forward, custom_id="forward", disabled=True if len(pages) == 1 else False)
            ]
            for i in btns:
                view.add_item(i)
