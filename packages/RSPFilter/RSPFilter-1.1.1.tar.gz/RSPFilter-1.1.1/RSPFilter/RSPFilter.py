...
pn.extension(loading_spinner="dots", notifications=True)
...
def compute_density_matrix(df, resolution, col1, col2):
    ...
    try:
        ...
        print("It is not possible to bin those two colomns.")
        return df, None
...
def cutoff_values(df, density_threshold, col1, col2):
    ...
    if len(df.query("density > @density_threshold")) > 0:        
        results = (
...
class UI:
    ...
    def update(self, event):
        ...
        if type(self.density) != None:
            self.new_density, self.results = cutoff_values(
                self.df, self.densitySlider.value,  self.col1, self.col2,
            )
            if type(self.new_density) == type(None) or type(self.results) == type(None):
                error = True
            else: 
                print(self.results)
                self.cleanN = len(self.results)
                self.NumberParticleIndicator.value = len(self.results)
        else:
            error = True

        if error:  
            self.new_density = np.zeros((10,10))
            self.density = np.zeros((10,10))
            self.cleanN = 0
            self.NumberParticleIndicator
            pn.state.notifications.error("It is not possible to calculate a density grid on those two columns. Please choose anothers", duration=4000)
