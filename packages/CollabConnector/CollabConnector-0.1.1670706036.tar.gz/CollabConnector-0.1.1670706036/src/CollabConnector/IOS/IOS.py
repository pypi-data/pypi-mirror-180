...
class Connect:
    ...
    def parse_config(self, config=None):
        if config:
            ...
            if self.config:
                ...
                self.config = self.fix_file_format(self.running_config()[0])
                config = self.config
        x = 0
        while x < len(config):
    ...
    def parse_inventory(self, config=None):
        if config:
            ...
            if self.inventory:
                ...
                config = self.fix_file_format(self.exec("show inventory")[0])
                self.inventory = []
    ...
    def parse_version(self, config=None):
        ...
        if config:
            ...
            config = self.exec("show version")[0].split("\n")
        x = -1
        while x < len(config):
            x += 1
    ...
    def parse_cdp(self, config=None):
        ...
        if config:
            ...
            config = self.exec("show cdp neighbor detail")[0].split("\n")
        x = 0
        cdp_neighbor = {}
