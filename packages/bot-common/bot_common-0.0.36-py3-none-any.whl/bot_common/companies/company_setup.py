import pandas as pd
from bot_common.companies.company_model import config_headers, flow_headers, responses_headers, ConfigPaths
...
import yaml
import json
from bot_common.lib_conf import company_configurations_table_name, flow_csv_subflow_identifier, \
    flow_csv_subflow_delimiter, flow_csv_multiple_response_ids_join, max_subflow_iterations, \
    conversational_flow_table_suffix, responses_table_suffix
...
def parse_yaml(yaml_path):
    with open(yaml_path, 'r') as stream:
        try:
            ...
            err = f"Failed to parse config file yaml {yaml_path}: {str(e)}"
            raise Exception(err)


class ConfigurationSetup:
    def __init__(self, company: str, db_config: DbConfig, paths: ConfigPaths):
        self.conversational_flow_table_name = company + conversational_flow_table_suffix
        self.responses_table_name = company + responses_table_suffix
