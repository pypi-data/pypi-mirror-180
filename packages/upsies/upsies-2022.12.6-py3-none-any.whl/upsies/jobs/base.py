...
class JobBase(abc.ABC):
    ...
    def prejobs(self):
        ...
        :attr:`autostart` stays `False` until all `prejobs` are either finished
        or :attr:`disabled <is_enabled>`
        """
    ...
    def __init__(self, *, home_directory=None, cache_directory=None,
        ...
        self._signal = signal.Signal('output', 'info', 'warning', 'error', 'executed', 'finished')
    ...
    def start(self):
        ...
        if self.is_enabled:
            ...
            _log.debug('%s: Disabled', self.name)
