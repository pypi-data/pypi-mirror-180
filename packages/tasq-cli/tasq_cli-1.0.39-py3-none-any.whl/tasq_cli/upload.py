...
def upload_file(dataset_name, db, settings, client, file_name_with_path, leaf_path):
    ...
    with open(f'{file_path}/{file_name}', 'rb') as infile:
        ...
        if not settings.dry:
            logger.info(f'Uploading {file_name_with_path} to {credentials.BUCKET} as {object_name}.')
        else:
            logger.info(f'Dry running {file_name_with_path} to {credentials.BUCKET} as {object_name}. Nothing will be uploaded.')
        try:
            upload_entry = {
                'bucket': credentials.BUCKET,
                'dataset_name': dataset_name,
                ...
                'md5_hash': md5_hash,
                'url': f'https://{credentials.BUCKET}.s3.amazonaws.com/{object_name}',
