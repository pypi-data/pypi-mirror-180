...
class SparkCredentials(Credentials):
    ...
    def __post_init__(self):
        ...
        if self.method == SparkConnectionMethod.SETU and (
            self.schema is None
            or self.url is None
            or self.session_name is None
            or self.proxy_user is None
        ):
            logger.error(
                " `schema`, `url`, `session_name`, `proxy_user` must be set in profile config"
            )
            raise dbt.exceptions.RuntimeException(
                " `schema`, `url`, `session_name`, `proxy_user` must be set when"
                f" using {self.method} method to connect to Spark"
            )
        ...
        for i in range(1 + creds.connect_retries):
            try:
                if creds.method == SparkConnectionMethod.HTTP:
                    ...
                    cls.validate_creds(
                        creds,
                        ["url", "schema", "session_name", "proxy_user"],
                    )
