...
from greatawesomeutils.lang import retry_if_is_error
...
from .selenium_driver import delete_cache
from .selenium_driver import delete_corrupted_files
from .selenium_driver import GOOGLE_DRIVER, UNDETECTED_DRIVER, create_driver_by_driver_type, delete_profile_path
...
class BaseTask():
    ...
    user = json.loads(os.environ['user'])
    task_type = os.environ['task_type']
    MODULE_NAME = os.environ['MODULE_NAME']

    task_id = os.environ['task_id']
    SKIP_IP_CHECK = get_boolean_variable('SKIP_IP_CHECK')

    MASTER_API = MASTER_API_URL

    def run(self, driver: WebDriver, data, user):
    ...
    def begin_task(self):
        ...
        try:
            ...
            save_screenshot(driver, BaseTask.user["user_id"],
                            SAVE_SCREENSHOT_SUCCESS_FOLDER, 'success')
            ...
            print(error)
            print_driver_url()
            save_html_logs(driver, user["user_id"])
            save_screenshot(driver, user["user_id"],
            ...
            if isinstance(error, RetryException):
                ask_master_to_restart()
            if isinstance(error, RerunException):
                self.begin_task()
            else:
        ...
        def do_create_driver():
            def run():
                ...
                return create_driver_by_driver_type(
                    driver_type,
                    BaseTask.user["user_id"] if BaseTask.user["user_id"] != NULL else None,
                    BaseTask.user["window_size"],
                    BaseTask.user["user_agent"],
                    is_eager)

            driver = retry_if_is_error(
                run, NETWORK_ERRORS + [(WebDriverException, lambda: delete_corrupted_files(BaseTask.user["user_id"]))], RETRY_ATTEMPTS_HIGH)
...
def short_random_sleep():
    sleep_for_n_seconds(random.randint(2, 4))


def long_random_sleep():
    sleep_for_n_seconds(random.randint(7, 17))
