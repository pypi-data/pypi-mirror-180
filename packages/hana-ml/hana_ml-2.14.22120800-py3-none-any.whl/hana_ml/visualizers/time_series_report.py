...
class DatasetAnalysis(object):
    ...
    def outlier_item(self, window_size=None, detect_seasonality=None, alpha=None, periods=None, outlier_method=None, threshold=None):
        ...
        odts = OutlierDetectionTS(window_size=window_size,
                                ...
                                threshold=threshold)
        result = odts.fit_predict(data=self.dataset, key=self.key, endog=self.endog).select(["TIMESTAMP", "RAW_DATA", "IS_OUTLIER"]).collect()
        result.set_index("TIMESTAMP")
        outliers = result.loc[result["IS_OUTLIER"] == 1, ["TIMESTAMP", "RAW_DATA"]]
    ...
    def stationarity_item(self, method=None, mode=None, lag=None, probability=None):
        ...
        option = {
            'columns': columns,
            'data': {
                columns[0]: list(stats.iloc[:, 0].astype(str)),
                columns[1]: list(stats.iloc[:, 1].astype(str))
            }

        }
        return TableItem('Stationarity', option)
