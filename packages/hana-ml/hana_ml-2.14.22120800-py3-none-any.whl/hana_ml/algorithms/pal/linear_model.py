...
class LinearRegression(PALBase):
    ...
    def __init__(self,
        ...
        self.pal_funcname = 'PAL_LINEAR_REGRESSION'
        ...
        self.aggressive_elimination, self.ps_verbose = None, None
...
class LogisticRegression(PALBase):#pylint:disable=too-many-instance-attributes
    ...
    enet_alpha : float, optional
        ...
        Elastic net mixing parameter. The elastic net mixing parameter. The value range is between 0 and 1 inclusively(0: Ridge penalty, 1: LASSO penalty).

        Only valid when ``solver`` is 'auto', 'newton', 'cyclical', 'lbfgs' or 'proximal'.
    ...
    epsilon : float, optional
        ...
        When METHOD is 'lbfgs', the condition is: ||g|| < EPSILON * max {1, ||x||}, where g is gradient of objective function, x is solve of current iteration, and ||∙|| denotes the L2 norm;

        When METHOD is 'newton', the condition is: ||x- x’|| < EPSILON * sqrt(n), where x is the solve of current iteration, x’ is the previous iteration, and n is the number of features.
    ...
    precompute : bool, optional
        ...
        Only valid when ``multi_class`` is False and ``solver`` is 'cyclical'.
