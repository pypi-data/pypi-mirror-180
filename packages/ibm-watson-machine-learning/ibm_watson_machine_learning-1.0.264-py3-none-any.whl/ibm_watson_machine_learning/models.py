...
from ibm_watson_machine_learning.utils.autoai.utils import load_file_from_file_system_nonautoai, init_cos_client, \
    prepare_auto_ai_model_to_publish_notebook, prepare_auto_ai_model_to_publish
from ibm_watson_machine_learning.utils import MODEL_DETAILS_TYPE, INSTANCE_DETAILS_TYPE, load_model_from_directory, \
    STR_TYPE, STR_TYPE_NAME, docstring_parameter, meta_props_str_conv, str_type_conv, is_lale_pipeline
...
class Models(WMLResource):
    ...
    def _publish_from_object(self, model, meta_props, training_data=None, training_target=None, pipeline=None, feature_names=None, label_column_names=None, project_id=None):
        ...
        if self._client.ICP_46 is None and self._client.ICP_45 is None and self._client.ICP_40 is None and self._client.ICP_35 is None and self._client.ICP_30 is None and self._client.WSD_20 is None:
        ...
        try:
            if 'pyspark.ml.pipeline.PipelineModel' in str(type(model)):
                ...
                if self.ConfigurationMetaNames.RUNTIME_UID in meta_props:
                    ...
                    meta_props[self._client.repository.ModelMetaNames.RUNTIME_UID] = {"href": API_VERSION + RUNTIMES + "/" + meta_props[self._client.repository.ModelMetaNames.RUNTIME_UID]}
                if self.ConfigurationMetaNames.SOFTWARE_SPEC_UID in meta_props:
                    if self._client.ICP_30 or self._client.ICP_35 or self._client.ICP_40 or self._client.ICP_45 or \
                            self._client.ICP_46:
                        self._validate_meta_prop(meta_props, self.ConfigurationMetaNames.SOFTWARE_SPEC_UID, STR_TYPE, False)
                        meta_props[self.ConfigurationMetaNames.SOFTWARE_SPEC_UID] = \
    ...
    def _publish_from_training(self, model_uid, meta_props, subtrainingId=None, feature_names=None, label_column_names=None, round_number=None):
        ...
        if "pipeline" in details["entity"]:
            if any([self._client.ICP_30, self._client.CLOUD_PLATFORM_SPACES, self._client.ICP_35, self._client.ICP_40,
        ...
        if "experiment" in details["entity"]:
            ...
            if "pipeline" in subtraining_object["entity"]:
                if any([self._client.ICP_30, self._client.ICP_35, self._client.ICP_40, self._client.ICP_45,
    ...
    def store(self, model, meta_props=None, training_data=None, training_target=None, pipeline=None,version=False,artifactid=None, feature_names=None, label_column_names=None,subtrainingId=None, round_number=None, experiment_metadata=None):
        """
        ...
        >>>        client.repository.ModelMetaNames.NAME: 'customer satisfaction prediction model'
        >>>}
        >>> stored_model_details = client.models.store(pipeline_model, meta_props=metadata, experiment_metadata=experiment_metadata)
        """
        ...
        if isinstance(model, STR_TYPE) and "autoai_sdk" in model:
            ...
            Models._validate_type(meta_props['name'], u'meta_props.name', STR_TYPE, True)
        else:
