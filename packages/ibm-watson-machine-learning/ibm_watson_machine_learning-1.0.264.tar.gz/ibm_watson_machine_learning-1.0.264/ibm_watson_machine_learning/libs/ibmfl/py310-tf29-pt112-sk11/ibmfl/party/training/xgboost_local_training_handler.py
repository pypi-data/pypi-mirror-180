...
from sklearn.ensemble._hist_gradient_boosting.common import X_DTYPE, Y_DTYPE, G_H_DTYPE
...
from ibmfl.util.xgboost.utils import is_classifier, _LOSSES
...
class XGBoostBaseLocalTrainingHandler(LocalTrainingHandler, ABC):
    ...
    def generate_sketch(self, party_id):
        ...
        get_ptile = lambda x, y: stats.percentileofscore(x, y) if not math.isnan(y) else -1
        self.X_ptiles_train = []
        for idx in range(self.X_train.shape[0]):
            self.X_ptiles_train.append([get_ptile(self.X_train[:, f_idx], \
                self.X_train[idx, f_idx]) for f_idx in range(self.X_train.shape[1])])
        self.X_ptiles_train = np.array(self.X_ptiles_train)
    ...
    def init_null_preds(self, baseline):
        ...
        self.raw_predictions = np.zeros(shape=(self.n_samples, self.n_trees),
    ...
    def collect_hist(self, party_id, k):
        ...
        if self.gradient.ndim == 1:
                ...
                h_view = self.hessian
        return party_id, g_view[:, k], h_view[:, k]
    ...
    def get_loss(self, sample_weight):
        ...
        if self.loss == 'auto':
            return _LOSSES['binary_crossentropy'](sample_weight=sample_weight) \
                ...
                _LOSSES['categorical_crossentropy'](sample_weight=sample_weight, n_classes=self.n_trees)
