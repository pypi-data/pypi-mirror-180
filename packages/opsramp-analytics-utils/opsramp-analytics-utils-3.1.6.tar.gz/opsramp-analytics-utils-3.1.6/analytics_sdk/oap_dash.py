...
class OAPDash(dash.Dash):
    ...
    def index(self, *args, **kwargs):  # pylint: disable=unused-argument
        print("entered into oap_dash index method")
        logger.info("entered into oap_dash index method")
        logging.info("entered into oap_dash index method")
        if is_authenticated():
            print("oap_dash index method IF is_authenticated")
            logger.info("oap_dash index method IF is_authenticated")
            logging.info("oap_dash index method IF is_authenticated")
            resp = self._index(args, kwargs)
            logger.info("oap_dash index method IF is_authenticated resp", resp)
            return resp
        else:
            print("entered into oap_dash index method ELSE")
            logger.info("entered into oap_dash index method ELSE")
            logging.info("entered into oap_dash index method ELSE")
            # return flask.Response('Not authorized', status=401)
            redirect_url = BASE_API_URL + f'/tenancy/web/login?cb=/loginResponse.do'
            print("oap_dash index method ELSE redirect_url", redirect_url)
            logger.info("oap_dash index method ELSE redirect_url", redirect_url)
            logging.info("oap_dash index method ELSE redirect_url", redirect_url)
