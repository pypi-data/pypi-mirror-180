...
class Circos:
    ...
    def __init__(
        ...
        for name, size in sectors.items():
            size_ratio = size / total_size
            deg_size = whole_deg_size_without_space * size_ratio
            rad_size = math.radians(deg_size)
            rad_lim = (rad_pos, rad_pos + rad_size)
            rad_pos += rad_size + math.radians(space)
            start_pos = sectors_start_pos.get(name, 0)
            sector = Sector(name, size, rad_lim, start_pos)
    ...
    def initialize_from_bed(
        ...
        return Circos(sectors, start, end, space, endspace, sectors_start_pos)
    ...
    def get_sector(self, name: str) -> Sector:
        ...
        if name not in name2sector:
            raise ValueError(f"SectorName='{name}' not found.")
    ...
    def text(
        ...
        **kwargs,
        ...
        By default, plot text on circle center
        ...
        text : str
            Text content
        deg : float
            Text degree position (0 <= degree <= 360)
        r : float
            Text radius position
        **kwargs
    ...
    def line(
        ...
        **kwargs,
        ...
        """Plot line for circos region

        Parameters
        ----------
        r : float
            Line radius position (0 <= r <= 100)
        deg_lim : tuple[float, float]
            Rectangle degree limit region (-360 <= degree <= 360).
            If None, start-end degree limit specified by user at initialization is set.
        **kwargs
    ...
    def rect(
        ...
        **kwargs,
        ...
        """Plot rectangle for circos region

        Parameters
        ----------
        r_lim : tuple[float, float]
            Rectangle radius limit region (0 <= r <= 100)
        deg_lim : tuple[float, float]
            Rectangle degree limit region (-360 <= degree <= 360).
            If None, start-end degree limit specified by user at initialization is set.
        **kwargs
    ...
    def link(
        ...
        **kwargs,
        ...
        r1 : float | None, optional
            Link radius end postion for sector_region1.
            If None, lowest radius of target sector is set.
        r2 : float | None, optional
            Link radius end postion for sector_region2.
            If None, lowest radius of target sector is set.
        color : str, optional
            Link color
    ...
    def _check_degree_range(self, start: float, end: float) -> None:
        ...
        if not min_deg <= start < end <= max_deg:
            ...
            err_msg += f"'{min_deg} <= start < end <= {max_deg}' "
            err_msg += f"(start={start}, end={end})"
            raise ValueError(err_msg)
        if end - start > max_deg:
            err_msg = f"'end - start' must be less than {max_deg} "
            err_msg += f"(start={start}, end={end})"
            raise ValueError(err_msg)
