...
class DataManageModel(object):
    ...
    def DeleteDataItem(self, id: str):
        '''
            ...
            :return: bool 类型，删除是否成功，如果itemID错误，应当抛出异常
        '''
        data = self._itemDataMap.get(str(id),None)
    ...
    def GetItemIDList(self, dataType):
        ...
        return self._kindItemDataMap[kind]
    ...
    def AddDataItem(self, dataType, data):
        ...
        assert (dataType in self._kindNameMap
                or dataType in self._kindUrlMap), "数据类型不存在"
        kind = self._kindNameMap.get(dataType, dataType)
        ret = self._saveItemData(self._kindUrlMap[kind], [data])

        if len(ret) > 0:
            self._kindItemDataMap[kind].append(ret[0])
            self._itemDataMap[str(ret[0]['timeid'])]=val
            self._itemDataMap[str(ret[0]['id'])]=val
            self._kindIdMap[str(ret[0]['timeid'])]=kind
            self._kindIdMap[str(ret[0]['id'])]=kind
            return ret[0].get('timeid', None)
        return None
