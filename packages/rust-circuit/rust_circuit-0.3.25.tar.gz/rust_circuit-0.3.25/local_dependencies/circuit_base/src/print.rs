...
impl PrintOptions {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.seen_children_same_line
            && (self.comment_arg_names
                || !self.commenters.is_empty()
                || self.print_size_threshold.is_some())
        {
            bail!("children same line + commenting (arg names, commenter, print_size_threshold) not supported")
        }
    ...
    pub fn repr_circuits(&self, circuits: Vec<CircuitRc>) -> Result<String> {
        ...
        fn recurse(
            ...
            // TODO: move to commenter
            if let Some(threshold) = selfy.print_size_threshold && circ.info().numel() > BigUint::from(threshold) && !circ.is_array() {
                node_comments.push(color(&oom_fmt(circ.info().numel()), clicolor!(Red)));
            }
...
pub fn get_child_comments(parent: &Circuit, child_pos: usize) -> Vec<String> {
    vec![match parent {
        ...
        Circuit::Module(module) => {
            if child_pos == 0 {
                "Spec".to_owned()
            } else if child_pos % 2 == 1 {
                "".to_owned()
            } else {
                let spec_pos = (child_pos - 2) / 2;
                "Arg ".to_owned() + module.spec.arg_specs[spec_pos].symbol.name().unwrap_or("")
            }
        }
        _ => "".to_owned(),
    }]
