...
pub fn rewrite_cum_to_circuit_of_cum(
    ...
    let r = match &**node {
        Circuit::Rearrange(n) => {
            ...
            let before_axis: SmallVec<[SmallVec<[u8; 8]>; 4]> = (new_spec_axis
                ..(new_spec_axis + nb_axis_before))
                .map(|i| sv![i as u8])
                .collect();
            let after_axis: SmallVec<[SmallVec<[u8; 8]>; 4]> = ((new_spec_axis + nb_axis_before)
