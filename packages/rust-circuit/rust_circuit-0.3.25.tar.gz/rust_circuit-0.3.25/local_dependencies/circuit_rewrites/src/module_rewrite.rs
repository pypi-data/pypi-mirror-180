...
use circuit_base::{
    circuit_utils::count_nodes, deep_map_op_context, module::has_sym, CircuitNode, CircuitRc,
    Concat, Module, ModuleArgSpec, ModuleSpec, Rearrange,
};
use pyo3::prelude::*;
use rr_util::{
...
#[pyfunction(use_elim_no_input_module = "true")]
#[pyo3(name = "deep_module_remove_unused_inputs")]
pub fn py_deep_module_remove_unused_inputs(
    circuit: CircuitRc,
    use_elim_no_input_module: bool,
) -> CircuitRc {
    deep_module_remove_unused_inputs(circuit, &mut Default::default(), use_elim_no_input_module)
}
    ...
    deep_map_op_context(
        ...
        &|c, _| {
            c.as_module().and_then(|m| {
                module_remove_unused_inputs(m, use_elim_no_input_module).ok()
...
pub fn module_remove_unused_inputs_op(
    ...
    let (nodes, arg_specs): (Vec<_>, Vec<_>) = m
        .arg_items()
        .into_iter()
        .filter(|(_, ModuleArgSpec { symbol, .. })| has_sym(&m.spec.circuit, symbol))
        .unzip();
    ...
    let m_out = Module::new(
        ...
        m.name().map(|s| format!("{} rem_unused", s)),
    );
...
pub fn module_remove_unused_inputs(
    ...
    Ok(module_remove_unused_inputs_op(m, use_elim_no_input_module)?.unwrap_or_else(|| m.crc()))
}
