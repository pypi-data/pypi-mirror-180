...
impl PrintOptions {
    ...
    #[staticmethod]
    pub fn size_commenter(size_threshold: Option<usize>) -> CircuitCommenter {
        let size_threshold = size_threshold.unwrap_or(0);
        CircuitCommenter::new_dyn(Box::new(move |circ| {
            if circ.info().numel() > BigUint::from(size_threshold) && !circ.is_array() {
                if size_threshold == 0 {
                    Ok(oom_fmt(circ.info().numel()))
                } else {
