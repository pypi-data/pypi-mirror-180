...
use crate::{compact_data::TinyVecU8, make_lazy, simple_from};
pub type AxisInt = u8;
...
pub fn hash_to_string(hash: &HashBytes) -> String {
    hash.iter()
        .map(|b| {
            alphabet()[((*b) % 52) as usize].clone() + &alphabet()[((*b) / 52) as usize].clone()
        })
        .join("")
}
...
macro_rules! fn_struct{
    ($vis:vis $name:ident : Fn($($iname:ident:$ity:ty),*)->$retty:ty)=>{
        paste::paste!{
            #[derive(Clone,Debug)]
            $vis enum $name{
                Py(pyo3::PyObject),
            ...
            impl $name{
                fn call(&self,$($iname:$ity),*)->anyhow::Result<$retty>{
                    match &self{
                        $name::Py(pyobj)=>rr_util::pycall!(pyobj,($($iname,)*),anyhow),
                        $name::Dyn(dynf)=>dynf.0($($iname),*),
                    }
                }
