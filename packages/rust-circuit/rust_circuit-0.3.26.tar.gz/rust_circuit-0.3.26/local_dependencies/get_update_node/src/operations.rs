...
pub struct Getter {
    ...
    pub(super) paths_cache:
        cached::UnboundCache<(HashBytes, IterativeMatcherRc), BTreeMap<CircuitRc, Vec<CircuitRc>>>,
    pub(super) all_paths_cache:
        cached::UnboundCache<(HashBytes, IterativeMatcherRc), Vec<CircuitLinkedList>>,
}
...
impl Default for Getter {
    fn default() -> Self {
        Self {
            ...
            paths_cache: cached::UnboundCache::new(),
            all_paths_cache: cached::UnboundCache::new(),
        }
    }
}
...
pub struct Expander {
    ...
    pub(super) batch_cache:
        cached::UnboundCache<(HashBytes, Vec<IterativeMatcherRc>, HashBytes), CircuitRc>,
    pub(super) validation_getter: Getter,
}
...
impl Expander {
    ...
    pub fn batch(&mut self, circuit: CircuitRc, fancy_validate: Option<bool>) -> Result<CircuitRc> {
        ...
        self.batch_impl(circuit, self.matchers.clone(), &Default::default())
    }
}
...
impl Expander {
    ...
    fn batch_impl(
        ...
        let new_children = circuit
            ...
            .map(|(c, new_matchers)| self_.batch_impl(c, new_matchers, extra_replacements))
            .collect::<Result<_>>()?;

        let expanded = expand_node(circuit.clone(), &new_children, |c, rep, child_idx| {
            self_.batch_impl(
                c,
