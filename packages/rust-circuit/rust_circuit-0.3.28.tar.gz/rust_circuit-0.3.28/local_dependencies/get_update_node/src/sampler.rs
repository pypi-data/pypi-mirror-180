...
impl RandomSampleSpec {
    ...
    pub fn indices_cache(&self) -> ::std::collections::HashMap<HashBytes, Tensor> {
...
fn combine_dtype_device_for_sample(
    ...
    device_dtype
        .clone()
        .combine(var.info().device_dtype.clone())
        .with_context(|| {
            format!(
                concat!(
                    "sampled var device_dtype failed to combine with passed device_dtype",
                    "var device_dtype={:?} device_dtype={:?}"
                ),
                &var.info().device_dtype,
                device_dtype
            )
        })
