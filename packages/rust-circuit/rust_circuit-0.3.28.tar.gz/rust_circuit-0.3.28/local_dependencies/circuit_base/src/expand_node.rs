...
use rr_util::{
    cached_method,
    compact_data::TinyBoundedBitvec,
...
pub fn expand_node(
    ...
    let get_expanded_inputs = |batchable: TinyBoundedBitvec| -> Result<Vec<CircuitRc>, usize> {
        let br = batch_rank;
    ...
    let out = match &**circuit {
        ...
        Circuit::Concat(node) => {
            ...
            let inputs = get_expanded_inputs(vec![true; inputs.len()].try_into().unwrap()).unwrap();
...
pub struct ReplaceExpander<F: ReplaceForExpand> {
    ...
    // Fix me!
    cache: cached::UnboundCache<(HashBytes, HashBytes), CircuitRc>,
}
...
type MapReplaceExpander = ReplaceExpander<ReplaceForExpandNoop>;
...
impl<F: ReplaceForExpand> ReplaceExpander<F> {
    ...
    pub fn replace_expand_with_map(
        ...
        expand_node(circuit, &new_children, |c, rep, _child_idx| {
            self_.replace_expand_with_map(c, &extra_replacements.extend_into(rep))
        })
    }
}
...
thread_local! {
    static MAP_REPLACE_EXPANDER: RefCell<MapReplaceExpander> =
        RefCell::new(MapReplaceExpander::new_noop());
}

pub fn map_replace_expand(circuit: CircuitRc, map: &ReplaceMapRc) -> Result<CircuitRc> {
    MAP_REPLACE_EXPANDER.with(|f| f.borrow_mut().replace_expand_with_map(circuit, map))
}
