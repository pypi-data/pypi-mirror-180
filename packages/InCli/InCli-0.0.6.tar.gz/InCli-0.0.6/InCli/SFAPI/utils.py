...
def deleteNulls(obj,delSystemFields=True,delNulls=True,delAttributes=True):
  ...
  #  if delSystemFields == False:
  #      delSystemFields = True
  #      systemFields = ['attributes']

    if delSystemFields == False and delNulls == False:
        return obj
    if 'records' in obj:
        obj = obj['records']
    if type(obj) is list:
        res = []
        for ob in obj:
            res.append(deleteNulls(ob,delSystemFields,delNulls))
        return res

    res = {}
    for key in obj.keys():
        if delSystemFields == True:
        ...
        if type(obj[key]) is dict:
            res[key] = deleteNulls(obj[key],delSystemFields,delNulls)
        else:
            if obj[key] is None and delNulls is True:
                continue
            res[key] = obj[key]
    return res

def printFormated(records,fieldsString=None):
    """
    ...
    for record in records:
        ...
        for key in _fields.keys():
          ...
          #  val = str(record[key]) if key in record else ''
            if _isInt(val):
