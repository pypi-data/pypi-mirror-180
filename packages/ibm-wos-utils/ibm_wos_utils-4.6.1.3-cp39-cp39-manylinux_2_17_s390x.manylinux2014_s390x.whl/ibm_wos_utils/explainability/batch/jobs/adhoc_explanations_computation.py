...
class AdhocExplanationsComputation(AIOSBaseJob):
    ...
    def run_job(self):
        ...
        try:
            ...
            self.__save_explanations(
                store, subscription.subscription_id, explanation_task_ids, explanation_types)
    ...
    def __save_explanations(self, store, subscription_id, explanation_task_ids, explanation_types):
    ...
    def __get_existing_lime_explanation(self, store, subscription):
        scoring_ids = self.arguments.get("scoring_ids")
        explanation_task_ids = self.arguments.get("explanation_task_ids")
        search_filters = "subscription_id:eq:{},status:eq:{},request_id:in:{},{}:in:{}".format(subscription.subscription_id, "FINISHED", ",".join(explanation_task_ids), subscription.scoring_id_column, ",".join(scoring_ids))
