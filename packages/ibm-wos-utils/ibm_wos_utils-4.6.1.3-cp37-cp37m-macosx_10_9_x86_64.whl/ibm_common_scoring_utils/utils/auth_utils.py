...
def get_iam_token(apikey, auth_url=None):
        ...
        try:
            ...
            logger.log_warning("Cached token has expired. New token will be generated")
            pass
        ...
        try:
            ...
            if not response.ok:
                raise Exception(f"IAM token generation failed with status code:{response.status_code}.Reason:{response.reason}.")
...
def get_cp4d_jwt_token(host:str,username:str, apikey:str=None):
    ...
    try:
        response = RestUtil.request(additional_retry_status_codes=[HTTPStatus.TOO_MANY_REQUESTS.value],method_list=["POST"]).post(url=auth_url,data=json.dumps(data),headers=headers,verify=False)
