...
class IAEInstanceClient():
    ...
    def _get_instance(self, name=None):
        ...
        instance = None
        # If service_instance_id is specified, use it to fetch the instance details
        if self.service_instance_id:
            instance = next((i for i in instances if str(i.get(
                "id")) == str(self.service_instance_id)), None)
        if instance is None:
            # Fallback to use instance_name
    ...
    def run_job(self, job_json, background=True, timeout=constants.SYNC_JOB_MAX_WAIT_TIME):
        ...
        # Not checking spark instance quota is IAE Queuing is enabled, because even if the quota is insufficient, the job will be queued and is run when there is sufficient quota. WI #26209
        if not environment.is_iae_jobs_queuing_enabled():
            resource_quota = self.get_resource_quota(self.spark_instance.instance_url)
        ...
        if self.spark_instance.use_iae_v3:
...
class IAESparkInstance():
    def __init__(self, server_url, instance):
        ...
        self.use_iae_v3 = False
        connection_info = instance.get("connection_info")
        # Check if spark instance has V3 endpoint. If not found, then fallback to use V2 endpoint
        spark_jobs_endpoint = connection_info.get("Spark jobs V3 endpoint")
        if spark_jobs_endpoint:
            self.use_iae_v3 = True
        else:
