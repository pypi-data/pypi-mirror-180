...
class MetricsUtils():
    ...
    def mainpulate_data(self, data, model_type, label_col, prediction_col, probability_col):
        ...
        if model_type != constants.REGRESSION_MODEL_TYPE:
            ...
            for label in distinct_labels:
                ...
                #put the column name within back-ticks when the column name contains the space and other special characters
            ...
            manipulated_df = df.select(prediction_col, label_col)
        ...
        if model_type == constants.REGRESSION_MODEL_TYPE:
            ...
            manipulated_df = data.select(prediction_col, label_col)
    ...
    def compute_quality_metrics(self, sc, data, model_type, label_col, prediction_col, probability_col, counts):
        ...
        self.logger.info("Fetching the rdd from the dataframe {}".format(df1))
        #predictionAndLabels = df1.rdd.map(tuple)
        predictionAndLabels = df1.rdd

        metrics = dict()
        quality_metric, confusion_matrix = self.get_metrics(
            model_type, predictionAndLabels, distinct_labels)
    ...
    def get_metrics(self, model_type, predictionAndLabels, distinct_labels = None):
