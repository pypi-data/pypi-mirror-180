...
from .utils import inNotebook, inIPythonTerminal
...
def renderTextTable(headers: List[TableHeader], rows: List[Any]) -> str:
  from texttable import Texttable
  table = Texttable()
  table.set_deco(Texttable.BORDER | Texttable.HEADER | Texttable.HLINES)

  table.set_cols_align([h.alignment[0] for h in headers])
  table.set_header_align([h.alignment[0] for h in headers])
  table.add_rows([[h.name for h in headers]] + rows)
...
def printTemplate(templateName: str, displayId: Optional[str], styles: Dict[str, str] = {}, **kwargs: Any):
  txtMode = useTxtMode()
