...
import inspect, re, json, pandas

from .environment import ALLOWED_PY_VERSIONS, getInstalledPythonVersion
from .utils import unindent
from .helpers import DeploymentTestDef, DeploymentTestError, RuntimeFile, RuntimePythonProps, getJson, getJsonOrPrintError, getMissingPackageWarningsFromEnvironment, getMissingPackageWarningsFromImportedModules, isAuthenticated, getDefaultPythonVersion, getDefaultPythonPackages, getDefaultSystemPackages
...
class Runtime:
  ...
  def _collectNamespaceDeps(self, func: Callable[..., Any], collection: NamespaceCollection):
    if not callable(func):
      return collection
    self._collectArgTypes(func, collection)
  ...
  def _collectArgTypes(self, func: Callable[..., Any], collection: NamespaceCollection):
    try:
      import ast
...
{maybeFuncVarName}.load_model('data/{maybeFuncVarName}_state.tmp')
  ...
  def _collectDataFrameModeColumns(self, df: pandas.DataFrame) -> List[Dict[str, Union[str, str]]]:
    if len(self.getFuncArgNames()) != 1:
    ...
    for col in cast(List[str], list(df.columns)):
      config.append({"name": col, "dtype": str(df[col].dtype)})
    return config
