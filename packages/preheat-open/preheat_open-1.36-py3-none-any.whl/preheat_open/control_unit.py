...
from .setpoints import InvalidSchedule, get_setpoint_schedule, send_setpoint_schedule
from .types import TYPE_DATETIME_INPUT


def validate_schedule(schedule_df: pd.DataFrame) -> None:
    if schedule_df["value"].isnull().any():
        raise InvalidSchedule("requested schedule has missing values")
    if np.isinf(schedule_df["value"]).any():
        raise InvalidSchedule("requested schedule has infinite values")
