...
# ----------------------------------------------------------------------------------------------------
    def load(self, path):
        ...
        maps['class1'] = class1
        maps['class2'] = class2
        maps['supplier'] = supplier
        maps['class_to_code'] = class_to_code
        maps['supplier_to_code'] = supplier_to_code
        return maps
# ----------------------------------------------------------------------------------------------------
    def get_format(self):
...
# ----------------------------------------------------------------------------------------------------
    def get_dict(self):
        ...
        return self.__dict
# ----------------------------------------------------------------------------------------------------
    def set_dict(self, path):
        """
        写入字典。
        """
        self.__dict = self.load(path)
        return self.__dict
# ----------------------------------------------------------------------------------------------------
    ...
    def encode(self, inf):
        ...
        for key in inf:
            if key == '价格':
                ...
                ret['分类（必填）'] = self.__dict['class2'][inf['分类']]
            elif key == '数量':
                ret['库存量（必填）'] = inf['数量']
            elif key == '补货':
                continue
