...
class EmailTool:
    ...
    def retrieve_emails(self, unseen=True, folder='Inbox', emails_to_query=10, remove_sender=None, download_type=None,
                        download_type_arg=None, extension_files=None, detach_dir='./', encode=True, subject=None):
		...
		:param subject: Search for a specific string within the email subject
        ...
        try:
            ...
            try:
                ...
                for uid in emails_uid:
                    good_to_go = True
                    typ, email_data_check = self.mailserver.uid('fetch', uid, '(BODY.PEEK[])')
                    if subject is not None:
                        raw_email_check = email_data_check[0][1].decode("UTF-8")
                        check_email_message = email.message_from_string(raw_email_check)
                        good_to_go = subject in check_email_message['Subject'].lower()
                    ...
                    if good_to_go:
                        ...
                        if all([sender.upper() not in send.upper() for send in remove_sender]):
                            ...
                            try:
                                received = pd.to_datetime(received_str, format='%d %b %Y %H:%M:%S %z').astimezone(
                                    pytz.utc).replace(tzinfo=None)
                            except:
                                received = pd.to_datetime(received_str, format='%a, %d %b %Y %H:%M:%S %z').astimezone(
                                    pytz.utc).replace(tzinfo=None)
