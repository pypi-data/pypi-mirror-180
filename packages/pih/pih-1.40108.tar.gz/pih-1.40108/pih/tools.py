from pih.collection import R, T, FieldItem, FieldItemList, FullName, Result, User
...
class EnumTool:
    ...
    @staticmethod
    def get(cls: Enum, key: str, default_value: Any = None):
        if key not in cls._member_map_:
            return default_value
        return cls._member_map_[key]

    def get_by_value(cls: Enum, value: Any):
...
class StringTool:
    ...
    @staticmethod
    def list_to_string(value: List, item_as_string: bool = False, separator: str = ", ", start: str = "", end: str = "", filter_empty: bool = False) -> str:
        return start + separator.join(list(map(lambda item: f"'{item}'" if item_as_string else str(item) if item is not None else "", list(filter(lambda item: not filter_empty or not DataTool.is_empty(item), value))))) + end
...
class ParameterList:
    ...
    def set(self, value: Any, index: int = 0) -> None:
...
class ResultTool:
    ...
    def data_is_empty(result: Result) -> bool:
        return DataTool.is_empty(result.data)
    ...
    @staticmethod
    def every(result: Result[List[T]], every_function: Callable) -> None:
        for item in result.data:
            every_function(item)

    @staticmethod
    def data_map(result: Result[List[T]], map_function: Callable) -> Result[List[R]]:
