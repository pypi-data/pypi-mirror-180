...
def create_new_workspace(ws_name: str, template: WorkspaceStarterTemplate) -> bool:
    ...
    if _existing_ws_data is not None:
        ...
        delete_existing_ws_data = typer.confirm(
            f"Would you like to delete existing record from phidata (does not delete any physical files)?",
            default=True,
        )
...
def setup_workspace(ws_root_path: Path) -> bool:
    ...
    # If this is not the users first workspace, then check if this is the active workspace
    if phi_conf.active_ws_name is None or phi_conf.active_ws_name == ws_data.ws_name:
        is_active_ws = True
