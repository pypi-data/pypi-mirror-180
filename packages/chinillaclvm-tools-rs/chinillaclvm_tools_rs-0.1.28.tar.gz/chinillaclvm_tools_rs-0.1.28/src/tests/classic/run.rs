...
use chinillaclvmr::allocator::Allocator;

use crate::classic::chinillaclvm::__type_compatibility__::{bi_one, bi_zero, Stream};
use crate::classic::chinillaclvm_tools::binutils::disassemble;
use crate::classic::chinillaclvm_tools::cmds::launch_tool;
...
#[cfg(test)]
pub struct RandomChinillaclvmNumber {
...
#[cfg(test)]
pub fn random_chinillaclvm_number<R: Rng + ?Sized>(rng: &mut R) -> RandomChinillaclvmNumber {
    ...
    RandomChinillaclvmNumber {
        intended_value: num,
    }
}

#[cfg(test)]
impl Distribution<RandomChinillaclvmNumber> for Standard {
    fn sample<R: Rng + ?Sized>(&self, rng: &mut R) -> RandomChinillaclvmNumber {
        random_chinillaclvm_number(rng)
    }
}
...
fn test_check_tricky_arg_path_random() {
    ...
    for (k, v) in path_map {
        ...
        let converted = convert_to_chinillaclvm_rs(
            &mut allocator,
