...
use crate::compiler::frontend::{from_chinillaclvm, frontend};
...
fn convert_chinillaclvm_to_chinillalisp(s: String) -> Result<Rc<SExp>, CompileErr> {
    let loc = Srcloc::start(&"*program*".to_string());
    parse_sexp(loc.clone(), s.bytes())
        .map_err(|e| {
            return CompileErr(e.0.clone(), e.1.clone());
        })
        .map(|parsed_program| from_chinillaclvm(parsed_program[0].clone()))
}

#[test]
fn test_simple_conversion_from_chinillaclvm_to_chinillalisp() {
    let converted = convert_chinillaclvm_to_chinillalisp("(+ (q . 3) 2)".to_string()).unwrap();
