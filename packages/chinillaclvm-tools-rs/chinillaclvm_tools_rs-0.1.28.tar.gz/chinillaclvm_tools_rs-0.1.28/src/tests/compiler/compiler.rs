...
use crate::compiler::chinillaclvm::run;
    ...
    let result =
        run_string(&indoc!{"
            (mod (password new_puzhash amount)
              (include *standard-cl-21*) ;; Specify chinillalisp-21 compilation.
...
#[test]
fn read_of_hex_constant_in_modern_chinillalisp() {
    let result = run_string(
...
(mod (Z)
  ...
  (check_lineage_proof 1 2 Z 4)
    ...
    assert_eq!(result.1, "chinillaclvm raise in (8 5) (() 99)");
}
