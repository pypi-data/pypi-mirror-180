...
use crate::classic::chinillaclvm::__type_compatibility__::{bi_one, bi_zero, Bytes, BytesFromType};
use crate::classic::chinillaclvm::casts::{bigint_to_bytes_chinillaclvm, bigint_to_bytes_unsigned};
use crate::classic::chinillaclvm_tools::stages::stage_0::DefaultProgramRunner;

use crate::compiler::chinillaclvm::{parse_and_run, sha256tree};
use crate::compiler::runtypes::RunFailure;
use crate::compiler::sexp::{parse_sexp, SExp};
use crate::compiler::srcloc::Srcloc;
use crate::tests::classic::run::RandomChinillaclvmNumber;

use crate::util::Number;

fn test_compiler_chinillaclvm(to_run: &String, args: &String) -> Result<Rc<SExp>, RunFailure> {
...
#[test]
fn test_chinillaclvm_1() {
    let loc = Srcloc::start(&"*test*".to_string());
    let result = test_compiler_chinillaclvm(
...
#[test]
fn test_chinillaclvm_2() {
    let loc = Srcloc::start(&"*test*".to_string());
    let result =
        test_compiler_chinillaclvm(
...
fn test_random_int_just_the_conversion_functions_and_no_other_things_from_the_stack_1() {
    ...
    for _ in 1..=200 {
        let number_spec: RandomChinillaclvmNumber = rng.gen();

        let to_bytes_chinillaclvm = bigint_to_bytes_chinillaclvm(&number_spec.intended_value).raw();
        ...
        if number_spec.intended_value == bi_zero() {
            assert!(to_bytes_chinillaclvm.is_empty());
...
fn test_sha256_tree_hash() {
    let filename = "resources/tests/assert.chinillaclvm.out";
    let assert_chinillaclvm_compiled = fs::read_to_string(filename).expect("should exist");
    let parsed = parse_sexp(
        Srcloc::start(filename),
        assert_chinillaclvm_compiled.as_bytes().iter().copied(),
    )
    .expect("should parse");
