...
use crate::classic::chinillaclvm_tools::stages::stage_2::optimize::optimize_sexp;

use crate::compiler::chinillaclvm::{
    convert_from_chinillaclvm_rs, convert_to_chinillaclvm_rs, sha256tree,
};
...
lazy_static! {
    pub static ref KNOWN_DIALECTS: HashMap<String, String> = {
        ...
        known_dialects.insert(
            ...
            indoc! {"(
           (defconstant *chinillalisp-version* 21)
        )"}
            .to_string(),
        );
        known_dialects.insert(
            ...
            indoc! {"(
           (defconstant *chinillalisp-version* 22)
        )"}
            .to_string(),
        );
        known_dialects
...
pub fn run_optimizer(
    ...
    let to_chinillaclvm_rs = convert_to_chinillaclvm_rs(allocator, r.clone())
        .map(|x| (r.loc(), x))
        .map_err(|e| match e {
    ...
    let optimized = optimize_sexp(allocator, to_chinillaclvm_rs.1, runner)
        .map_err(|e| CompileErr(to_chinillaclvm_rs.0.clone(), e.1))
        .map(|x| (to_chinillaclvm_rs.0, x))?;

    convert_from_chinillaclvm_rs(allocator, optimized.0, optimized.1).map_err(|e| match e {
        RunFailure::RunErr(l, e) => CompileErr(l, e),
        RunFailure::RunExn(s, e) => CompileErr(s, format!("exception {}\n", e)),
    })
}
...
// Extracts the environment from a chinillaclvm program that contains one.
