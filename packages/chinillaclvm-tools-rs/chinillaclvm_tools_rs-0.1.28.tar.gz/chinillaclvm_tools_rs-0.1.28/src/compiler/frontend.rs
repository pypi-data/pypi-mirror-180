...
fn from_chinillaclvm_args(args: Rc<SExp>) -> Rc<SExp> {
    match args.borrow() {
        SExp::Cons(l, arg, rest) => {
            let new_arg = from_chinillaclvm(arg.clone());
            let new_rest = from_chinillaclvm_args(rest.clone());
            Rc::new(SExp::Cons(l.clone(), new_arg, new_rest))
        }
        _ => {
            // Treat tail of proper application list as expression.
            from_chinillaclvm(args.clone())
        }
    }
}

// Form proper frontend code from CHINILLACLVM.
...
pub fn from_chinillaclvm(sexp: Rc<SExp>) -> Rc<SExp> {
    match sexp.borrow() {
        SExp::Atom(l, _name) => {
            // An atom encountered as an expression is treated as a path.
            from_chinillaclvm(Rc::new(SExp::Integer(l.clone(), sexp.to_bigint().unwrap())))
        }
        ...
        SExp::Cons(l, op, args) => {
            ...
            if is_quote_op(op.clone()) {
                ...
                let new_args = from_chinillaclvm_args(args.clone());
