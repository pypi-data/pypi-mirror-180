...
use crate::classic::chinillaclvm::__type_compatibility__::{bi_zero, Bytes, BytesFromType};
use crate::classic::chinillaclvm::casts::{
    bigint_from_bytes, bigint_to_bytes_chinillaclvm, TConvertOption,
};
...
impl SExp {
    ...
    pub fn encode_mut(&self, v: &mut Vec<u8>) {
        match self {
            ...
            SExp::Integer(_, i) => {
                let mut bi_bytes = bigint_to_bytes_chinillaclvm(i).data().to_vec();
