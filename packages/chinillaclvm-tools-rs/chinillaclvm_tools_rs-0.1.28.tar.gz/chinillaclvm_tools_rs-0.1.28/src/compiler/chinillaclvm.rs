...
fn translate_head(
    ...
    match sexp.borrow() {
        ...
        SExp::Cons(_l, _a, nil) => match nil.borrow() {
            ...
            _ => Err(RunFailure::RunErr(
                sexp.loc(),
                format!("Unexpected head form in chinillaclvm {}", sexp),
            )),
        },
    }
}
...
pub fn convert_to_chinillaclvm_rs(
    allocator: &mut Allocator,
    head: Rc<SExp>,
    ...
    match head.borrow() {
        ...
        SExp::Cons(_, a, b) => convert_to_chinillaclvm_rs(allocator, a.clone()).and_then(|head| {
            convert_to_chinillaclvm_rs(allocator, b.clone()).and_then(|tail| {
...
pub fn convert_from_chinillaclvm_rs(
    allocator: &mut Allocator,
    loc: Srcloc,
    head: NodePtr,
) -> Result<Rc<SExp>, RunFailure> {
    match allocator.sexp(head) {
        ...
        allocator::SExp::Pair(a, b) => convert_from_chinillaclvm_rs(allocator, loc.clone(), a)
            .and_then(|h| {
                convert_from_chinillaclvm_rs(allocator, loc.clone(), b)
...
fn apply_op(
    ...
    let converted_app = convert_to_chinillaclvm_rs(allocator, application.clone())?;
    let converted_args = convert_to_chinillaclvm_rs(allocator, wrapped_args.clone())?;
    ...
    runner
        ...
        .and_then(|v| convert_from_chinillaclvm_rs(allocator, head.loc(), v.1))
}
