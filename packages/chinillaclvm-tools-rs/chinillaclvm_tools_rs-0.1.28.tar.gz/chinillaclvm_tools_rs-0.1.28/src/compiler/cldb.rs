...
use crate::classic::chinillaclvm::serialize::{sexp_from_stream, SimpleCreateCHINILLACLVMObject};
...
use crate::compiler::chinillaclvm::{convert_from_chinillaclvm_rs, run_step, RunStep};
...
impl CldbOverrideBespokeCode {
    ...
    fn find_function_and_override_if_needed(
        ...
        let fun_hash = chinillaclvm::sha256tree(f);
...
pub fn hex_to_modern_sexp_inner(
    ...
    match allocator.sexp(program) {
        ...
        _ => convert_from_chinillaclvm_rs(allocator, srcloc, program).map_err(|_| {
            EvalErr(
                Allocator::null(allocator),
                "chinillaclvm_rs allocator failed".to_string(),
            )
        }),
    }
}
