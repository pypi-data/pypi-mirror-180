...
use crate::classic::chinillaclvm::as_rust::{TToSexpF, TValStack};
use crate::classic::chinillaclvm::casts::int_from_bytes;
use crate::classic::chinillaclvm::sexp::{to_sexp_type, CastableType};
            ...
            .and_then(|r| val_stack.pop().map(|l| (l, r)))
        {
            ...
            Some((l, r)) => {
                match to_sexp_f.invoke(allocator, CastableType::TupleOf(Rc::new(l), Rc::new(r))) {
                    Ok(c) => {
                        val_stack.push(CastableType::CHINILLACLVMObject(c.1));
                        None
                    }
                    Err(e) => Some(e),
                }
...
impl OpStackEntry for OpReadSexp {
    fn invoke<'a>(
        ...
        match atom_from_stream(allocator, f, b, to_sexp_f) {
            Ok(v) => {
                val_stack.push(CastableType::CHINILLACLVMObject(v));
                None
            }
            Err(e) => Some(e),
        }
...
pub struct SimpleCreateCHINILLACLVMObject {}

impl<'a> TToSexpF<'a> for SimpleCreateCHINILLACLVMObject {
