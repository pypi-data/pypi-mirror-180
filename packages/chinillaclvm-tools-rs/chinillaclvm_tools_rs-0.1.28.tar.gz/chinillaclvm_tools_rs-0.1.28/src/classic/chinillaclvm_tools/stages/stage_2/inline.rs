...
use crate::classic::chinillaclvm::sexp::{enlist, proper_list};
...
// (__chinilla__enlist X)
fn wrap_in_compile_time_list(allocator: &mut Allocator, code: NodePtr) -> Result<NodePtr, EvalErr> {
    let chinilla_enlist_atom = allocator.new_atom("__chinilla__enlist".as_bytes())?;
    enlist(allocator, &[chinilla_enlist_atom, code])
}
...
// classic chinillalisp into a macro like this:
//
...
// so i adapted list into __chinilla__enlist.
...
// In the case of inlines in classic chinillalisp, we must adjust how arguments
