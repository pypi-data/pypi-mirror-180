...
use crate::classic::chinillaclvm::__type_compatibility__::{bi_one, bi_zero, Bytes};
use crate::classic::chinillaclvm::casts::{
    bigint_from_bytes, bigint_to_bytes_chinillaclvm, bigint_to_bytes_unsigned,
};
...
impl NodePath {
    pub fn new(index: Option<Number>) -> Self {
        match index {
            Some(index) => {
                if index < bi_zero() {
                    let bytes_repr = bigint_to_bytes_chinillaclvm(&index);
