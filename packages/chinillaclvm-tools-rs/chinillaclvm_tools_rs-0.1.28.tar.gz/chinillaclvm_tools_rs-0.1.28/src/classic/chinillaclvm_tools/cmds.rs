...
use chinillaclvm_rs::run_program::PreEval;

use crate::classic::chinillaclvm::__type_compatibility__::{
    t, Bytes, BytesFromType, Stream, Tuple,
};
use crate::classic::chinillaclvm::keyword_from_atom;
use crate::classic::chinillaclvm::serialize::{
    sexp_from_stream, sexp_to_stream, SimpleCreateCHINILLACLVMObject,
};
use crate::classic::chinillaclvm::sexp::{enlist, proper_list, sexp_as_bin};
use crate::classic::chinillaclvm_tools::binutils::{
    assemble_from_ir, disassemble, disassemble_with_kw,
};
use crate::classic::chinillaclvm_tools::chinillaclvmc::detect_modern;
use crate::classic::chinillaclvm_tools::debug::{
    check_unused, trace_pre_eval, trace_to_table, trace_to_text,
};
use crate::classic::chinillaclvm_tools::ir::reader::read_ir;
use crate::classic::chinillaclvm_tools::sha256tree::sha256tree;
use crate::classic::chinillaclvm_tools::stages;
use crate::classic::chinillaclvm_tools::stages::stage_0::{
    DefaultProgramRunner, RunProgramOption, TRunProgram,
};
use crate::classic::chinillaclvm_tools::stages::stage_2::operators::run_program_for_search_paths;
...
pub fn call_tool(
    ...
    parser.add_argument(
        ...
        Argument::new()
            ...
            .set_help("path to chinillaclvm script, or literal script".to_string()),
    );
...
impl TConversion for OpdConversion {
    fn invoke<'a>(
        ...
        sexp_from_stream(
            allocator,
            &mut stream,
            Box::new(SimpleCreateCHINILLACLVMObject {}),
        )
        .map_err(|e| e.1)
        .map(|sexp| {
...
pub fn opd(args: &[String]) {
    ...
    call_tool(
        &mut allocator,
        "opd",
        "Disassemble a compiled chinillaclvm script from hex.",
...
pub fn cldb(args: &[String]) {
    ...
    let props = TArgumentParserProps {
        description: "Execute a chinillaclvm script.".to_string(),
        prog: format!("chinillaclvm_tools {}", tool_name),
    };
    ...
    parser.add_argument(
        ...
        Argument::new()
            .set_type(Rc::new(PathOrCodeConv {}))
            .set_help("filepath to chinillaclvm script, or a literal script".to_string()),
    );
    parser.add_argument(
        vec!["env".to_string()],
        Argument::new()
            ...
            .set_type(Rc::new(PathOrCodeConv {}))
            .set_help("chinillaclvm script environment, as chinillaclvm src, or hex".to_string()),
    );
...
pub fn launch_tool(stdout: &mut Stream, args: &[String], tool_name: &str, default_stage: u32) {
    ...
    // chinillaclvm_rs uses boxed callbacks with unspecified lifetimes so in order to
