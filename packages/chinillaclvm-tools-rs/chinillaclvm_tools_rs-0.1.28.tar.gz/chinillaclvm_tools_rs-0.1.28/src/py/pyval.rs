...
use crate::classic::chinillaclvm::__type_compatibility__::bi_zero;
...
pub fn python_value_to_chinillaclvm(py: Python, val: Py<PyAny>) -> Result<Rc<SExp>, RunFailure> {
    ...
    val.as_ref(py)
        ...
        .map(|l| {
            if l.is_empty() {
                ...
                for i_rev in 0..l.len() {
                    ...
                    result = SExp::Cons(
                        srcloc.clone(),
                        python_value_to_chinillaclvm(py, any_of_elt)?,
                        Rc::new(result),
                    );
                }
                Ok(Rc::new(result))
            ...
            val.as_ref(py)
                ...
                .map(|t| {
                    if t.len() != 2 {
                        ...
                        Ok(Rc::new(SExp::Cons(
                            srcloc.clone(),
                            python_value_to_chinillaclvm(py, any_of_e0)?,
                            python_value_to_chinillaclvm(py, any_of_e1)?,
                        )))
                    }
                })
                .ok()
        ...
        .unwrap_or_else(|| {
            Err(RunFailure::RunErr(
                srcloc.clone(),
                "no way to convert python value to chinillaclvm".to_string(),
            ))
        })
}

pub fn chinillaclvm_value_to_python(py: Python, val: Rc<SExp>) -> Py<PyAny> {
    val.proper_list()
        .map(|lst| {
            let mut vallist = Vec::new();
            for i in lst {
                vallist.push(chinillaclvm_value_to_python(py, Rc::new(i.clone())));
            }
        ...
        .unwrap_or_else(|| match val.borrow() {
            SExp::Cons(_, a, b) => PyTuple::new(
                py,
                vec![
                    chinillaclvm_value_to_python(py, a.clone()),
                    chinillaclvm_value_to_python(py, b.clone()),
                ],
            )
            .into_py(py),
            SExp::Integer(_, i) => {
