...
def _read_results(genome_name, contig_dict, feature_data: pd.DataFrame, result_files) -> tuple:
    ...
    rna_count = len(feature_data[feature_data['type'].isin(RNA_TARGETS)])

    context.log(f'({genome_name}) Now writing {cds_count} proteins to fasta file...')
    cds_file = context.spawn_file('cds.faa', genome_name)
    fasta.write_features_to_fasta(feature_data, 'aa', cds_file, targets=('CDS',))
    context.log(f'({genome_name}) Now writing {rna_count} RNA genes and features to fasta file...')
