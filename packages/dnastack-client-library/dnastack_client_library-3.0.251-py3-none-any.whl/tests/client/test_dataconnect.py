...
from ..exam_helper import measure_runtime, DataConversionSample, BaseTestCase
...
class TestEndToEnd(BaseTestCase, DataConnectTestCaseMixin):
    ...
    def test_auth_client_interacts_with_data_connect_service_with_no_auth(self):
        collections = [
            c.slugName for c in self._factory.get('collection-service').list_collections()
            if c.metadata.get('accessTypeLabel') and c.metadata.get('accessTypeLabel').lower() != 'public'
        ]

        if len(collections) == 0:
            self.skipTest(f'This test is not applicable to {self._context_urls}.')
