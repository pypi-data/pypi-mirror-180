...
wallet_url = env('E2E_WALLET_BASE_URL', required=False, default='https://passport.dnastack.com/')
...
class BaseTestCase(TestCase):
    ...
    _raw_context_urls = env('E2E_CONTEXT_URLS',
                            ...
                            default=','.join([
                                'viral.ai',
                                'collection-service.publisher.dnastack.com'
                            ]))
    _context_urls = _raw_context_urls.split(',')
    _factory = use(_context_urls[0],
                   no_auth=True)
