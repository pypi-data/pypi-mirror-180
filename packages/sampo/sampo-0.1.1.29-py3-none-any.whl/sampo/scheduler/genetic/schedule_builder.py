...
def build_schedule(wg: WorkGraph,
    ...
    while g < generation_number and best_fitness < prev_best_fitness:
        ...
        # renewing population
        pop.extend([ind for ind in offspring if ind.fitness.values[0] != Time.inf().value])
        hof.update(pop)
        ...
        # take 2 individuals as input 1 modified individuals
        for child1, child2 in zip(offspring[::2], offspring[1::2]):
            for ind_worker in workers_for_mate:
                # mate resources
...
def wrap(ind: ChromosomeType) -> Individual:
    """
    ...
    return initRepeat(Individual, ind_getter, n=1)
