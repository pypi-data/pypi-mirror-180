...
class ModelCheckpoint(object):
    ...
    args:
        ...
        save_best_only: When `True`, always saves the best score model to a file `checkpoint-best`. Default: ``False``.
    """
    mode_dict = {'min': torch.lt, 'max': torch.gt}

    def __init__(self, ckpt_dir, mode='min', monitor='eval_loss', verbose=True, save_best=False,
                 keys_to_ignore_on_save=None):
        if keys_to_ignore_on_save is None:
    ...
    def _save_optimizer(self, state):
        if state.get('optimizer', None):
            self._extracted_from__save_scaler_3(state, OPTIMIZER_NAME, 'optimizer')

    def _save_scheduler(self, state):
        if state.get('scheduler', None):
            self._extracted_from__save_scaler_3(state, SCHEDULER_NAME, 'scheduler')

    def _save_scaler(self, state):
        if state.get('scaler', None):
            self._extracted_from__save_scaler_3(state, TRAINER_STATE_NAME, 'scaler')

    # TODO Rename this here and in `_save_optimizer`, `_save_scheduler` and `_save_scaler`
    def _extracted_from__save_scaler_3(self, state, arg1, arg2):
        file_path = os.path.join(state['save_dir'], arg1)
        torch.save(state[arg2].state_dict(), file_path)
        state.pop(arg2)
