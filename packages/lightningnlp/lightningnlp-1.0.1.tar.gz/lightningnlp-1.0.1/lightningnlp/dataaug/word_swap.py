...
class WordSwap(BaseAugment):
    ...
    def _skip_words(self, seq_tokens):
        ...
        return [i for i, seq_token in enumerate(seq_tokens[:-1]) if seq_token not in self.stop_words and not seq_token.isdigit() and not seq_token.encode('UTF-8').isalpha() and seq_tokens[i + 1] not in self.stop_words and not seq_tokens[i + 1].isdigit() and not seq_tokens[i + 1].encode('UTF-8').isalpha()]
