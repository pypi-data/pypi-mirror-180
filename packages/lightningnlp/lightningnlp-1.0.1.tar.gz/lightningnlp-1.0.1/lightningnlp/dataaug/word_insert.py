...
class WordInsert(BaseAugment):
        ...
        if isinstance(aug_type, str):
            ...
            if aug_type in ['synonym', 'homonym', 'custom']:
                ...
                self.mlm_model = BertForMaskedLM.from_pretrained(
                    self.model_name)
                self.mlm_tokenizer = BertTokenizer.from_pretrained(
                    self.model_name)
    ...
    def _load_insert_dict(self, source_type):
        ...
        if source_type in ['synonym', 'homonym']:
            fullname = self._load_file(f'word_{source_type}')
        ...
        while t < self.create_n * self.loop and len(sentences) < self.create_n:
            ...
            predicted = ''.join(
                self.mlm_tokenizer.convert_ids_to_tokens(
                    torch.argmax(output[0][masked_positions], axis=-1)))
            for p in predicted:
    ...
    def _augment_multi(self, seq_tokens, aug_n, aug_indexes):
        ...
        if self.type in ['synonym', 'homonym', 'combination', 'custom']:
            candidate_tokens = [[aug_index, self.dict[seq_tokens[aug_index]]] for aug_index in aug_indexes if seq_tokens[aug_index] in self.dict]
        ...
        if self.type in ['synonym', 'homonym', 'combination', 'custom']:
            ...
            for aug_index in aug_indexes:
                if seq_tokens[aug_index] in self.dict:
                    candidate_tokens.extend([aug_index, token] for token in self.dict[seq_tokens[aug_index]])
