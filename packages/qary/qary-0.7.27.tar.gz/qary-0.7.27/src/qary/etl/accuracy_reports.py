...
def accuracy_report(*skills, scored_qa_pairs=None, upload=True):
    ...
    FIXME: qa skill is broke because transformers and torch upgraded
    >> filepath = pathlib.Path(init.DATA_DIR, 'testsets', 'dialog', 'qa-tiny-2020-05-24.json')
    ...
    if not scored_qa_pairs:
        scored_qa_pairs_path = pathlib.Path(init.DATA_DIR, 'testsets', 'dialog', 'qa-2020-04-25.json')
    ...
    for skill in skills:
        ...
        gzip_report_filepath = pathlib.Path(init.DATA_DIR, 'testsets', file_name)
        gzip_summary_report = pathlib.Path(init.DATA_DIR, 'testsets', "summary_accuracy_report.gz")
        ...
        if upload:
            ...
            gzip_report_filepath = pathlib.Path(init.DATA_DIR, 'testsets', file_name).unlink()

            file_to_upload = str(gzip_summary_report)
            new_name_file_to_upload = "summary_accuracy_report.gz"
            upload_to_digitalocean(file_to_upload, name_of_space, digital_ocean_path_and_newfolders, new_name_file_to_upload)
            gzip_summary_report = pathlib.Path(init.DATA_DIR, 'testsets', "summary_accuracy_report.gz").unlink()


def gzip_decompress_decode_load_json(GZIP_FILEPATH):
    """opens gzip file decompresses decodes and returns object

    >>> GZIP_FILEPATH = init.DATA_DIR / 'tests' / 'unit_test_data.gz'
