...
from qary.config import DATA_DIR, BASE_DIR, MAX_LEN_FILEPATH
...
LARGE_FILES['stanford_words'] = {
    'url': 'https://nlp.stanford.edu/~lmthang/morphoNLM/cwCsmRNN.words',
    'relative_path': 'corpora/stanford.words',
    'url_filename': 'cwCsmRNN.words',
    'filename': 'stanford.words',
    'documentation': "[Better Word Representations with Recursive Neural Networks for Morphology](https://nlp.stanford.edu/~lmthang/morphoNLM/)"
}
...
def ensure_open(f, mode='r'):
    ...
    >>> fn = maybe_download(
    ...     filename='tests/wsj_pugnlp.detector_morse.Detector.json.gz')
    >>> fp = ensure_open(DATA_DIR / fn)
    >>> fp
    ...
    if isinstance(f, io.TextIOWrapper):
        if not f.closed:
            return f
        return ensure_open(Path(f.name), mode=mode)
    if isinstance(f, Path):
        f = maybe_download(filename=f)
        return Path(f).open(mode=mode)
    if isinstance(f, bytes):
        return ensure_open(f.decode(), mode=mode)
    if isinstance(f, str):
        if len(f) > MAX_LEN_FILEPATH:
            return io.StringIO(f)
        # assume it's a path rather than data for StreamIO:
        f = maybe_download(filename=Path(f))
        f = find_filepath(f) or f
        if f and (not hasattr(f, 'seek') or not hasattr(f, 'readlines')):

            if f.lower().endswith('.gz'):
                return gzip.open(f, mode=mode)
            return ensure_open(Path(fin), mode=mode)
        return ensure_open(Path(f.name), mode=mode)  # reset path in case it is the text that needs to be opened with StringIO
    if f and getattr(f, 'closed', None):
