...
from qary.config import STOPWORDS, GLOSSARY_DOMAINS
...
def load(domains=GLOSSARY_DOMAINS):
    ...
    glossary_raw = {}
    for filepath in generate_domain_filepaths(prefix='glossary', domains=domains,
                                              data_subdir='faq', suffix=''):
        try:
            glossary_raw.update(yaml.safe_load(open(filepath)))
        except FileNotFoundError as e:
            log.error(f"{e}\n    Unable to find the glossary: {filepath}")
            break
