...
class Utilities:
    ...
    def mirror(self):
        ...
        slack_mirror = self.read_mirrors(
            self.read_file(f'{self.configs["etc_slackpkg"]}mirrors'))
    ...
    def fetch(self):
        ...
        if mir:
            ...
            if os.path.isfile(
                    f'{self.configs["var_lib_slackpkg"]}{self.configs["changelog_txt"]}'):
                slackpkg_last_date = self.read_file('{0}{1}'.format(
                    self.configs['var_lib_slackpkg'],
                    self.configs['changelog_txt'])).split('\n', 1)[0].strip()
