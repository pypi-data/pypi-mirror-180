...
from sun.__metadata__ import __all__, data
...
class Notify:
    ...
    def __init__(self):
        ...
        self.icon = f'{data["icon_path"]}{__all__}.png'
        self.n = notify2.Notification(self.summary, self.message, self.icon)
        self.n.set_timeout(60000 * int(Utilities().config()['STANDBY']))
...
def main():
    ...
    while True:
        ...
        try:
            ...
            http.request('GET', Utilities().mirror())
        ...
        if connection:
            n = Notify()
            n.show_notify()
            time.sleep(60 * int(Utilities().config()['INTERVAL']))
