...
from sun.__metadata__ import __version__, data
...
def process(cmd, message):
    ...
    start = (f'{data["bin_path"]}{data["daemon_tool"]} -frB --pidfiles=~/.run '
             f'--name=sun_daemon {data["bin_path"]}python3 '
             f'{data["bin_path"]}sun_daemon &')

    stop = (f'{data["bin_path"]}{data["daemon_tool"]} '
            '--pidfiles=~/.run --name=sun_daemon --stop')

    restart = (f'{data["bin_path"]}{data["daemon_tool"]} '
...
def init():
    ...
    if len(args) == 1:
        if args[0] == 'start':
            message = f'Starting SUN daemon:  {data["daemon_tool"]} &'
            print(process(args[0], message))

        elif args[0] == 'stop':
            message = f'Stopping SUN daemon:  {data["daemon_tool"]}'
            print(process(args[0], message))

        elif args[0] == 'restart':
            message = f'Restarting SUN daemon:  {data["daemon_tool"]}'
            print(process(args[0], message))
