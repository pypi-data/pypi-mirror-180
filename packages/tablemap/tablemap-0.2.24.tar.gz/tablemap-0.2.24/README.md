...
+ #### 4 things to keep in mind if you are familiar with computer programming. You may safely skip this if you are relatively new to programming.
    ...
    2. `rs.chain(t1)` does not contain any data in the table, yet. It simply holds instructions and is executed when it's needed. (when you want it to be saved in the database, to be printed out, to be listed up, or simply to get the size of it)

    So you can easily combine all the methods safely and freely, for example, (`filter` is not covered yet, hopefully, it's self-represented.)
    ...
    3. `tablemap` requires very little memory.

    ```python
    conn['t1_1'] = rs2
    ```

    Now it does the work. Still, when you save `rs2` to table `t1_1`, `tablemap` does not load up all of `rs2` on memory. It loads and saves one-by-one.  

    4. `tablemap` opens and closes the database automatically. Users don't have to worry about it. 
...
Or you may append an index column.
