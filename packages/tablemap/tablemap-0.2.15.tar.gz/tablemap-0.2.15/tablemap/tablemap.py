...
from itertools import (chain, dropwhile, groupby, islice, product,
                       takewhile, tee)
...
class Dics:
    ...
    def zip(self, other):
        ...
        def gen():
            for r1, r2 in zip(pgen(), _dics2iter(other)):
                r1.update(r2)
                yield r1
    ...
    def _merge(self, fn, other):
        ...
        def gen():
            yield from _step(fn, pgen(), _dics2iter(other))
    ...
    # experimental
    def tee(self, n):
        return tuple(Dics(x) for x in tee(self.iter(), n))

    # experimental
    def new(self):
        return Dics([r.copy() for r in self.iter()])
...
def _dics2iter(obj):
    return obj.iter() if isinstance(obj, Dics) else iter(obj)
