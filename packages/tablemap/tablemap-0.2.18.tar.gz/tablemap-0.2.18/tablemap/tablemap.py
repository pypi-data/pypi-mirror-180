...
# TODO: must be safer from modifications, even if it becomes slower
# Easy error messages
# Semantic warnings maybe.
...
class Rows:
    ...
    def zip(self, other):
        ...
        # because tablemap creates new rows everytime you iterate rows
        def gen2():
            for r1, r2 in zip(pgen(), _rows2iter(other)):
                r1.update(r2)
                yield r1
    ...
    def update(self, **kwargs):
        ...
        if self._history[0]['cmd'] == 'fetch':
            return self.apply(updatefn2)
        return self.apply(updatefn1)
