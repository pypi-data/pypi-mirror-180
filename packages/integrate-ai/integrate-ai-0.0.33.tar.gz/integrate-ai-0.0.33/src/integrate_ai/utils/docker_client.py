...
class DockerClient:
    ...
    def get_repo_name(self) -> str:
        ...
        If `IAI_ALT_DOCKER_IMAGE_NAME` or `IAI_ALT_SERVER_IMAGE_NAME` environment variable is set,
        this can be changed to a different image name for internal use or debugging.
        ...
        """
        if self.image_name == "fl-client":
            name = os.environ.get("IAI_ALT_DOCKER_IMAGE_NAME")
        else:
            name = os.environ.get("IAI_ALT_SERVER_IMAGE_NAME")

        base_path = "919740802015.dkr.ecr.ca-central-1.amazonaws.com/edge/"
        if not name:
            name = base_path + self.image_name
        return name
