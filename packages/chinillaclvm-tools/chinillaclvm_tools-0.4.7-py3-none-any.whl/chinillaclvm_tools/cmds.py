...
from chinillaclvm import to_sexp_f, KEYWORD_FROM_ATOM, KEYWORD_TO_ATOM, SExp
import chinillaclvm_tools_rs
from chinillaclvm.EvalError import EvalError
from chinillaclvm.serialize import sexp_from_stream, sexp_to_stream
...
try:
    from chinillaclvm_rs import run_chinilla_program, MEMPOOL_MODE
except ImportError:
    run_chinilla_program = None
...
def call_tool(tool_name, desc, conversion, input_args):
    ...
    parser.add_argument(
        ...
        nargs="*",
        type=path_or_code,
        help="path to chinillaclvm script, or literal script",
    )
...
def opc(args=sys.argv):
    ...
    call_tool("opc", "Compile a chinillaclvm script.", conversion, args)
...
def opd(args=sys.argv):
    ...
    call_tool("opd", "Disassemble a compiled chinillaclvm script from hex.", conversion, args)
...
def run(args=sys.argv):
    sys.stdout.write(bytes(chinillaclvm_tools_rs.launch_tool("run", args, 2)).decode('utf8'))
...
def launch_tool(args, tool_name, default_stage=0):
    ...
    parser = argparse.ArgumentParser(
        description='Execute a chinillaclvm script.'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="filepath to chinillaclvm script, or a literal script")

    parser.add_argument(
        "env", nargs="?", type=path_or_code,
        help="chinillaclvm script environment, as chinillaclvm src, or hex")
