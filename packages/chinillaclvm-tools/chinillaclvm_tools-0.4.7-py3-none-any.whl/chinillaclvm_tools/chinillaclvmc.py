# chinillaclvm_tools setuptools integration
...
def compile_chinillaclvm_text(text, search_paths):
...
def compile_chinillaclvm(input_path, output_path, search_paths=[]):
    if newer(input_path, output_path):
        log.info("chinillaclvmcc %s -o %s" % (input_path, output_path))
        with open(input_path) as f:
            text = f.read()
        result = compile_chinillaclvm_text(text, search_paths)
