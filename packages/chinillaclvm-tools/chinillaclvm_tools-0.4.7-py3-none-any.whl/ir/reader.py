...
from chinillaclvm import to_sexp_f
from chinillaclvm.CHINILLACLVMObject import CHINILLACLVMObject
...
def tokenize_cons(token: str, offset: int, stream: Stream) -> CHINILLACLVMObject:
    if token == ")":
...
def tokenize_int(token: str, offset: int) -> Optional[CHINILLACLVMObject]:
    try:
...
def tokenize_hex(token: str, offset: int) -> Optional[CHINILLACLVMObject]:
    if token[:2].upper() == "0X":
        try:
            token = token[2:]
            if len(token) % 2 == 1:
