...
__all__ = ["getDataTypeSize", "float16ToFloat32", "float32ToFloat16", "float16ToFloat32Array", "float32ToFloat16Array",
           "getFloat16Array", "destroyDatasetAll", "mallocHost", "freeHost", "getFloat32Array", "getDeviceIdInfo"]
...
def destroyDatasetAll(dataset: Dataset) -> int:
    ...
    """
    return _vacm.destroyDatasetAll(dataset.ptr)
...
def mallocHost(memSize: int) -> DataHandle:
    ...
    """
    return DataHandle(_vacm.mallocHost(memSize))
...
def freeHost(handle: DataHandle) -> int:
    ...
    """
    return _vacm.freeHost(handle.ptr)
...
def getDeviceIdInfo(devIdx: int) -> List[int]:
    """
    Get device index information \n
    ----------\n
    devIdx [in]: Device index..\n
    devInfo [in]: the mapping hostDevIdx,renderIdx,videoIdx.\n
    """
