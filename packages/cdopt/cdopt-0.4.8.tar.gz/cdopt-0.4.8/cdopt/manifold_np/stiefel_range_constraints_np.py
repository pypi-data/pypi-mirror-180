...
class stiefel_range_constraints_np(basic_manifold_np):
    def __init__(self, var_shape, e_vec = None) -> None:
        self._n = var_shape[-2]
        self._p = var_shape[-1]
    ...
    def C(self, X:np.ndarray):
        XX = X.swapaxes(-2,-1) @ X
        eX = (self.e_vec).swapaxes(-2,-1) @ X
        return np.concatenate(((XX-self.Ip).flatten(), (X @ eX.swapaxes(-2,-1) - self.e_vec).flatten() ),axis = 0)
    ...
    def JC(self, X, Lambda):
        ...
        return 2 * X@ T  + self.e_vec @ (gamma.T @ X) + gamma @ ((self.e_vec).swapaxes(-2,-1) @ X) 
