...
class CreateAccessTokenCommand(qumulo.lib.opts.Subcommand):
    ...
    def options(parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            ...
            help=(
                ...
                'longer be usable for authentication. The timezone is interpreted as UTC.'
            ),
        )
        ...
        parser.add_argument(
            ...
            help=(
                ...
                '--credential-store argument to authenticate using the created access token.'
            ),
        )
    ...
    def main(rest_client: RestClient, args: argparse.Namespace) -> None:
        access_token = rest_client.access_tokens.create(
            args.identifier, expiration_time=args.expiration_time
        )
...
class ModifyAccessTokenCommand(qumulo.lib.opts.Subcommand):
    ...
    @staticmethod
    def main(rest_client: RestClient, args: argparse.Namespace) -> None:
        metadata = rest_client.access_tokens.modify(args.id, expiration_time=args.expiration_time)
...
def make_access_keys_table(access_tokens: Sequence[AccessTokenMetadata]) -> str:
    headers = ['id', 'user', 'creator', 'creation time', 'expiration time']
    rows = []
...
class ListAccessTokensCommand(qumulo.lib.opts.Subcommand):
    ...
    @staticmethod
    def main(rest_client: RestClient, args: argparse.Namespace) -> None:
        responses = rest_client.access_tokens.list(user=args.user)
