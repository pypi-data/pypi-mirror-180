def stml(input_file) :
 output_file=input_file.replace(".stml",".html")
 ...
 for j in file1:
  ...
  if jj.strip() != "" and jj[len(jj)-len(jj.lstrip())]!="#":
   str,colon,c1,p,leadspace,colon_after,cl1,i_count ="",0,0,0,0,0,0,0
   close_ind1=[]
   for i in j:
    i_count+=1
    if i!="\n":
   ...
   if colon >=1 and colon_after>0and exc!=1:
      temt="<"+j.lstrip()+">\n"
      file2.write(temt)
      if str not in remove_list:
         close_ind.append(leadspace)
