...
class EFSVolume(Construct):
    """Construct to aggregate EFS Volumes operations
    ...
    Args:
        ...
        to_be_backed_up (bool): Flag to tag the resource to be backed up and grant access from Rubrik's Cloud Cluster. Default: True (only works if the environment is production)
    ...
    def __init__(
        ...
        if to_be_backed_up:
            Tags.of(self.efs).add("RubrikBackup", "true")
            ...
            _ec2.CfnSecurityGroupIngress(
                ...
                group_id=self._efs_security_group.security_group_id,
            ...
            Tags.of(self.efs).add("RubrikBackup", "false")
        ...
        if aws_back_up:
            ...
            key_arn = account_id
            key_arn = (
                "arn:aws:kms:eu-west-1:" + account_id + ":key/" + aws_account["kms_vault_key"]
            )
            encryption_key = _kms.Key.from_key_arn(self, "encryption_key", key_arn)

            # Create AWS Backup Role

            backup_role = _iam.Role(
                self,
                "aws_backup_role",
                description=app_name + "_" + "aws_backup_role",
                assumed_by=_iam.ServicePrincipal("backup.amazonaws.com"),
                role_name=app_name + "_" + "aws_backup_role",
            )
                ...
                _iam.Policy(
                    ...
                    statements=[
                        ...
                        _iam.PolicyStatement(
                            ...
                            resources=["arn:aws:backup:*:*:backup-vault:*" + app_name + "*"],
                        ),
                        # Policy for EFS
                        _iam.PolicyStatement(
                            actions=[
                            ...
                            resources=["arn:aws:elasticfilesystem:*:*:file-system/" + self.efs.file_system_id]
                        ),
                        _iam.PolicyStatement(
            ...
            vault_encryption_key= _kms.Key.from_key_arn(self, id=app_name + "vault_key", key_arn=key_arn)
            backup_vault = _backup.BackupVault(self, app_name, backup_vault_name =app_name + "_" + "Vault", encryption_key=vault_encryption_key, removal_policy=vault_removal_policy)

            # Create AWS Backup Plans

            # Backup Plan
            backup_plan = _backup.BackupPlan(self, app_name + "_" + "BackupPlan", backup_vault=backup_vault,
                            backup_plan_rules=([
                                                    (_backup.BackupPlanRule(
                                                                             backup_vault=backup_vault,
                                                                             delete_after=cdk.Duration.days(35),
                                                                             rule_name=app_name + "_" + "DailyPlan",
                                                    ...
                                                    (_backup.BackupPlanRule(
                                                                             ...
                                                                             rule_name=app_name + "_" + "MonthlyPlan",
                                                                             schedule_expression=_events.Schedule.cron(
                                                                             hour="20",
                                                                             minute="30",
                                                                             day="L",
            ...
            # Add Efs to the Backup Plans

            plan_selection = _backup.CfnBackupSelection(self, app_name + "_backup_selection",
              backup_plan_id=backup_plan.backup_plan_id,
              backup_selection=_backup.CfnBackupSelection.BackupSelectionResourceTypeProperty(
                iam_role_arn=backup_role.role_arn,
                selection_name=app_name + "_" + "backup_selection",
                resources=[
                  'arn:aws:elasticfilesystem:' + "*" + ':' + "*" + ':file-system/' + self.efs.file_system_id
                ]
              )
            )
