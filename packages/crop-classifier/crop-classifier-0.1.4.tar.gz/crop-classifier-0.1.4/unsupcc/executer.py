...
class IndexExecuter:
    def __init__(self, output_dir_path='tmp'):
        ...
        self.shape_file_path = input('provide complete path of district shapefile:  ')
        self.bbox = input('provide valid bounding box in order - minx miny maxx maxy! Skip if '
                          'passing shapefile') or None
        self.width = int(input('Enter width of array if input is raster: ')) or None
        self.height = int(input('Enter height of array if input is raster:')) or None
