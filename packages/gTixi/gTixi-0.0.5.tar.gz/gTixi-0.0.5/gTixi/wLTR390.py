...
'''
def LoopShow():
    # record LTR390 readings on GTC platform
# save every hour
    ...
    wa.Sleep(0.5)
    Q=sensor.AReadings
    #Time=K.f('wiTimeStr','!get|longStr',M)
    ftSave=3600
    sf='LTR390测试\n{}  \n{}\n{}↑暂停 ←退出 '
    ...
    #fStart=time.time()
    while k!='L':
        if count%10 == 0 and not pause:
            try:
                al=Q()
            except:
                print('LTR reading error')
            #DispChar(sf.format('UV:{iUv:.3g}, {fLux:.3g} lux'.format(**al),
    'Light: {:.3g}'.format(al["fLight"]),''))
            wa.Sleep(0.1)
        count+=1
        k=CheckKey()
        if k=='U':
