...
PixConsole::PixConsole(int w, int h, std::string const& font_file, int size)
    : font_ptr{std::make_shared<ConsoleFont>(font_file, size)}, font{*font_ptr},
      cols(w), rows(h)
{
    init();
}
...
PixConsole::PixConsole(int w, int h, FreetypeFont& ftfont)
    : font_ptr{std::make_shared<ConsoleFont>(ftfont)}, font{*font_ptr}, cols(w),
      rows(h)
...
*/
PixConsole::PixConsole(int w, int h, std::shared_ptr<ConsoleFont> const& _font)
