...
SIMULATOR_ROOT_PATH = Path(os.path.expanduser(os.getenv("CHINILLA_SIMULATOR_ROOT", "~/.chinilla/simulator"))).resolve()
...
async def start_async(root_path: Path, group: Any, restart: bool) -> None:
    """
    Simulator wrapper of the chinilla async_start function
    """
    import sys

    from chinilla.cmds.start_funcs import async_start

    sys.argv[0] = sys.argv[0].replace("chdv", "chinilla")  # this is the best way I swear.
    if root_path.exists():
        config = load_config(root_path, "config.yaml")
        await async_start(root_path, config, group, restart, True)
    else:
        print(f"Simulator root path: {root_path} does not exist.")
        print("please run 'chdv sim create' to create and configure a new simulator.")
...
def create_chinilla_directory(
    ...
    """
    This function creates a new chinilla directory and returns a heavily modified config,
    suitable for use in the simulator.
    """
    from chinilla.cmds.init_funcs import chinilla_init

    if not chinilla_root.is_dir() or not Path(chinilla_root / "config" / "config.yaml").exists():
        # create chinilla directories & load config
        chinilla_init(chinilla_root, testnet=True, fix_ssl_permissions=True)
...
def display_key_info(fingerprint: int, prefix: str) -> None:
    """
    Display key info for a given fingerprint, similar to the output of `chinilla keys show`.
    """
...
async def generate_plots(config: Dict[str, Any], root_path: Path, fingerprint: int, bitfield: bool) -> None:
    ...
    from chinilla.simulator.start_simulator import PLOT_SIZE, PLOTS
...
async def async_config_wizard(
    ...
    if fingerprint is None:
        ...
        return
    # create chinilla directory & get config.
    print("Creating chinilla directory & config...")
    config = create_chinilla_directory(root_path, fingerprint, farming_address, plot_directory, auto_farm, docker_mode)
...
async def print_status(
    ...
    """
    from chinilla.cmds.show_funcs import print_blockchain_state  # TODO: Change for 1.6
    ...
    if show_key:
        ...
        if fingerprint is not None:
            ...
            print(
                "No fingerprint in config, either rerun 'chdv sim create' "
                "or use --fingerprint to specify one, skipping key information."
            )
    # chain status ( basically chinilla show -s)
    ...
    print(
        ...
        f"with a balance of: "
        f"{sum(coin_records.coin.amount for coin_records in farming_coin_records) / units['chinilla']} TXCH."
    )
    if show_addresses:
        print("All Addresses: ")
...
async def farm_blocks(
    ...
    if target_address is None:
        print(
            ...
            "You can use 'chdv sim create' or use --target-address to specify a different address."
        )
        target_ph: bytes32 = await node_client.get_farming_ph()
    else:
