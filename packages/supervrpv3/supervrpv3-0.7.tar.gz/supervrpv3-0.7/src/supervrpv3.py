...
class RouteLocation(VRPObject):
    ...
    def __add__(self, other) -> RouteLocation:
        ...
        loc_route = RouteLocation(self.index, self.location_index, self.working_interval, self.break_intervals, items,
                                  self.vrp_type, self.lat, self.lng, self.load_coef)
...
class FreeVehicle(VRPObject):
    ...
    def __gt__(self, other: tuple) -> bool:
        return self.weight > other[0] and self.cbm > other[1]
    ...
    def from_drop_assigned_vehicle(cls, veh: AssignedVehicle, context_veh: VRPVehicle):
        working_interval = TimeInterval.split_interval([context_veh.working_interval], veh.assigned_interval.start)[-1][
            0]
...
class AssignedVehicle(VRPObject):
    ...
    @classmethod
    def from_assign_route(cls, veh: FreeVehicle, route: VRPObjectList[VRPLocation],
                          assigned_requests: VRPObjectList[VRPRequest]):
        return cls(
            veh.index,
            veh.trip_index,
    ...
    def extract_route(self, context, start_time=None, has_load_time=True):
        ...
        self.free_time_rate = round((
                                                context_veh.working_interval.end - self.assigned_interval.end) / context_veh.working_interval.interval_range,
                                    2)
...
class LocationPreprocessor(BasePreprocessor):
    ...
    def __post_init__(self, index_manager: IndexManager, locations: List[Location], customers: List[Customer], depots: List[Depot]):
        ...
        self.set_coord(vrp_stations, locations)

        self.set_coord(vrp_customers, locations)

        self.set_coord(vrp_depots, locations)
    ...
    def set_coord(self, vrp_locations: List[VRPLocation], locations: List[Location]):
        for loc in vrp_locations:
            for i in range(len(locations)):
                if loc.location_index == locations[i].location_code:
                    loc.lat = locations[i].lat
                    loc.lng = locations[i].lng
                    break
...
class ElementResult:
    ...
    def from_location(cls, location: RouteLocation, vrp_code: str, index_manager, context: ContextManager):
        ...
        arrival_time_datetime = datetime.strftime(datetime.fromtimestamp(arrival_time), const.DATETIME_FORMAT)
        departure_time_datetime = datetime.strftime(datetime.fromtimestamp(departure_time), const.DATETIME_FORMAT)
