...
from python_jose_TPM import jws
...
def encode(claims, esapi, key_handle, algorithm=ALGORITHMS.RS256, headers=None, access_token=None):
    ...
    return jws.sign(claims, esapi, key_handle, headers=headers, algorithm=algorithm)


def decode(token, esapi, key_handle, algorithms=None, options=None, audience=None, issuer=None, subject=None,
           access_token=None):
    ...
    try:
        payload = jws.verify(token, esapi, key_handle, algorithms, verify=verify_signature)
