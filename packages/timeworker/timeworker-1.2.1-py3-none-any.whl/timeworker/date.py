...
from timeworker.date_parser import date_parser
from timeworker.freshness_date_parser import freshness_date_parser
from timeworker.languages.loader import LocaleDataLoader
from timeworker.conf import apply_settings, check_settings
from timeworker.parser import _parse_absolute, _parse_nospaces
from timeworker.timezone_parser import pop_tz_offset_from_string
from timeworker.utils import apply_timezone_from_settings, \
    set_correct_day_from_settings, \
    get_timezone_from_tz_string
from timeworker.custom_language_detection.language_mapping import map_languages
...
class _DateLocaleParser:
    ...
    def _parse(self):
        # 四种方式: ['timestamp', 'relative-time', 'custom-formats', 'absolute-time']
        for parser_name in self._settings.PARSERS:
            date_data = self._parsers[parser_name]()
            # fix period bug start
            if type(date_data) is list:
                if all([self._is_valid_date_data(l) for l in date_data]):
                    return date_data
            # fix period bug end
    ...
    def _try_freshness_parser(self):
        try:
            # fix period bug start
            time_str = self._get_translated_date()
            if 'year' in time_str and 'ago' not in time_str:
                return None
            if time_str == 'yesterday':
                return [freshness_date_parser.get_date_data("1 day ago", self._settings),
                        freshness_date_parser.get_date_data("1 day ago", self._settings)]
            elif time_str == 'the_day_before_yesterday':
                return [freshness_date_parser.get_date_data("2 day ago", self._settings),
                        freshness_date_parser.get_date_data("2 day ago", self._settings)]
            elif time_str == 'a_few_days_ago':
                return [freshness_date_parser.get_date_data("3 day ago", self._settings),
                        freshness_date_parser.get_date_data("3 day ago", self._settings)]
            # 上周几 / 本周几
            elif re.match(r'(\d) week ago (\d)', time_str):
                r = int(re.match(r'(\d) week ago (\d)', time_str).group(1))
                weekday = int(re.match(r'(\d) week ago (\d)', time_str).group(2))
                y, w, _ = datetime.now().strftime("%Y-%W-%w").split('-')
                start_time = DateData(date_obj=datetime.fromisocalendar(int(y), int(w)-r, weekday), period='day')
                end_time = DateData(date_obj=datetime.fromisocalendar(int(y), int(w)-r, weekday), period='day')
                return [start_time, end_time]
            return [freshness_date_parser.get_date_data(time_str, self._settings),
                    freshness_date_parser.get_date_data("0 day ago", self._settings)]
    ...
    def _try_parser(self, parse_method):
        ...
        try:
            ...
            time_str = self._get_translated_date()
            date_obj, period = date_parser.parse(time_str, parse_method=parse_method, settings=self._settings)
            self._settings.DATE_ORDER = _order
            # fix period bug
            month_str = ["january", "february", "march", "april", "may", "june",
                         "july", "august", "september", "october", "november", "december"]
            if ('year' in time_str) and ('ago' not in time_str) and (not any([True for m in month_str if m in time_str])):
                start_date_obj = date_obj.replace(date_obj.year, 1, 1)
                date_obj = date_obj.replace(date_obj.year, 12, 31)
            else:
                start_date_obj = date_obj.replace(date_obj.year, date_obj.month, 1)
            return [DateData(date_obj=start_date_obj, period=period),
                    DateData(date_obj=date_obj, period=period)]
        except ValueError:
            self._settings.DATE_ORDER = _order
            return None
    ...
    def _get_translated_date(self):
        if self._translated_date is None:
            self._translated_date = self.locale.translate(self.date_string, keep_formatting=False,
...
class DateData:
    """
    Class that represents the parsed timeworker_data with useful information.
    ...
    def get_date_data(self, date_string, date_formats=None):
        ...
        *Period* represents the granularity of date parsed from the given string.
        ...
        for locale in self._get_applicable_locales(date_string):
            ...
            if parsed_date:
                # fix period bug start
                if type(parsed_date) is list:
                    for i in range(len(parsed_date)):
                        parsed_date[i]['locale'] = locale.shortname
                    return parsed_date
                # fix period bug end
