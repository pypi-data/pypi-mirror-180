...
from .utils.additional import str_preprocess, get_time_again

_default_parser = DateDataParser()


@apply_settings
def parse(date_string, date_formats=None, locales=None,
          region=None, settings=None, detect_languages_function=None, languages=['zh-Hans']):
    ...
    date_string = str_preprocess(date_string)

    try:
        data = parser.get_date_data(date_string, date_formats)
        if data:
            if isinstance(data, list):
                if (data[1]['date_obj'] - data[0]['date_obj']).days < 0:
                    data[0], data[1] = data[1], data[0]
                return [str(data[0]['date_obj']).split(' ')[0], str(data[1]['date_obj']).split(' ')[0]]
            else:
                return get_time_again(date_string)

    except Exception as e:
        print('Time parser Error: ', e)
        return []

    # if timeworker_data:
    #     return timeworker_data['date_obj']
