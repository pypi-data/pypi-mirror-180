...
class Pattern:
    ...
    def get_mask_single(self, score, exact_sparsity_ratio):  # pragma: no cover
    ...
    def get_sparsity_ratio(self, pre_masks):  # pragma: no cover
        zero_cnt = 0
        total_cnt = 0
    ...
    def get_pattern_lock_masks(self, modules):  # pragma: no cover
        """
...
class PatternNxM(Pattern):
    ...
    def get_sparsity_ratio(self, pre_masks):
        ...
        if isinstance(self.block_size, list):
            self.block_size = self.get_block_size_dict(pre_masks)  # pragma: no cover
        for key in pre_masks.keys():
            ...
            if shape[0] % block_size[0] != 0 or shape[1] % block_size[1] != 0:  # pragma: no cover
    ...
    def get_masks_global(self, scores, target_sparsity_ratio, pre_masks, max_sparsity_ratio_per_layer,
        ...
        if not k < 1:
            ...
            for key in not_divided_keys:  # pragma: no cover
                p = scores[key]
