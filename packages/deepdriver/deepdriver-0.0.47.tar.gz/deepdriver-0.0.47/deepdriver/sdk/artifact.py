...
class ArtifactEntry:
    ...
    def __init__(self, path: str, local_path: str, size: int, digest: str, status: str, lfs_yn :str, repo_tag: str, type: str, metadata: str, key: str=""):
    ...
    def download(self, local_root_path: str, artifact_id: int, team_name: str, exp_name: str, artifact_name: str, artifact_type: str, versioning: str ,  file_index: int, total_file_count:int ):
        ...
        if self.digest != downloaded_file_digest:
            raise Exception(f"file check sum error : checksum value are '{self.digest}' expected, but '{downloaded_file_digest}'")
        ...
        if extension.upper() == ".JSON":
            if len(file_name.split(".")) > 2:
                self.key, self.type = file_name.split(".") #Table
        elif extension.upper() == ".PNG" or extension.upper() == ".JPG":
            if len(file_name.split(".")) > 2:
                self.key, self.type = file_name.split(".")  #Image
...
class Artifacts:
    ...
    def add(self, data: Union[str, Table, Image, Chart], name: str = None) -> None:
        if isinstance(data, str):
            ...
            self.entry_list.append(ArtifactEntry(path=data.get_path(name), local_path=local_path, size=size, digest=digest, status="ADD", lfs_yn="", repo_tag="", type="TABLE", metadata=data.to_json(name), key=name))
        ...
        if isinstance(data, str):
            if not os.path.isfile(data):
                ...
                if entry:
                    ...
                    self.entry_list.append(ArtifactEntry(path, local_path, size, digest, status="ADD", lfs_yn="", repo_tag="", type="FILE", metadata=""))  # 파일이 추가된 경우
