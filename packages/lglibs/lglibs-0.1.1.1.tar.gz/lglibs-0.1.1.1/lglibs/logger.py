...
def Log(messages, level=None, writer=Echo_Mode.InfoWriter, file_write=True, file_color=False, **kwargs):
    ...
    if type(messages) in (list, dict):
        ...
        if _config["file"] and file_write and not file_color:
            ...
            level_color = ansi_escape.sub("", level_color)
            messages = ansi_escape.sub("", messages)
            end = ansi_escape.sub("", end)
