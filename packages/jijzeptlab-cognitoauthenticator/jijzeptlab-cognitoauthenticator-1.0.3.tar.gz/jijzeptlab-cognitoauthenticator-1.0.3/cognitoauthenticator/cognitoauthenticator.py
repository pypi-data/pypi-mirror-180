...
from .handlers import (AuthorizationAreaHandler, ChangePasswordAdminHandler,
                       ChangePasswordHandler, DiscardHandler,
                       EmailAuthorizationHandler, LoginHandler, LogoutHandler,
                       SignUpHandler, ToggleAuthorizationHandler)
from .orm import UserInfo
from .apihandlers import CognitoAPIHandler
...
class CognitoAuthenticator(Authenticator):
    ...
    def successful_login(self, username, access_token, id_token, refresh_token):
        if self.login_attempts.get(username):
            self.login_attempts.pop(username)
        if access_token is not None and id_token is not None and refresh_token is not None :
    ...
    async def authenticate(self, handler, data):
        ...
        try:
            CognitoCli = boto3.client("cognito-idp", region_name=self.CognitoRegion)
            response = CognitoCli.initiate_auth(
                AuthFlow=self.CognitoAuthFlow,
        ...
        validations = [user.is_authorized]
        if user.has_2fa:
    ...
    async def logout(self, username):
        user = self.get_user(username)
        self.reset_cognito_token(user)
        print(user)
        return

    def reset_cognito_token(self, user):
        user.access_token = ""
        user.id_token = ""
        user.refresh_token = ""
        self.db.commit()
    ...
    def change_password(self, username, old_password, new_password):
        ...
        try:
            ...
            response = CognitoCli.change_password(
    ...
    def get_handlers(self, app):
        native_handlers = [
            ...
            (r"/api/cognito-token", CognitoAPIHandler),
        ]
    ...
    def get_cognito_token(self, username):
        user = self.get_user(username)
        return {
