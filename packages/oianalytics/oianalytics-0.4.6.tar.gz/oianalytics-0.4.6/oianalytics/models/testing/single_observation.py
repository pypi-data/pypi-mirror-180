...
def test_process_data(
    ...
    if len(vector_time_values_names) > 0:
        df_list = [
            pd.concat(
                output_vector_time_values[name].to_dict(), axis=1, sort=True,
            ).T
        ...
        model_outputs.add_output(
            outputs.VectorTimeValuesOutput(
                data=df_list, data_reference=vector_time_values_names,
            )
        )
        ...
        if len(vector_batch_values_names) > 0:
            ...
            model_outputs.add_output(
                outputs.VectorBatchValuesOutput(
                    data_reference=vector_batch_values_names, data=df_list,
                )
            )
