...
def init_db(db, schema, file_list, version_file_list):
    ...
    try:
        ...
        if not version:
            ...
            # 获取配置数据
            recreate_schame = False
            recreate_schame_key = "RECREATE_SCHEMA"
            if current_app.config.get(recreate_schame_key):
                recreate_schame = current_app.config.get(recreate_schame_key)
            elif os.environ.get(recreate_schame_key):
                recreate_schame = bool(strtobool(os.environ.get(recreate_schame_key)))

            # 判断是否创建
            if schema_exist and recreate_schame:
