...
from byteplus_sdk.const.Const import REGION_AP_SINGAPORE1
...
api_info = {
    ...
    "DescribePullToPushBandwidthData": ApiInfo("POST", "/",
                                               {"Action": "DescribePullToPushBandwidthData",
                                                "Version": LIVE_SERVICE_VERSION}, {},
                                               {}),
    "ListStorageSpace": ApiInfo("POST", "/",
                                      {"Action": "ListStorageSpace",
                                       "Version": LIVE_SERVICE_VERSION}, {},
                                      {}),
    "DescribeLiveStorageSpaceData": ApiInfo("POST", "/",
...
class LiveService(Service):
    ...
    def __init__(self, region=REGION_AP_SINGAPORE1):
    ...
    def list_storagespace(self, params):
        action = "ListStorageSpace"
        res = self.json(action, dict(), params)
        if res == '':
            raise Exception("%s: empty response" % action)
    ...
    def describe_live_storagespace_data(self, params):
        action = "DescribeLiveStorageSpaceData"
        res = self.json(action, dict(), params)
        if res == '':
            raise Exception("%s: empty response" % action)
