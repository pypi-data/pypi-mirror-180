...
from pandera.model import _extract_config_options_and_extras
...
def _pydantic_schema_fix_config_inheritance(schema: Any = None):
    # src: _collect_config_and_extras from pandera.model.SchemaModel
    config = getattr(schema, pandera.model._CONFIG_KEY, {})

    bases = inspect.getmro(config)[:-1]
    bases = tuple(base for base in bases if issubclass(base, pandera.model.BaseConfig))
    ...
    return type(
        f"{str(schema.__module__)}.Config", (pandera.model.BaseConfig,), dict(ChainMap(*options))
    )
