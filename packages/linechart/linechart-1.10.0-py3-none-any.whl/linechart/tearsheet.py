...
from .utils import get_cagr, get_cum_returns, get_drawdowns, get_sharpe, with_baseline
...
class Tearsheet(BaseTearsheet):
    ...
    def create_full_tearsheet(self, performance):
        ...
        if num_cols > self.max_cols_for_details:
            warnings.warn(
                "Suppressing details because there are more than {0} columns "
                "(you can control this setting by modifying "
                "Tearsheet.max_cols_for_details)".format(self.max_cols_for_details)
            )
        ...
        if any([exposures is not None for exposures in (performance.net_exposures, performance.abs_exposures)]):
    ...
    def create_summary_tearsheet(self, performance, agg_performance=None):
        ...
        stats.append(
            [
                "Total Months",
                round(
                    (agg_performance.returns.index.max() - agg_performance.returns.index.min())
        ...
        stats.append(["Sortino Ratio", "%.2f" % ep.sortino_ratio(agg_performance.returns)])
        stats.append(["Calmar Ratio", "%.2f" % ep.calmar_ratio(agg_performance.returns)])
        stats.append(["Skew", "%.2f" % scipy.stats.skew(agg_performance.returns)])
        stats.append(["Kurtosis", "%.2f" % scipy.stats.kurtosis(agg_performance.returns)])
        ...
        with sns.color_palette(color_palette):
            ...
            if performance.abs_exposures is not None:
                ...
                norm_cagrs.plot(
                    ax=axis,
                    kind="bar",
                    title="Normalized CAGR (CAGR/Absolute Exposure) (Details)",
                )
    ...
    def _create_returns_by_year_plots(self, performance, rows, row, fig, extra_label):
        ...
        cagrs_by_year = grouped_returns.apply(
            lambda x: get_cagr(
    ...
    def create_montecarlo_tearsheet(self, performance, cycles=5, aggregate_before_shuffle=True):
        ...
        with_baseline(cum_sim_returns).plot(
            ax=axis,
            title="Montecarlo Cumulative Returns ({0} cycles)".format(cycles),
            legend=False,
        )
        ...
        get_drawdowns(cum_sim_returns).plot(
            ax=axis,
            title="Montecarlo Drawdowns ({0} cycles)".format(cycles),
            legend=False,
        )
