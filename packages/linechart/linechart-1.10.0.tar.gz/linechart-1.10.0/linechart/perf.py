    ...
    def __init__(
        ...
        if trim_outliers:
            ...
            if max_zscore > 20:
                ...
                warnings.warn(
                    "Found returns which are {0} standard deviations from the "
                    "mean, consider removing them with the `trim_outliers` parameter".format(round(max_zscore))
                )
        self.pnl = pnl
        ...
        try:
            ...
            if "StrategyOrDate" in results.columns:
                raise MoonchartError(
                    "this looks like a parameter scan CSV, please use ParamscanTearsheet.from_csv"
                ) from e
            else:
                raise
    ...
    def rolling_sharpe(self):
        if self._rolling_sharpe is None:
            self._rolling_sharpe = get_rolling_sharpe(
                self.returns, window=self.rolling_sharpe_window, riskfree=self.riskfree
            )
