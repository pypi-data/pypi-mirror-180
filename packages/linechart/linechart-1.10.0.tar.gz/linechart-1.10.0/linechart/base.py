...
class BaseTearsheet:
    ...
    def __init__(self, pdf_filename=None, figsize=None, max_cols_for_details=25):
        ...
        self._suptitle = None
        self._suptitle_kwargs = {"bbox": {"facecolor": "#EAEAF2", "edgecolor": "white", "alpha": 0.5}}
    ...
    def _create_returns_plots(self, performance, subplot, extra_label, figsize=None, legend_title=None):
        """
        ...
        with sns.color_palette(color_palette):
            ...
            if (
                ...
                subplot != 212
                and (include_commissions or include_slippage)
            ):
            ...
            if len(performance.rolling_sharpe.index) > performance.rolling_sharpe_window:
                ...
                self._y_format_at_least_two_decimal_places(axis)
                plot = performance.rolling_sharpe.plot(
                    ax=axis,
                    title="Rolling Sharpe ({0}-day) {1}".format(performance.rolling_sharpe_window, extra_label),
                )
                axis.set_xlabel("")
            ...
            if performance.benchmark_returns is not None:
                ...
                try:
                    vs_benchmark = pd.concat(
                        (performance.cum_returns, benchmark_cum_returns),
                        axis=1,
                        sort=True,
                    )
                except TypeError:
                ...
                plot = with_baseline(vs_benchmark).plot(
                    ax=axis,
                    title="Cumulative Returns vs Benchmark {0}".format(extra_label),
                )
                axis.set_xlabel("")
