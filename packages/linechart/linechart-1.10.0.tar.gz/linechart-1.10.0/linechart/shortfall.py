...
from .perf import AggregateDailyPerformance, DailyPerformance
from .utils import get_cum_returns, with_baseline
...
class ShortfallTearsheet(BaseTearsheet):
    ...
    def __init__(self, *args, labels=("simulated", "actual"), **kwargs):
    ...
    def from_csvs(
        ...
        return t.create_full_tearsheet(x_perf, y_perf, largest_n=largest_n, shift_x_positions=shift_x_positions)

    def create_full_tearsheet(self, x_performance, y_performance, largest_n=None, shift_x_positions=0):
        """
        ...
        if largest_shortfalls is not None:
            largest_shortfalls_x_mask = largest_shortfalls.shortfall.unstack().reindex(
                index=x_performance.returns.index, columns=x_performance.returns.columns
            )
            largest_shortfalls_y_mask = largest_shortfalls.shortfall.unstack().reindex(
                index=y_performance.returns.index, columns=y_performance.returns.columns
            )
    ...
    def create_largest_shortfalls_tearsheet(self, x_performance, y_performance, largest_n=5):
        ...
        if shared_columns.empty:
            warnings.warn(
                "skipping largest shortfalls table because {x} and {y} have no shared columns".format(
        ...
        largest_shortfalls = (
            returns.sort_values("abs_shortfall", ascending=False).drop("abs_shortfall", axis=1).head(largest_n)
        )
    ...
    def _plot_field(
        ...
        if cumplots:
            ...
            fig, (ax1, ax2, axis) = plt.subplots(
                ...
                1,
                sharex=True,
                gridspec_kw={"height_ratios": [0.5, 0.25, 0.25], "hspace": 0.1},
                num="Cumulative " + fig_title + fig_title_suffix,
    ...
    def create_summary_tearsheet(self, x_performance, y_performance):
        ...
        stats.append(
            [
                ...
                [
                    round(
                        (x_performance.returns.index.max() - x_performance.returns.index.min())
                        / pd.Timedelta(365.25 / 12, "D")
                    ),
                    round(
                        (y_performance.returns.index.max() - y_performance.returns.index.min())
                        / pd.Timedelta(365.25 / 12, "D")
                    ),
                ],
            ]
        ...
        stats.append(
            [
                "Sharpe Ratio",
                ["%.2f" % x_performance.sharpe, "%.2f" % y_performance.sharpe],
            ]
        )
        stats.append(
            [
            ...
            stats.append(
                [
                    ...
                    [
                        "{0}%".format(
                            round(
                                (x_performance.cagr / x_performance.abs_exposures.mean()) * 100,
                                1,
                            )
                        ),
                        "{0}%".format(
                            round(
                                (y_performance.cagr / y_performance.abs_exposures.mean()) * 100,
                                1,
                            )
                        ),
                    ],
