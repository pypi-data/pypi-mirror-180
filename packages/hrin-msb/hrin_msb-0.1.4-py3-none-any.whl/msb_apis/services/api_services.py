...
class ApiService(CustomExceptionHandler, metaclass=Singleton):
	...
	def search(self, params: SearchParameter):
		try:
			model_query = self.db_model.objects.only(*params.fields).filter(**params.filters)
			if params.order_field is not None:
				model_query.order_by(params.order_field)
	...
	def create(self, *model_data_list):
		try:
			...
			if len(model_data_list) == 1:
				_model = (self.db_model(**model_data_list[0])).save()
				create_status = True
			else:
