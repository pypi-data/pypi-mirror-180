...
def current_time(frmt=DT_CONST.DEFAULT_TIME):
	return datetime.now().time().strftime(frmt)


def current_date(frmt=DT_CONST.DEFAULT_DATE):
	return datetime.now().date().strftime(frmt)


def current_timestamp(frmt=DT_CONST.TIMESTAMP):
...
def timestamp_diff(latest=None, old=None, return_in='sec|min|hr|day'):
	...
	try:
		latest = datetime.strptime(latest, DT_CONST.TIMESTAMP)
		old = datetime.strptime(old, DT_CONST.TIMESTAMP)
		diff = (latest - old)
...
def create_datetime(
	inp_dtime=None, hours=0, mins=0, secs=0, days=0, inp_fmt=DT_CONST.TIMESTAMP,
	out_fmt=None
):
	try:
...
def parse_datetime(inp_dtime=None, inp_fmt=DT_CONST.TIMESTAMP):
	result = None
	try:
...
def convert_to_utc(inp_dtime=None, inp_fmt=DT_CONST.TIMESTAMP, out_fmt=None, ist_time_diff='0:00'):
	try:
		out_fmt = inp_fmt if out_fmt is None else out_fmt
