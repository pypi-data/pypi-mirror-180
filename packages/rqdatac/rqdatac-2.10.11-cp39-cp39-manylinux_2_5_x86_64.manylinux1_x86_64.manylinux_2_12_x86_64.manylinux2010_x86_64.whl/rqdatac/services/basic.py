...
def _convert_to_wind(order_book_id):
    ...
    if inst_type == 'Future':
        if exchange != 'CZCE' or order_book_id.endswith(('88', '88', '888', '889')):
...
_wind_reg = re.compile(r'(((?P<future>[A-Z]{2})(?P<future_id>\d{3,4}))|((?P<option_id>((\d{8})|((?P<option>[A-Z]{1,2})(?P<option_suffix>\d{3,4}\-?(P|C)\-?\d{3,}))))))\.?(?P<ex>(CFE|SHF|INE|DCE|CZC|SH|SZ))')
...
def _id_convert_one(order_book_id):  # noqa: C901
    ...
    if r:
        ...
        if d['future']:
            if d['ex'] != 'CZC':
                return d['future'] + d['future_id']
            return d['future'] + _current_year + d['future_id']
        if not d['option']:
            return d['option_id']
