...
def process_template(config, args):
    template_name = args[0][1:]
    args = args[1:]

    td = config.get("templates", template_name)
    if not td:
        errors.syntax_error("Template not found: {}".format(template_name))

    hidden = utils.safe_value(td, "hidden", False)
    if hidden:
        errors.syntax_error("cannot use a template that is marked as hidden: {}".format(template_name))

    templ = td["command"]
    targs = templ.split()
    tats = [a for a in targs if a.startswith("@")]

    if len(tats) > len(args):
        errors.syntax_error("templates requires {} args, but only {} were supplied".format(len(tats), len(args)))
...
def main(cmd=None, new_start_time=None, capture_output=False, basic_mode=False, raise_syntax_exception=True):
    ...
    if args and args[0].startswith("@"):
        args = process_template(config, args)
