...
class Runner():
    ...
    def send_email_to_approvers(self, request_id, requested_by, description, target, nodes, runs, cmd):
        ...
        requested_by_contacts = team_dict[requested_by]["contact"]
        ...
        subject = "job submission request: {}".format(request_id)
        ...
        try:
            utils.send_email(self.config, approver_contacts, requested_by_contacts, subject, body)
            sent = True
    ...
    def create_request(self, ws_name, snapshot_dir, username, target, nodes, runs, args):
        ...
        rd = {"status": "requested", "requested_by": username, "description": description, 
        ...
        run_cmd = orig_xt_cmd
        code_option = " --code=$requests/{}/code ".format(request_id) 

        # replace the "--request" option with code_option
        index = run_cmd.index("--req")
        index2 = run_cmd.find(" ", index+5)
        if index2 > -1:
            run_cmd = run_cmd[0:index].strip() + code_option + run_cmd[index2:].strip()
        else:
            run_cmd = run_cmd[0:index].strip() + code_option
        ...
        approvers, sent = self.send_email_to_approvers(request_id, username, description, target, nodes, runs, run_cmd)

        # final message to requesting user
        if sent:
            console.print("  request emailed to approvers: {}".format(", ".join(approvers)))
