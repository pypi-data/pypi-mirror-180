...
class OdbcDatabase(implements(DbInterface)):
    ...
    def _init_core_tables(self, dbx, create=False):
        ...
        col_dict["node_stats"] = {
            ...
            "post_start_time": TIME, "post_end_time": TIME,
        ...
        col_dict["requests"] = {"_id": VARCHAR, "ws_name": VARCHAR, "request_id": VARCHAR, "status": VARCHAR,
            "target": VARCHAR, "nodes": INT, "runs": INT, "cmd": BIGCHAR,
    ...
    def create_db_job(self, jd, update_job_stats=True):
        ...
        self.update_job_info(ws_name, job_id, jd, update_primary=True, new_job=True, update_job_stats=update_job_stats)

    # API call
    def update_job_info(self, ws_name, job_id, orig_dd, update_primary=False, new_job=False, update_job_stats=True):
        '''
        Args:
        ...
        if self.update_job_stats and update_job_stats:
    ...
    def create_request(self, ws_name, request_id, request_dict):
        ...
        request_dict["create_time"] = utils.get_arrow_time_str()
    ...
    # API call
    def mark_job_cancelled(self, ws_name, job_id):
        '''
        ...
        if self.update_job_stats:
            ...
            cmd += " [running_runs] = 0, "
            cmd += " [running_nodes] = 0, "
            cmd += " [job_status] = 'cancelled'"
            #cmd += " [end_time] = '{}'".format(now_str)
            cmd += " WHERE [_id] = '{}' ".format(_id)

            record = self._execute_with_retry("mark_job_cancelled", cmd=cmd, dbx=dbx)

            console.diag("mark_job_cancelled")
