...
class Dispatcher():
    ...
    def extract_template_option(self, args):
        ...
        return template_option, new_args
    ...
    def dispatch(self, args, is_rerun=False, capture_output=False, raise_syntax_exception=False):
        ...
        # extract the template option since it has curly brackets that are not converted to quotes
        template_option, args = self.extract_template_option(args)

        # join args together to get original cmd line (but without quotes)
        text = " ".join(args)
