...
class ImplCompute(ImplBase):
    ...
    @command(kwgroup="cancel", kwhelp="cancels active runs, jobs, or requests", help="cancels the specified jobs and their active or queued runs")
    ...
    @command(kwgroup="cancel", kwhelp="cancels jobs, runs, requests", help="cancels job submission requests (created by xt run --request)")
    ...
    def send_email_to_requester(self, request_id, requester, approver, job_id):
        team_dict = self.config.get("team")
        requester_contacts = team_dict[requester]["contact"]
        approver_contacts = team_dict[approver]["contact"]
        ...
        try:
            utils.send_email(self.config, requester_contacts, approver_contacts, subject, body)
            sent = True
        except BaseException as ex:
            console.print("email send failed: ex: {}".format(ex))
    ...
    def approve(self, names, workspace):
        ...
        for record in records:
            ...
            if status == "requested":
                ...
                # run the command, capturing the output so we can extract the job_id
                ...
                self.send_email_to_requester(name, requester, username, job_id)
                console.print("approval emailed to requester: {}".format(requester))
    ...
    @command(kwgroup="cancel", help="cancels all queued/active runs on the specified compute target")
