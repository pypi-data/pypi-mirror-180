...
from factoree_ai_connectors.source.s3.formatted_s3_source_connector import FormattedS3SourceConnector
from factoree_ai_connectors.source.s3.s3_source_connector import S3SourceConnector
from factoree_ai_connectors.source.sqs.sqs_utils import get_sqs_url
from factoree_ai_connectors.text.text_parsers import CsvParser
...
class S3Utility:
    def __init__(
        ...
        self.source_connector: FormattedS3SourceConnector[list[list[any]]] = FormattedS3SourceConnector(
            S3SourceConnector(
                region_name,
                self.sqs_url,
                aws_access_key,
        ...
        self.sink_connector = S3SinkConnector(
            S3Client(aws_access_key, aws_secret_key),
            logger
        )
        self.logger = logger
    ...
    def move_file_to_completed_folder(self, event: S3FileCreatedEvent) -> bool:
        return self.__move_file(
            event.bucket_name,
            event.filename,
            event.filename.replace(f'{INPUT}{PATH_SEPARATOR_CHAR}', f'{COMPLETED}{PATH_SEPARATOR_CHAR}')
        )
