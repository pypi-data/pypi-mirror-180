...
TransformErrors = list
TransformTuple = (TransformOutput, TransformErrors)
...
class TransformUtility(ABC, Generic[T]):
    def __init__(
            self,
            logger: Logger,
            is_test: bool = False
    ...
    @abstractmethod
    def transform(self, file: T, timezone_name: str) -> TransformTuple:
        pass


class BronzeToSilverRunner(ABC, Generic[T]):
    ...
    def attempt_to_run_iteration(self, file_created_event: S3FileCreatedEvent) -> (list[S3JsonFile], bool):
        if file_created_event:
            file: T = self.read_s3_file(file_created_event)
            ...
            self.move_file_to_completed_folder(file_created_event)
    ...
    @ final
    def transform_bronze_file(self, event: S3FileCreatedEvent, file_created: T) -> list[S3JsonFile]:
        silver_files: list[S3JsonFile] = []
        try:
    ...
    @final
    def read_s3_file(self, event: S3FileCreatedEvent) -> T:
    ...
    @final
    def move_file_to_completed_folder(self, file_created_event: S3FileCreatedEvent) -> bool:
        return self.s3_utility.move_file_to_completed_folder(file_created_event)
