...
class App(cmd2.Cmd):
    ...
    def do_connect(self, connect_args):
        ...
        if self.nucleus_driver.connect(connection_type=connect_args.connection_type):
            self.nucleus_driver.messages.write_message('Successfully connected to Nucleus device')
            self.nucleus_driver.messages.write_message('ID: {}'.format(self.nucleus_driver.connection.nucleus_id))
            self.nucleus_driver.messages.write_message('GETFW: {}'.format(self.nucleus_driver.connection.firmware_version))
        else:
            self.nucleus_driver.messages.write_warning('Failed to connect to Nucleus device')
    ...
    def do_assert_download(self, download_asserts_args):
        ...
        if download_asserts_args.path is not None:
            path = download_asserts_args.path
        else:
            print('PATH argument is None')
    ...
    download_syslog_parser.add_argument('path', help='Set path for syslog file download', completer=cmd2.Cmd.path_complete)
    ...
    def do_syslog_download(self, download_syslog_args):
        ...
        if download_syslog_args.path is not None:
