...
from delta_utils.utils import find, get_team_name


def pkl_checker_value_dict(pkl_loader: Callable[[str], Dict], expected_type: Type) -> None:
    """Checks a dictionary acting as a value lookup table."""
    main = find("main.py")
    import_path = "main" if Path("main.py").exists() else f"{main.parent.name}.{main.stem}"

    try:
        team_name = get_team_name(__import__(import_path, fromlist=["None"]))
    except AttributeError as e:
        raise ImportError("No TEAM_NAME found in file main.py") from e
