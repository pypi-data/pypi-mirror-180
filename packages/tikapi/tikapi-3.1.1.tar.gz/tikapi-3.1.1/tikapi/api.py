...
def Rests(options: dict):
	...
	class Public(Rests.BaseClass):
		...
		def check(cls, username: str = None, user_id: str = None, country: str = None, **otherParams) -> APIResponse:
			...
			return wrap({"path":"/public/check","help":"Get a user's profile information","comment":"Get profile information and statistics from a username or user_id.","params":{"username":{"help":"The TikTok user username","validate":"^([a-zA-Z0-9_.]+|https?://vm.tiktok.com/[a-zA-Z0-9]+/?)$","type":"string","example":"lilyachty"},"user_id":{"help":"Optionally you can get the profile information using the user_id parameter.","type":"string","validate":"^[0-9]+$"}},"method":"GET"}, cls.__get_options__(), "Public.check")(username=username, user_id=user_id, country=country, **otherParams)
