        ...
        if (req_path).is_file():            
            ...
            with open(req_path, 'w') as f:                
                ...
                if 'shareddata==' in s:
                    ...
                    lines[lidx] = new_version
                    _s = '\n'.join(lines)                    
                    f.write(_s)
            
            send_command('git -C '+str(path)+' commit -a -m "'+new_version+'"')
            send_command('git -C '+str(path)+' push')
            send_command(str(path/'venv\Scripts\python.exe')+' -m pip install '+new_version)
