...
res_ved_dict =  {'Агрегат сварочный_res_fact' : 'АПС (агрегат передвижной сварочный)_res_fact',
...
res_dict_all = {' Трубоукладчик ТГ-122Я_res_fact':'Трубоукладчик_res_fact',
 ...
 'Бурильщик, помошник бурильщика_res_fact':'Бурильщик, помошник бурильщика_res_fact',
 'Бурильщик, помощник бурильщика_res_fact':'Бурильщик, помошник бурильщика_res_fact',
 'Бурильщик_res_fact':'Бурильщик, помошник бурильщика_res_fact',
...
def get_work_pulls(ksg_for_val_data):
    ...
    for i in model_dataset.index:
        ...
        for c in model_dataset.columns:
            if model_dataset.loc[i,c] != 0:
                pull.append(c)
        work_pulls.append(pull)
    work_pulls_final = []
    for p in work_pulls:
         if p not in work_pulls_final and len(p) !=0 :
            work_pulls_final.append(p)
    return work_pulls_final


def get_validation_dataset(val_files, pulls, act, res):
    """Function for generating a validation dataset

    Args:
        val_files (list): list of json files names which contain acts from ksg
        pulls (list): list of work pulls
    ...
    for file in val_files:
        ...
        for w in msg_json['work']:
            name = w["work title"]
            if name:
                if name not in act_msg:
                    act_msg.append(name)
                for w_d in w['work_data']['progress']:
                    try:
                        msg_df.loc[list(w_d.keys())[0], name] = float(w_d[list(w_d.keys())[0]]['fact'])
                    except:
                        msg_df.loc[list(w_d.keys())[0], name] = 0
