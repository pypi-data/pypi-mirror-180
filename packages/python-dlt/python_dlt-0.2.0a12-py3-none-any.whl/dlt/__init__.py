...
How to create a data loading pipeline with dlt in 3 seconds:
    ...
    >>> import dlt
    >>> import requests
    >>> dlt.run(requests.get("https://api.chess.com/pub/player/magnuscarlsen/games/2022/11").json()["games"], destination="bigquery", table_name="games")

    2. Run your pipeline script
    $ python magnus_games.py

    3. See and query your data
    $ dlt pipeline show magnus_games.py
