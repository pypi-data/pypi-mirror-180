...
class PyocsDemand:
    ...
    def get_ci_plus(self):                     
        if self.Ltcresponsejson.get("data").get("CI PLUS 功能") != None:
            ciplus_list = list()
            ciplus_list.append(self.Ltcresponsejson.get("data").get("CI PLUS 功能"))        
    ...
    def get_ddr_size(self):
        ...
        if self.Ltcresponsejson.get("data").get("External DDR") and self.Ltcresponsejson.get("data").get("主芯片"):
            ...
            if external_ddr_str:
                if "总内存是" in chip_desc_str:
                    ddr_original_str = chip_desc_str.split('总内存是')
                    ...
                    if ddr_digital_list:
                        ...
                        return ""
                elif "共内存是" in chip_desc_str:
                    ddr_original_str = chip_desc_str.split('共内存是')
                    ddr_digital_list = re.findall(r"(.+?)B", ddr_original_str[-1])
                    if ddr_digital_list:
                        ddr_size = ddr_digital_list[0]
                        self._logger.info("ddr size：" + ddr_size)
                    ...
                    ddr_original_str = chip_desc_str.split('使用')
                    ddr_digital_list = re.findall(r"(.+?)B", ddr_original_str[-1])
                    if ddr_digital_list:
                        ddr_size = ddr_digital_list[0]
                        self._logger.info("ddr size：" + ddr_size)
