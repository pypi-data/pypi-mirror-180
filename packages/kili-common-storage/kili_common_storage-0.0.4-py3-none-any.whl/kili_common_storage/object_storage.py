...
class ObjectStorage(object):
    ...
    def parse_file_key(self, system, url):
        return {
            self.common_struct.OBJECT_STORAGE_SYSTEM_OBS: (
                self.parse_file_key_of_obs),
            self.common_struct.OBJECT_STORAGE_SYSTEM_S3: (
    ...
    @args_log()
    def get_file_url(self, file, resource=None, size=None):
        if not file:
            return file
    ...
    @args_log()
    def get_file_urls(self, files, resource=None, size=None):
        return [self.get_file_url(file, resource, size)
