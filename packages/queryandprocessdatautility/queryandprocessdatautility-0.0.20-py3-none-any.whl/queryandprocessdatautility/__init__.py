...
class IncentiveCalculation:
    ...
    def trainemodel(self,input_column_name,train_column_name,ModelName):
        try:
            if ModelName == 'sentiment':
                logger.log(f"inside sentimentTraining()","0")
                jsonData = request.get_data('jsonData', None)
                logger.log(f"requestParam:::::::{jsonData}{type(jsonData)}","0")
                jsonData = json.loads(jsonData[9:])
                logger.log(f"jsonData: {jsonData}","0")
                dfJson   = jsonData['dfJson']
                df = pd.DataFrame(dfJson)
                colNamesLst = jsonData['colNamesLst']
                if 'filename' in jsonData.keys():
                    if jsonData['filename'] != None and jsonData['filename'] != "":
                        filename = jsonData['filename'] 
                else:
                    filename = "sentiment_training_model" 
            ...
            elif ModelName == 'classification':
                logger.log(f"inside classificationTraining()","0")
                jsonData = request.get_data('jsonData', None)
                logger.log(f"requestParam:::::::{jsonData}{type(jsonData)}","0")
                jsonData = json.loads(jsonData[9:])
                logger.log(f"jsonData: {jsonData}","0")
                ...
                review_df = df[colNamesLst]
                logger.log(f'\nreview_df.shape : {review_df.shape}',"0")
                train_column_name=review_df[review_df[colNamesLst[1]]]
                # sentiment_label=review_df[colNamesLst[1]].factorize()  # creates vector Array
                input_column_name = review_df.colNamesLst[0].values
            ...
            logger.log(f'\n In sentimentTraining exception stacktrace : ', "1")
            trace = traceback.format_exc()
            descr = str(e)
            returnErr = self.getErrorXml(descr, trace)
            logger.log(f'\n Print exception returnSring inside sentimentTraining : {returnErr}', "0")
            return str(returnErr)
        else:
            dir = 'models'
            if not os.path.exists(dir):
                os.makedirs(dir)
            model.save("models/" + filename)     # to serialize model configuration and save model                  
            with open('models/'+ filename +'/tokenizer.dictionary', 'wb') as tokenizer_dictionary_file:
                pickle.dump(tokenizer, tokenizer_dictionary_file)
            logger.log("Sentiment Model has been trained and saved successfully.","0") 


    def predict(self, textColumn, model):
        logger.log(f"inside predict_sentiment","0")
        predicted_Sentiment=None
        filename= model + "_training_model"
        predicted_label_lst=[]
        if model == "sentiment":
            with open('models/'+ filename +'/tokenizer.dictionary', 'rb') as config_dictionary_file:
            ...
            loaded_Model = keras.models.load_model("models/"+ filename)
            ...
            logger.log(f"filename 1120 : {filename}","0")
            predictmodel = joblib.load("models/"+model+".pkl")
            logger.log(f"predictmodel : {predictmodel}","0")
            # logger.log(f"{knn_from_joblib.predict(textColumn)}","0")
