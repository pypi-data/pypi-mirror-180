...
import time
import random
from chinillaclvm import KEYWORD_FROM_ATOM, KEYWORD_TO_ATOM
from chinillaclvm.operators import OP_REWRITE
...
def p2_delegated_or_hidden_puzzle():
    # src/wallet/puzzles/p2_delegated_puzzle_or_hidden_puzzle.chinillaclvm
    # parameters:
...
print('compiling...')
for fn in glob.glob('benchmark/*.chinillaclvm'):
...
for n in range(5):
    ...
    for fn in glob.glob('benchmark/*.hex'):
        ...
        if '--brun' in sys.argv:
            ...
            cost, result = run_chinilla_program(
                program_data,
                env_data,
                max_cost,
                flags,
