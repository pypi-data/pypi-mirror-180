...
from chinillaclvm_rs import run_chinilla_program

def run_chinillaclvm(fn, env=None):
    ...
    # constants from the main chinilla blockchain:
    # https://github.com/Chinilla/chinilla-blockchain/blob/main/chinilla/consensus/default_constants.py
    max_cost = 11000000000
    cost_per_byte = 12000

    max_cost -= (len(program_data) + len(env_data)) * cost_per_byte
    return run_chinilla_program(
        program_data,
        env_data,
        max_cost,
        0,
...
if __name__ == "__main__":
    ...
    try:
        start = time()
        cost, result = run_chinillaclvm(sys.argv[1], sys.argv[2])
        duration = time() - start;
        print(f"cost: {cost}")
