    ...
    def getEvent(self):
        ...
        for event in eventList:
            ...
            if event.type == pygame.KEYDOWN:
                #点击ESC按键让我方坦克重生
                if (event.key == pygame.K_ESCAPE or event.key==pygame.K_r) and not MainGame.TANK_P1:
                    #调用创建我方坦克的方法
                    self.creatMyTank()
                if MainGame.TANK_P1 and MainGame.TANK_P1.live:
                    # 具体是哪一个按键的处理
...
class Bullet(BaseItem):
    def __init__(self,people): #输入的参数people可以是己方老师和对方的学生 
        #图片
