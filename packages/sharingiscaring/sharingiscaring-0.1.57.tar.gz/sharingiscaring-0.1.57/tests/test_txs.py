...
def test_tx_delegation_decreased(node: ConcordiumClient):
    ...
    delegationResult = tx.classify_transaction_for_bot_for_delegation()
    ...
    assert delegationResult.delegation_message == 'Delegation target set to 77,032. Stake decreased (90.00%) to 1,000 CCD.'
...
def test_tx_delegation_increased(node: ConcordiumClient):
    ...
    assert delegationResult.delegation_message == 'Stake increased (112.50%) to 8,500 CCD.'