...
class Transaction:
            ...
            for event in self.result.get('events', []):
                ...
                if event['tag'] in [TransactionType.ContractUpdated.name, 'Updated']:
                    ...
                    if event["receiveName"] == 'BictoryCns.register':
                        ...
                        self.cns_domain.domain_name = self.cns_domain.decode_domain_from(event["message"])
    ...
    def init_from_node(self, t):
        ...
        self.block      = t.get('blockInfo', None)
        self.cost       = t['cost']
    ...
    def request_account_on_previous_block(self, account_id, block_height):
    ...
    def classify_transaction_for_bot_for_delegation(self):
        t = self
            ...
            if t.result['outcome'] == 'success':
                ...
                for event in events:
                    ...
                    if event['tag'] in ['DelegationStakeIncreased', 'DelegationStakeDecreased']:
                        previous_amount = self.request_account_on_previous_block(result.sender, self.block['blockHeight'])
                        if previous_amount:
                    ...
                    if event['tag'] == 'DelegationSetDelegationTarget':
                        if event['delegationTarget']['delegateType'] == 'Baker':
                            ...
                            result.delegation_message += f"Delegation target set to {result.bakerId:,.0f}. "
                    
                    elif event['tag'] == 'DelegationStakeIncreased':
                        if previous_amount:
                            result.delegation_message += f"Stake increased{result.perc} to {(int(event['newStake'])/1_000_000):,.0f} CCD."
                        else:
                            result.delegation_message += f"Stake set to {(int(event['newStake'])/1_000_000):,.0f} CCD."
                        result.request_target = True
                    
                    elif event['tag'] == 'DelegationStakeDecreased':
                        result.delegation_message += f"Stake decreased{result.perc} to {(int(event['newStake'])/1_000_000):,.0f} CCD."
                        result.request_target = True
                    
                    elif event['tag'] == 'DelegationRemoved':
                        result.delegation_message += f"Delegation removed."
                        result.request_target = True
