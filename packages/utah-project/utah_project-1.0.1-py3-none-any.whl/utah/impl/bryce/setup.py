...
from utah.core.setup import DEPLOY_PATTERN_NONE
from utah.core.setup import DEPLOY_PATTERN_DEV_PROD
from utah.core.setup import DEPLOY_PATTERN_DEV_UAT_PROD
...
class UtahInitializer(BaseUtahInititializer):
    def __init__(self, src_repo_loc:str, mongo_admin_username:str, mongo_admin_password:str, mongo_admin_host:str, mongo_admin_port:int, mongo_admin_tls:bool, db_purpose:str, deploy_pattern:int):
        super().__init__(src_repo_loc, deploy_pattern)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--deploy_pattern', dest='deploy_pattern', action='store', required=True, choices=["none", "dev_prod", "dev_uat_prod"], help='Deployment pattern for the database infrastructure')
    args = parser.parse_args()

    source_repo_path = bootstrap.get_app_home() + "/sample_content"

    if args.deploy_pattern == "none":
        deploy_pattern = DEPLOY_PATTERN_NONE
    elif args.deploy_pattern == "dev_prod":
        deploy_pattern = DEPLOY_PATTERN_DEV_PROD
    elif args.deploy_pattern == "dev_uat_prod":
        deploy_pattern = DEPLOY_PATTERN_DEV_UAT_PROD


    initializer = UtahInitializer(source_repo_path, args.mongo_admin_username, args.mongo_admin_password, args.mongo_admin_host, args.mongo_admin_port, args.mongo_admin_tls, args.db_purpose, deploy_pattern)
