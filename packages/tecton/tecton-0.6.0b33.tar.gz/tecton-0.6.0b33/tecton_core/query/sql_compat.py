...
def struct_extract(name: str, field_names: List[str], aliases: List[str]) -> List[Field]:
    """
    ...
    if dialect() == Dialect.SPARK:
        ...
        c = CustomFunction("get", ["array", "idx"])
        return [c(name, idx).as_(aliases[idx]) for idx in range(len(aliases))]
    else:
        raise NotImplementedError
