...
class _ODFVPipelineBuilder:
    ...
    def _get_dict_keys_from_udf_output(self, output_df: "snowflake.snowpark.DataFrame"):
        from snowflake.snowpark.functions import col, object_keys
    ...
    def _call_udf(
        ...
        from snowflake.snowpark.functions import array_construct, col, object_keys
        ...
        if is_top_level_transformation:
            ...
            columns_to_select = list(
                filter(lambda x: "_UDF_INTERNAL" not in x, list(input_df.columns) + list(self._output_schema.keys()))
            )
