...
def build_get_full_agg_features(
    ...
    return FullAggNode(base, fdw).as_ref()
...
def build_spine_join_querytree(
    ...
    if fdw.is_temporal or fdw.is_feature_table:
        ...
        if fdw.feature_start_timestamp:
            ...
            #   feature values.
            join = RespectFeatureStartTimeNode(
                join, ANCHOR_TIME, ts, fdw.features, fdw.get_feature_store_format_version
            ).as_ref()
...
def build_feature_set_config_querytree(
    ...
    for dac in other_dacs:
        ...
        subspine = SelectDistinctNode(spine_node, [jk[0] for jk in dac.join_keys] + [spine_time_field]).as_ref()
