...
class FeatureDefinition(Fco):
    ...
    # TODO(Alvin): spine needs to be DataframeWrapper, not pysparkDF
    def _point_in_time_join(self, spine: pysparkDF, timestamp_key: Optional[str], from_source: bool) -> TectonDataFrame:
        ...
        if conf.get_bool("QUERYTREE_ENABLED") and (
            ...
            # TODO(Daryl): will merge the ALPHA_ATHENA_COMPUTE_ENABLED codepath
            if conf.get_or_none("SQL_DIALECT") == "athena":
                return TectonDataFrame._create(
                    builder.build_spine_join_querytree(
                        dac,
                        UserSpecifiedDataNode(TectonDataFrame._create(spine.toPandas())).as_ref(),
                        timestamp_key,
                        from_source,
                    )
                )
                ...
                return TectonDataFrame._create(
                    builder.build_spine_join_querytree(
                        dac,
                        UserSpecifiedDataNode(TectonDataFrame._create(spine)).as_ref(),
                        timestamp_key,
                        from_source,
                    )
                )
