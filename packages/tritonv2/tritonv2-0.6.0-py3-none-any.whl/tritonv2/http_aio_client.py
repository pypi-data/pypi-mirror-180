...
class TritonAioHTTPClient(TritonClient):
    ...
    async def server_ready(self, headers=None, query_params=None):
        ...
        """
        try:
            return await self._client.is_server_ready(headers=headers, query_params=query_params)
    ...
    @retry(stop=(stop_after_attempt(NUMBER_RETRIES + 1) | stop_after_delay(MAX_INTERVAL_SECS)),
           wait=wait_fixed(BASE_INTERVAL_SECS), reraise=True)
    async def model_ready(self, model_name, model_version="1", headers=None, query_params=None):
        """
    ...
    @retry(stop=(stop_after_attempt(NUMBER_RETRIES + 1) | stop_after_delay(MAX_INTERVAL_SECS)),
           wait=wait_fixed(BASE_INTERVAL_SECS), reraise=True)
    async def model_meta(self, model_name, model_version="1", headers=None, query_params=None):
        """
        get model meta
        Parameters
        :param model_name:
    ...
    @retry(stop=(stop_after_attempt(NUMBER_RETRIES + 1) | stop_after_delay(MAX_INTERVAL_SECS)),
           wait=wait_fixed(BASE_INTERVAL_SECS), reraise=True)
    async def model_config(self, model_name, model_version="1", headers=None, query_params=None):
        """
        get model config
        Parameters
        :param model_name:
        ...
        """
        try:
            model_config = await self._client.get_model_config(model_name, model_version, headers=headers,
                                                               query_params=query_params)
            return model_config
        except Exception as e:
    ...
    @retry(stop=(stop_after_attempt(NUMBER_RETRIES + 1) | stop_after_delay(MAX_INTERVAL_SECS)),
           wait=wait_fixed(BASE_INTERVAL_SECS), reraise=True)
    async def model_statistics(self, model_name, model_version="1", headers=None, query_params=None):
        """
