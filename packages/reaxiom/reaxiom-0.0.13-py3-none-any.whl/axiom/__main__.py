...
def main(sys_argv):
    ...
    def compile(txt):
        ...
        axiom.common.add_obj_server(os.path.join(folder, 'obj'))
    ...
    if sys_argv[1] == 'add': # add the current folder as the obj
        axiom.common.add_obj_server(sys_argv[2])

    if sys_argv[1] == 'rm': # remove an axiom object by given name
        axiom.common.rm(sys_argv[2])
    ...
    if sys_argv[1] == 'pwd':
        print(axiom.common.getcwd())

    if sys_argv[1] == 'ls':
        if len(sys_argv) == 2:
            axiom.common.ls()
        else:
            axiom.common.ls(sys_argv[2])

    if sys_argv[1] == 'ls.update':
        if len(sys_argv) == 2:
            axiom.common.fs_func[axiom.common.get_server()]['ls'](None, update=True)
        else:
            axiom.common.fs_func[axiom.common.get_server()]['ls'](sys_argv[2], update=True)

    if sys_argv[1] == 'cd':
        if len(sys_argv) == 2:
            target_folder = ""
            axiom.common.set_cwd(target_folder)
        else:
            working_folder = axiom.common.get_working_folder(axiom.common.get_stage())
            sub = sys_argv[2]
            cwd = open(axiom.common.get_cwd_file()).read()
            if sub == '..':
    ...
    if sys_argv[1] == 'server': # set the server, from [local, huggingface, bypy] or get the server
        if len(sys_argv) == 2:
            print(axiom.common.get_server())
        else:
            axiom.common.set_server(sys_argv[2])
...
type: data.int'''
    ...
    if sys_argv[1] == 'stage': # ['simple', 'test', 'dev', 'stable', 'publish', 'axiom_test']
        if len(sys_argv) == 2:
            print(axiom.common.get_stage())
        else:
            axiom.common.set_stage(sys_argv[2])
    if sys_argv[1] == 'reset':
        axiom.common.reset()
    ...
    if sys_argv[1] == 'example':
        ...
        main(['a', 'add', os.path.join(folder_me, 'example', 'simple_add', 'int_3', 'obj')])
        main(['a', 'add', os.path.join(folder_me, 'example', 'simple_add', 'int_4', 'obj')])
