...
def put_node_huggingface(compressed_obj_file, info_yaml, sha256, given_name): # node means obj + info
    ...
    hh.upload_file(path_or_fileobj=tmp_file,path_in_repo=sha256+'.yaml',repo_id=get_repo_id('sha256_info'),repo_type="dataset",token=True)
    hh.upload_file(path_or_fileobj=tmp_file,path_in_repo=given_name+'.yaml',repo_id=get_repo_id('info'),repo_type="dataset",token=True)
    os.remove(tmp_file)
...
def ls_huggingface(path, update=False):
    info_folder = get_info_huggingface(update)
    print('info_folder', info_folder)
    axiom_ls(info_folder, path, ignores=['.gitattributes'])

def rm_huggingface(given_name):
    sha256 = get_sha256_by_given_name_huggingface(given_name)
    repo_id = get_repo_id('info')
    hh.delete_file(given_name+'.yaml', repo_id=repo_id, repo_type='dataset', token=True)
    repo_id = get_repo_id('obj')
    hh.delete_file(sha256, repo_id=repo_id, repo_type='dataset', token=True)
    repo_id = get_repo_id('sha256_info')
    hh.delete_file(sha256+'.yaml', repo_id=repo_id, repo_type='dataset', token=True)
...
def get_repo_id(what): # what in ['info', 'obj', 'sha256_info']
    stage = get_stage()
    user_name = open(os.path.join(config['home'], '.huggingface', 'user_name')).read()
