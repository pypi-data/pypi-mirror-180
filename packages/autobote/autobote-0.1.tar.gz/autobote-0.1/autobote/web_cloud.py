...
import subprocess,datetime,json,socket,time,os,logging
...
class Web:
    ...
    def __init__(self, ip="127.0.0.1", port=36678, options = {}):
        """
        ...
        # 定义日志
        self.log.basicConfig(filename=path + 'weblog_' + today_date + '.txt', level=logging.DEBUG, filemode='a',
                             format='【%(asctime)s】 【%(levelname)s】 >>>  %(message)s', datefmt='%Y-%m-%d %H:%M')
        # #########


        browserName = "edge"
        if 'browserName' in options:
            browserName = options["browserName"]
        debugPort = 0
        if 'debugPort' in options:
            debugPort = options["debugPort"]
        userDataDir = "./UserData"
        if "userDataDir" in options:
            userDataDir = options['userDataDir']
        browserPath = 'null'
        if 'browserPath' in options:
            browserPath = options['browserName']
        argument = "null"
        if 'argument' in options:
            argument = options['argument']
        driverPath = r"C:\Aibote\Aibote_cloud\WebDriver.exe"
        browserParam = [driverPath, json.dumps(
            {"serverIp": ip,"driverPort": port, "browserName": browserName, "debugPort": debugPort,
             "userDataDir": userDataDir, "browserPath": browserPath, "argument": argument})]
        if ip == "127.0.0.1":
            subprocess.Popen(browserParam)
