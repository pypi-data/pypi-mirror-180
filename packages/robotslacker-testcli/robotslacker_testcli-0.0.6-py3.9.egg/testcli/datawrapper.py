...
class DataWrapper(object):
    def __init__(self):
        ...
        self.TestOptions = None  # 程序处理参数

    # 创建seed文件，默认在TESTCLI_HOME/data下
    def create_seed_cache(self,
        ...
        if self.c_SeedFileDir is None:
            if "TESTCLI_HOME" not in os.environ:
                raise TestCliException(
                    "Missed TESTCLI_HOME, please set it first. Seed file will created in TESTCLI_HOME/data")
            self.c_SeedFileDir = os.path.join(os.environ["TESTCLI_HOME"], "data")
        # 如果不存在数据目录，创建它
        ...
        if data_type.upper() not in ('STRING', 'INTEGER'):
            raise TestCliException("Data Type must be String or integer.")
        if data_length <= 0:
            raise TestCliException("Data Length must great then zero.")
        if row_count <= 0:
            raise TestCliException("rows must great then zero.")
    ...
    # 缓存seed文件，默认在TESTCLI_HOME/data下
    ...
    def Convert_file(self, p_srcfileType, p_srcfilename, p_dstfileType, p_dstfilename):
        try:
            ...
            if m_srcFSType == "Not Supported" or m_dstFSType == "Not Supported":
                raise TestCliException("Not supported convert. From [" + p_srcfileType + "] to [" + p_dstfileType + "]")
            ...
            if "TESTCLI_DEBUG" in os.environ:
                raise TestCliException(he.message)
            else:
                raise TestCliException(he.message.split('\n')[0])
    ...
    def Create_file(self, p_filetype, p_filename, p_formula_str, p_rows, p_encoding='UTF-8'):
        try:
            ...
            if p_filetype.upper() == "MEM":
                ...
                raise TestCliException("Unknown file format.")
    ...
    def Process_SQLCommand(self, p_szSQL):
        # 设置数据种子文件读取的位置，如果不设置，默认是TESTCLI_HOME\data
        ...
        if match_obj:
            ...
            self.Create_file(p_filetype=m_filetype,
                             ...
                             p_rows=m_rows,
                             p_encoding=self.TestOptions.get("RESULT_ENCODING"))
            yield (
                None,
                None,
                None,
