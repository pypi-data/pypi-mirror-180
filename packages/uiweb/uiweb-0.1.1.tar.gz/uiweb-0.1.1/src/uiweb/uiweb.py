...
class Simple:
  ...
  def __init__(self,socket,pyton_path):
    ...
    fullpath = Simple.PYTHONPATH+os.sep+ 'current_handlers'
...
<script>
    ...
    if not nav==None and "ClientConfiguration" in self.configuration:
      ...
      if not self.islogin or Simple.isreload:
...
<h1>Панель администрирования Simple</h1>


<form >
<h3>Загрузка файла конфигурации</h3>
  ...
  <p >{{settings.last_update_uifile}}</p>
  </div>
 

<h3>Загрузка файла обработчиков (Python)</h3>
  ...
  <input type="file" id="handlersfile" name="handlersfile"><br><br>
    ...
    <div class=horizontal>
  <p >Последняя загрузка:</p>
  <p >{{settings.last_update_handlersfile}}</p>
  </div>
