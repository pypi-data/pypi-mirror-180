...
class Simple:
  ...
  def close_maintab(self,message):
    print("TODO освобождать место:"+str(message)) 
  ...
  def set_values(self,jSetValues):
    ...
    for el in jSetValues:
         for key,value in el.items():
          self.socket_.emit('setvalue', {'key':"d"+self.current_tab_id+"_"+key,'value':el[key],'tabid':self.current_tab_id},sid=self.sid,namespace='/'+SOCKET_NAMESPACE)
  def set_values_pulse(self,jSetValues):
    ...
    for el in jSetValues:
         for key,value in el.items():
          self.socket_.emit('setvaluepulse', {'key':"d"+self.current_tab_id+"_"+key,'value':el[key],'tabid':self.current_tab_id},sid=self.sid,namespace='/'+SOCKET_NAMESPACE)         
  ...
  def handle_command(self,current_tab=None):
    ...
    if  'SetValuesPulse' in self.hashMap:
          ...
          for el in jSetValues:
              for key,value in el.items():
                     self.socket_.emit('setvaluepulse', {'key':"d"+active_tab+"_"+key,'value':el[key],'tabid':active_tab},sid=self.sid,namespace='/'+SOCKET_NAMESPACE) 
    ...
    if  'SetValuesTable' in self.hashMap:
          ...
          for el in jSetValues:
              for key,value in el.items():
                     self.socket_.emit('setvaluehtml', {'key':"tablediv_"+"d"+self.current_tab_id+"_"+key,'value':str(self.add_table(json.dumps(el[key],ensure_ascii=False),"d"+self.current_tab_id+"_"+key)),'tabid':Simple.current_tab_id},sid=self.sid,namespace='/'+SOCKET_NAMESPACE) 
    ...
    if 'OpenScreen' in self.hashMap:
            ...
            self.current_tab_id=tabid
            added_tables=[]
            button,tab = self.new_screen_tab(self.configuration,tabparameters['process'],tabparameters['screen'],soup,tabid)
                        
            self.socket_.emit('add_html', {"id":"maintabs","code":str(button)},sid=self.sid,namespace='/'+SOCKET_NAMESPACE)
            self.socket_.emit('add_html', {"id":"maincontainer","code":str(tab)},sid=self.sid,namespace='/'+SOCKET_NAMESPACE)             
            self.socket_.emit('click_button', {"id":"maintab_"+tabid},sid=self.sid,namespace='/'+SOCKET_NAMESPACE)

            firsttabslayout=[]
            for t in added_tables:
              self.socket_.emit('run_datatable', t,sid=self.sid,namespace='/'+SOCKET_NAMESPACE)

            for t in firsttabslayout:
              self.socket_.emit('click_button', {"id":t},sid=self.sid,namespace='/'+SOCKET_NAMESPACE)  
  ...
  #<span class="closebtn" onclick="this.parentElement.style.display='none';">&times;</span> """ + text+ '</div>'
            self.socket_.emit('toast', {'code':text,'id':toastid},sid=self.sid,namespace='/'+SOCKET_NAMESPACE,to=self.sid) 
    ...
    if 'beep' in self.hashMap:
            ...
            self.socket_.emit('beep', {},sid=self.sid,namespace='/'+SOCKET_NAMESPACE,to=self.sid)          
    ...
    if 'ShowDialog' in self.hashMap:
            ...
            self.socket_.emit('setvaluehtml', {"key":"modaldialog","value":dialogHTML},sid=self.sid,namespace='/'+SOCKET_NAMESPACE)
            self.socket_.emit('show_dialog', {'code':text,'id':toastid},sid=self.sid,namespace='/'+SOCKET_NAMESPACE)  
    ...
    if 'basic_notification' in self.hashMap:
            ...
            self.socket_.emit('notification', {'text':text,'id':notificationid,'title':title},sid=self.sid,namespace='/'+SOCKET_NAMESPACE)     
    ...
    if 'ShowScreen' in self.hashMap:
            ...
            if screen==None:
              self.socket_.emit('setvaluehtml', {'key':"root_"+self.current_tab_id,'value':'<h1>Не найден экран: '+screenname+'</h1>','tabid':self.current_tab_id},sid=self.sid,namespace='/'+SOCKET_NAMESPACE)
            else:  
              ...
              # if len(screen.get('DefOnCreate',''))>0:
              #   operation = screen.get('DefOnCreate','')

              #   json_str = {"process":Simple.process.get("ProcessName",""),"operation":screen.get("Name",""),"hashmap":hashMap}

              #   process_data = {"process":"тест","operation":"новый экран"} 

              #   #Python
              #   response = self.set_input(operation,json.dumps(json_str,ensure_ascii=False).encode('utf-8'),process_data)

              # if len(screen.get('DefOnlineOnCreate',''))>0:
              #   operation = screen.get('DefOnlineOnCreate','')
              ...
              #   #Online
              #   response = self.set_input_online(operation,json.dumps(json_str,ensure_ascii=False).encode('utf-8'))  


              layots = self.get_layouts(soup,screen,0)

              self.socket_.emit('setvaluehtml', {'key':"root_"+self.current_tab_id,'value':str(layots),'tabid':self.current_tab_id},sid=self.sid,namespace='/'+SOCKET_NAMESPACE)
  ...
  def get_layouts(self,soup,root,level,var_prefix=''):
    ...
    for elem in root['Elements']:
        ...
        if elem.get('type')=='LinearLayout' or elem.get('type')=='Tabs' or elem.get('type')=='Tab':
            ...
            if elem.get('type')=='LinearLayout':
              ...
              layouts = self.get_layouts(soup,elem,level+1,var_prefix)
            ...
            if elem.get('type')=='TextView':
                ...
                new_element.string = html.unescape(self.calculateField(elem.get("Value")))
                currentcontainer.append(new_element)
            ...
            if elem.get('type')=='EditTextText':
                ...
                if len(elem.get("Value",''))>0:
                  new_element = soup.new_tag("input", id=tvkey,type="text",value = self.calculateField(elem.get("Value")))
            ...
            if elem.get('type')=='EditTextAuto':
                ...
                if len(elem.get("Value",''))>0:
                  new_element = soup.new_tag("input", id=tvkey,type="text",value = self.calculateField(elem.get("Value")), **{'class':'autotext'})
            ...
            if elem.get('type')=='EditTextNumeric':
                ...
                if len(elem.get("Value",''))>0:
                  new_element = soup.new_tag("input", id=tvkey,type="number",value = self.calculateField(elem.get("Value")),step=step,placeholder=placeholder)
            ...
            if elem.get('type')=='EditTextPass':
                ...
                if len(elem.get("Value",''))>0:
                  new_element = soup.new_tag("input", id=tvkey,type="password",value = self.calculateField(elem.get("Value")))
            ...
            if elem.get('type')=='MultilineText':
                ...
                if len(elem.get("Value",''))>0:
                  new_element.string =  self.calculateField(elem.get("Value"))
            ...
            if elem.get('type')=='DateField':
                ...
                if len(elem.get("Value",''))>0:
                  new_element = soup.new_tag("input", id=tvkey,type="date",value = self.calculateField(elem.get("Value")))
            ...
            if elem.get('type')=='SpinnerLayout':
                ...
                if len(elem.get("Value",''))>0:
                  values = self.calculateField(elem.get("Value",'')).split(";")
            ...
            if elem.get('type')=='CheckBox':
                ...
                if str(self.calculateField(elem.get("Value",''))).lower()=='true':
            ...
            if elem.get('type')=='Button':
                ...
                if 'style_class' in elem:
                  new_element = soup.new_tag("button", id=tvkey,style=get_decor(elem), **{'class':elem.get('style_class')})
                else:  
            ...
            if elem.get('type')=='Picture':
                ...
                new_element = soup.new_tag("img", id=tvkey,style=get_decor(elem),src="/static/"+self.calculateField(elem.get("Value")))
            ...
            if elem.get('type')=='TableLayout':
                if not elem.get("Value")==None:
                  ...
                  htmltable = self.add_table(self.calculateField(elem.get("Value")),tvkey,stylestr)
            ...
            if elem.get('type')=='CardsLayout':
                ...
                if not elem.get("Value")==None:
                  htmlcards = self.add_cards(self.calculateField(elem.get("Value")),tvkey,stylestr)
  ...
  def add_cards(self,value,variable,stylestr):
   ...
   try:
    ...
    i=0
    for row in jcards['customcards']['cardsdata']:
      rowid="cardrow_"+str(i)+"_"+variable
      row=base.new_tag("div",  **{'class':'card shadow-1'},id=rowid)
      i+=1

      layout = self.get_layouts(base,jcards['customcards']['layout'],0,rowid+"_")
      row.append(layout)
        ...
        $(document).ready(function() {
            ...
            var socket = io(namespace);
             ...
             $(document).on('click', 'tr', function () {
              ...
              //alert('You clicked row '+ $(this).val() );
            ...
            $(document).on('click', 'button', function()
            {
              var clickedID=this.id
                ...
                formdata={data: 'button',values: jsonString,source:clickedID}
...
<script>
    ...
    if not nav==None and "ClientConfiguration" in self.configuration:
      ...
      #tabid = str(uuid.uuid4().hex) 
      #Simple.current_tab_id=tabid
      #self.new_screen_tab(self.configuration,'экран','',soup,tabid)
  ...
  def new_screen_tab(self,configuration,processname,screenname,soup,tabid):
  ...
  def RunEvent(self,event,postExecute=None): 
    ...
    if 'Handlers' in self.screen or not postExecute==None:
    #NEW HANDLERS 
    ...
    if len(self.hashMap.get("ErrorMessage",""))>0:
      ...
      self.socket_.emit('error', {'code':self.hashMap.get("ErrorMessage","")},sid=self.sid,namespace='/'+SOCKET_NAMESPACE)
