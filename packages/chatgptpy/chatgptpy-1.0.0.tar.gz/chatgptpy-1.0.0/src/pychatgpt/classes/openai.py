...
def token_expired() -> bool:
    ...
    """
    try:
        # Get access token & expiry on env
        access_token = os.getenv("CHATGPT_ACCESS_TOKEN")
        expiry = os.getenv("CHATGPT_ACCESS_TOKEN_EXPIRY")

        if not access_token or not expiry:
            return True

        print(f"{Fore.GREEN}>> Access token found in env variables.")
...
class Auth:
    ...
    def save_access_token(access_token: str, expiry: int or None = None):
        ...
        try:
            ...
            os.environ["CHATGPT_ACCESS_TOKEN"] = access_token
            os.environ["CHATGPT_ACCESS_TOKEN_EXPIRY"] = str(expiry)

            dotenv.set_key(dotenv_path=dotenv.find_dotenv(),
                           key_to_set="CHATGPT_ACCESS_TOKEN",
                           value_to_set=access_token)
            dotenv.set_key(dotenv_path=dotenv.find_dotenv(),
                           key_to_set="CHATGPT_ACCESS_TOKEN_EXPIRY",
                           value_to_set=expiry)
