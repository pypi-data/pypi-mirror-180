...
class Chat:
    ...
    def _setup(self):
        ...
        if not os.getenv("CHATGPT_ACCESS_TOKEN") or not os.getenv("CHATGPT_ACCESS_TOKEN_EXPIRY"):
            print(f"{Fore.RED}>> CHATGPT_ACCESS_TOKEN and CHATGPT_ACCESS_TOKEN_EXPIRY is missing in your env variables."
                  f" {Fore.GREEN}Attempting to create them...")
            self._create_access_token()
        else:
            self.__auth_access_token = os.getenv("CHATGPT_ACCESS_TOKEN")
            self.__auth_access_token_expiry = os.getenv("CHATGPT_ACCESS_TOKEN_EXPIRY")

            try:
                self.__auth_access_token_expiry = int(self.__auth_access_token_expiry)
            except ValueError:
                print(f"{Fore.RED}>> CHATGPT_ACCESS_TOKEN_EXPIRY is not an integer.")
                raise Exceptions.PyChatGPTException("CHATGPT_ACCESS_TOKEN_EXPIRY is not an integer.")
