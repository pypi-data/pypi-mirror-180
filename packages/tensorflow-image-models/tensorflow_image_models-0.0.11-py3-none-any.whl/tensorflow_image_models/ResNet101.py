...
# ResNet50 Model
class ResNet101Class(tf.keras.Model):
    def __init__(self, classes=10):
        super(ResNet101Class, self).__init__()
    ...
    def call(self, inputs):
        ...
        x = self.conblock1(x)
        for _ in blocks[1:3]:
            x = identity_block(3, [64, 64, 256], stage=2, block=_)(x)

        # ******** 4 Times *************
        x = self.conblock2(x)
        for _ in blocks[1:4]:
            x = identity_block(3, [128, 128, 512], stage=3, block=_)(x)

        # ******* 23 Times *************
        x = self.conblock3(x)
        for _ in blocks[1:23]:
            x = identity_block(3, [256, 256, 1024], stage=4, block=_)(x)

        # ******* 3 Times **************
        x = self.conblock4(x)
        for _ in blocks[1:3]:
            x = identity_block(3, [512, 512, 2048], stage=5, block=_)(x)
...
def ResNet101(classes=10,input_shape=(128, 128, 3,)):
    input_layer = Input(shape=input_shape)
    x = ResNet101Class(classes=classes)(input_layer)
