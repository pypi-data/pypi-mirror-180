...
class GenericControllerClass:
    ...
    def exploreCluster(self, connect_db_output, source, unique_source_data, index,source_by_create_date, data_cluster_representative, flag_data, generic):
        try:
            ...
            if(self.data_app_config["continuity"]["required"]):
                ...
                if(len(data_continuity) > 0):
                    ...
                    if(len(unique_source_data) == index+1):
                    ...
                    for pk in self.data_app_config["report"]["field_pk"]:
                        if(source[pk] != unique_source_data.iloc[index+1][pk]):
                            continuity_status = False
                            break

                    if(continuity_status):
                        date_format = "%Y-%m-%d"
                        delta = datetime.strptime(unique_source_data.iloc[index+1][self.data_app_config["report"]["field_start_date"]], date_format) - datetime.strptime(source[self.data_app_config["report"]["field_start_date"]], date_format)
    ...
    def handleProcessMember(self, connect_db_output, source, source_by_create_date, source_detail_by_create_date, source_data_no_detail, model_app, flag_data, data_cluster_representative, data_explore_cluster, generic):
        self.controller_app["member"].processMember(connect_db_output, source, source_by_create_date, source_detail_by_create_date, source_data_no_detail, model_app, flag_data, data_cluster_representative, data_explore_cluster, generic)

    def handleProcessSummary(self, connect_db_output, source, source_by_create_date, source_detail_by_create_date, model_app, flag_data, data_cluster_representative, generic):
        self.controller_app["summary"].processSummary(connect_db_output, source, source_by_create_date, source_detail_by_create_date, model_app, flag_data, data_cluster_representative, generic)

    def handleProcessReport(self, source, model_app, flag_data, data_cluster_representative, generic):
        return self.controller_app["report"].processReport(source, model_app, flag_data, data_cluster_representative, generic)
