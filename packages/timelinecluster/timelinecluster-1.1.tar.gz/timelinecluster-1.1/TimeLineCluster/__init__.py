            ...
            if(self.status):
                ...
                if(len(source_data) > 0): # check source data for process
                    ...
                    for i , source in enumerate(unique_source_data.iloc): # self.pk_report_name, self.pk_report_start_date
                        ...
                        self.flag_data["sql_for_insert_cluster_execute"]["flag_insert_for_summary_details"] = False
                        ...
                        self.generic.handleProcessMember(self.connect_db_output, source, source_by_create_date, source_detail_by_create_date, source_data_no_detail, self.model_app, self.flag_data, data_cluster_representative, data_explore_cluster, self.generic)
                        self.generic.handleProcessSummary(self.connect_db_output, source, source_by_create_date, source_detail_by_create_date, self.model_app, self.flag_data, data_cluster_representative, self.generic)

                        data_process_report = self.generic.handleProcessReport(source, self.model_app, self.flag_data, data_cluster_representative, self.generic)
                        ...
                        if(self.generic.data_app_config["execute_status"] == 1):
                            self.generic.executeSQLForProcess(self.connect_db_output, self.flag_data, self.generic)
                        else:
