...
def get_image_magick_command(work_dir: str) -> str:
    ...
    download_file("https://download.imagemagick.org/ImageMagick/download/binaries/magick", os.path.join(work_dir, "magick"))
    return os.path.join(work_dir, "magick")
...
def build_appimage(project_dir: str, work_dir: str, pyproject: PyprojectDict, args: argparse.Namespace) -> None:
    ...
    subprocess.run([os.path.join(app_root, "usr", "bin", "pip"), "install", project_dir], check=True)
    ...
    if "icon" in pyproject:
        ...
        if "rename-icon" in pyproject:
            os.rename(os.path.join(work_dir, "squashfs-root", os.path.basename(pyproject["icon"])), os.path.join(app_root, pyproject["rename-icon"]))
    ...
    if "desktop-entry" in pyproject:
        shutil.copyfile(os.path.join(project_dir, pyproject["desktop-entry"]), os.path.join(app_root, "App.desktop"))
    else:
    ...
    if args.no_fuse:
        subprocess.run([os.path.join(work_dir, "Appimagetool.AppImage"), "--appimage-extract-and-run",  app_root, output], check=True)
    else:
        subprocess.run([os.path.join(work_dir, "Appimagetool.AppImage"),  app_root, output], check=True)
