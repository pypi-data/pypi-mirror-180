...
def get_sig(sig):
    ...
    if PY2:
        if isinstance(sig, STR_TYPE):
            sig = sig.encode()
        if isinstance(sig, BYTES_TYPE):
...
def daemon_start(main, pidfile, daemon=True, workspace=None):
    ...
    """
    _logger.debug("start daemon application pidfile={pidfile} daemon={daemon} workspace={workspace}.".format(pidfile=pidfile, daemon=daemon, workspace=workspace))
    new_pid = os.getpid()
    workspace = workspace or os.getcwd()
    os.chdir(workspace)
    daemon_flag = False
    if pidfile and daemon:
        ...
        if old_pid:
            _logger.debug("pidfile {pidfile} already exists, pid={pid}.".format(pidfile=pidfile, pid=old_pid))
        ...
        if daemon and os.name == "posix":
            ...
            daemon_flag = True
    if daemon_flag:
        _logger.info("Start application in DAEMON mode, pidfile={pidfile} pid={pid}".format(pidfile=pidfile, pid=new_pid))
    else:
