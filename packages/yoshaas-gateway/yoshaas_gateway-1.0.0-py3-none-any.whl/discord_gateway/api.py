...
def handle_api_response(resp: requests.Response) -> Response | dict:
...
class HttpClient:
    def __init__(self, token: str, bot: bool = True) -> None:
        log.debug('INIT discord rest api')
        self._token: str = token
        self.is_bot = bot
        self.headers = {'Authorization': f'{"Bot " if bot else ""}{self._token}'}

    def requests(self, method: str, path: str, *, data: Optional[dict | str] = None, headers: Optional[dict] = None,
                 threads: bool = False,
                 **kwargs) -> Optional[Response | dict]:
        """
    ...
    @staticmethod
    def send(method: str, url: str, data: Optional[dict | str], headers: Optional[dict],
             **kwargs) -> Response | dict | bool:
        try:
            if method == "GET":
            ...
            log.warning('Sending too many requests')
            return False
