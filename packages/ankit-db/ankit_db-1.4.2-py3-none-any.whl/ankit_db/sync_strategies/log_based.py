...
from ankit_db import dynamodb, deserialize
...
def sync(config, state, stream):
    ...
    expression = metadata.get(md_map, (), 'ankit-db.expression-attributes')
...
def has_stream_aged_out(state, table_name):
    ...
    return time_span > datetime.timedelta(hours=0, minutes=30)
