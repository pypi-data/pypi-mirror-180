...
class Base(Controller):
    ...
    def fd(self):
        ...
        # use_clipboard = self.app.pargs.c
        delimiter = self.app.pargs.__dict__.get('d') or ','
        generate_fake_data(n_lines, template, delimiter)
    ...
    @ex(
        ...
        arguments=[
            ...
            (['-r','--remote'],{
                "help" : "The remote to search for forks.",
                "action" : "store"
            }),
        ]
    )
    def gh(self):
        if self.app.pargs.action=='ff':
            ...
            print(find_branches(self.app.pargs.url, remote))
