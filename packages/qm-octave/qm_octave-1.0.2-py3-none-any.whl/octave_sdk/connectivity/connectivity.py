...
class SynthMapping:
    index: int = None
    synth_output: Dict[OctaveOutput, SynthPanelMapping] = field(default_factory=dict)
    synth_input: Dict[SynthRfOutputOutputPort, SynthInputMapping] = field(default_factory=dict)


def _convert_rf_in_enum_to_octave_lo_input(
...
def _convert_rf_in_lo_by_rf_out(name: RFInputLOSource, synth_index, rf_out_by_synth) -> RFInputLOSource:
...
def _convert_octave_lo_input_enum_to_rf_in(name: OctaveLOSource, synth_index) -> RFInputLOSource:
...
def _validate_rf_source_type(rf_source, rf_source_type, message):
    one_of, value = betterproto.which_one_of(rf_source, "source")
    if one_of != rf_source_type:
        raise InvalidIdentityException(f"{message} - {one_of}")


def _get_rf_source_name(rf_source: RfSource) -> RFInputRFSource:
    one_of, value = betterproto.which_one_of(rf_source, "source")
    ...
    if one_of == "rf_up_conv_output":
        ...
        if external_one_of == "rf_in_index":
            ...
            raise Exception("external input is not valid")
    elif one_of == "" and value is None:
        return RFInputRFSource.Off
    else:
        # "synth_output" and "constant_source" are not supported
        raise Exception("rf source is not supported")
...
def _get_rf_source_from_synth_panel_output(rf_source: RfSource) -> SynthRfOutput:
    one_of, value = betterproto.which_one_of(rf_source, "source")
...
class Connectivity:
    def __init__(
        ...
        for item in self._synth_by_rf_out.items():
            s = item[1][0]
            if s not in self._rf_out_by_synth:
                self._rf_out_by_synth[s] = []
            self._rf_out_by_synth[s].append(item[0])
    ...
    def _fill_synth_panel_mapping(self):
        for synth_output in self._identity_response.panel_identity.synth_outputs:
            output = _get_rf_source_from_synth_panel_output(synth_output.source)
            self._synth_connectivity[output.index].synth_output[OctaveOutput(synth_output.index)] = SynthPanelMapping(
    ...
    def _get_synth_by_rf_out(
        ...
        _validate_rf_source_type(
            lo_input,
            "synth_output",
            f"LO source 1 of RF output {rf_out_index} is unknown",
        )
    ...
    def _get_synth_by_rf_in(
        ...
        _validate_rf_source_type(
            lo_input,
            "synth_output",
            f"LO source 1 of RF input {rf_in_index} is unknown",
        )
    ...
    def _get_synth_lo_inputs(self, synth: SynthIdentity) -> (OctaveLOSource, OctaveLOSource):
    ...
    def _get_rf_port_name(lo_input: RfSource, message: str = None):
        _validate_rf_source_type(lo_input, "external_input", message)
    ...
    def _get_synth_from_output_port(self, octave_output: OctaveOutput) -> SynthMapping:
        try:
            for synth in self._synth_connectivity.values():
                if octave_output in synth.synth_output:
                    return synth
        except KeyError:
    ...
    def get_rf_source_of_rf_input(self, index) -> List[RFInputRFSource]:
        ...
        if betterproto.serialized_on_wire(rf_input.connectivity.rf_main_input):
            sources.append(_get_rf_source_name(rf_input.connectivity.rf_main_input))
        if betterproto.serialized_on_wire(rf_input.connectivity.debug_rf_input_1):
            sources.append(_get_rf_source_name(rf_input.connectivity.debug_rf_input_1))
        if betterproto.serialized_on_wire(rf_input.connectivity.debug_rf_input_2):
            sources.append(_get_rf_source_name(rf_input.connectivity.debug_rf_input_2))
        if betterproto.serialized_on_wire(rf_input.connectivity.debug_rf_input_3):
            sources.append(_get_rf_source_name(rf_input.connectivity.debug_rf_input_3))
        if betterproto.serialized_on_wire(rf_input.connectivity.debug_rf_input_4):
            sources.append(_get_rf_source_name(rf_input.connectivity.debug_rf_input_4))
        if betterproto.serialized_on_wire(rf_input.connectivity.debug_rf_input_5):
            sources.append(_get_rf_source_name(rf_input.connectivity.debug_rf_input_5))
    ...
    def get_synth_index_of_rf_out_lo(self, index) -> (int, SynthRfOutputOutputPort):
