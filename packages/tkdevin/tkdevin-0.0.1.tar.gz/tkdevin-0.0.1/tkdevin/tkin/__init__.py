...
class DTk(Tk, DWidget):
    ...
    def wm_destroy_with_mark_size(self, id) -> None:
        def record(_event):
            ...
            data["size"] = {"x": self.winfo_x(), "y": self.winfo_y(), "width": self.winfo_width(), "height": self.winfo_height()}
            self.wm_mark_id(id, data)
        self.bind("<Destroy>", record, add="+")

    destroy_with_mark_size = wm_destroy_with_mark_size

    def wm_mark_size(self, id) -> None:
        self.wm_destroy_with_mark_size(id)
        geo = self.wm_mark_id(id)
        if "size" in self.wm_mark_id(id):
            geo_size = [geo["size"]["width"], geo["size"]["height"], geo["size"]["x"], geo["size"]["y"]]
...
if __name__ == '__main__':
    ...
    from tkdevin import draw_gradient, make_movable_object
