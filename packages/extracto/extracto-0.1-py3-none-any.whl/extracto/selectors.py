...
Nodes must be annotated with preorder index."""
    ...
    if by_id:
        if 'id' in needle.attrs and _valid_id(needle.attrs['id']):
    ...
    if by_class > 0:
        ...
        if 'class' in needle.attrs and needle.attrs['class']:
            classes = [x for x in needle.attrs['class'].split(' ') if x]
            for x in classes:
                if _valid_css_class(x):
                    candidates.append('.' + x)

    if by_attr or by_attr_value:
        for x in needle.attrs:
            if x == 'class' or x == 'id' or x == 'data-preorder-index' or x.startswith('on'):
                continue

            if by_attr:
                candidates.append('[' + x + ']')

            if by_attr_value and needle.attrs[x]:
    ...
    if by_previous_tr:
        ...
        if parent_tr and parent_tr.prev and parent_tr.prev.tag == 'tr':
            ...
            for candidate in previous_tr_candidates:
                for me_candidate in simple_me_candidates:
                    candidates.append(candidate + ' + tr ' + me_candidate)
    ...
    if previous_elements:
        if by_adjacent_sibling:
            ...
            for x in adjacent_candidates:
                for y in simple_me_candidates:
...
def _candidate_repeating_selectors(tree, permutations, cache={}):
...
to enumerate the attributes."""
    ...
    # That's the theory. The practice... oy. I'm not at all confident I can implement case 1
    # correctly -- some of the nodes we've found may be red herrings.
    ...
    print('! candidate_repeating_selectors, len(permutations)=' + str(len(permutations)))
    ...
    # Each entry is len(singleton_attributes) long.

    for i in range(len(permutations)):
        attributes = permutations[i]

        for (nodes, selectors) in attributes:
            #print(nodes)
            for node in nodes:
                node = node[0]
                index = int(node.attrs['data-preorder-index'])

                if index in candidate_roots:
                    continue

                parent_tr = _find_parent(node, 'tr')
                if parent_tr and parent_tr.prev and parent_tr.prev.tag == 'tr':
                    candidate_roots[int(parent_tr.prev.attrs['data-preorder-index'])] = True

                while node and node.tag != 'html':
                    index = int(node.attrs['data-preorder-index'])
                    candidate_roots[index] = True
                    node = node.parent
    ...
    for node in tree.root.traverse():
        ...
        #candidate_repeating_selectors.extend([x for x in candidate_selectors(tree, node) if len(x) < 20])
