...
def can_extract_examples(cached_tree_css, row_examples, candidate_repeating_selector, ok_attribute_selectors):
    ...
    for candidate_repeating_selector in candidate_repeating_selectors:
        ...
        for i in range(len(attribute_examples)):
            ...
            for attribute_selector in candidate_selector_sets[i]:
                #print('attribute_selector={}'.format(attribute_selector))
                needed = {}
                ...
                for parent in search_roots:
                    # Possible optimization: if attribute selector starts with
                    # HTML, I think repeated searches are fruitless.
                    #
                    # ...also, I think we're probably not handling HTML selectors
                    #    correctly here at all... how does that work?
        ...
        if len(ok_attribute_selectors) == len(attribute_examples):
            ...
            ok = can_extract_examples(cached_tree_css, row_examples, candidate_repeating_selector, ok_attribute_selectors)
            #print('can_extract_examples took {}'.format(time.time() - t))
