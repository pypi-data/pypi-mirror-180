...
def _candidate_repeating_selectors(tree, singleton_attributes, attribute_nodes, cache={}):
...
to enumerate the attributes."""
    ...
    non_singleton_attribute_exists = False
    for x in singleton_attributes:
        if not x:
            non_singleton_attribute_exists = True


    for i, nodes in enumerate(attribute_nodes):
        if singleton_attributes[i] and non_singleton_attribute_exists:
            continue
