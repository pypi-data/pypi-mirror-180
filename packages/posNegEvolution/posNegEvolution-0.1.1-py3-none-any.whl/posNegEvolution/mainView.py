...
class mainFormat(qtWidget.QWidget):
    ...
    def createMainView(self):
        ...
        self._VAF = qtWidget.QCheckBox("Checked: Include VAF")
        layout.addWidget(self._VAF)
        self._analytical = qtWidget.QCheckBox("Checked: Analytical model (omit VAF)")
    ...
    def cloneHistAction(self):
        ...
        if all(map(lambda x: x.endswith('.npz'), fname)):
            self._th_hist = (Thread(target=externalPlots.VAFplot, args=(fname)))
