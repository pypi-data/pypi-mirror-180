            ...
            for index, row in ddl_df.iterrows():
                try:
                    ...
                    if table_name not in big_data_table_names:
                        ...
                        df_from_redshift = ro.read_from_redshift(method="ctx", limit=1)
                    ...
                    if df_from_redshift.__len__() > 0:
                        ...
                        if table_name not in big_data_table_names:
                            ...
                            sf.create_parquet_stage(table=table_name, staging_name='aws_big_data_parquet')
                            sf.create_parquet_file_format(file_format_name="parquet_format")
                            sf.create_table_from_template(table=table_name, staging_name='aws_big_data_parquet', file_format_name='parquet_format')
                            sf.copy_into(table=table_name, col_names=col_names_list, staging_name='aws_big_data_parquet', pattern=".*.parquet")
...
class SnowflakeOperator:
    ...
    def create_parquet_stage(self, table, staging_name: str):
        # self.conn.cursor().execute(f"""USE SCHEMA {self.schema.upper()};""")
        self.conn.cursor().execute(
            f"""
            CREATE OR REPLACE STAGE {staging_name}
            url='s3://oip-openinnovationprogram-datamart/big_data_format/{self.schema.lower()}/{table.lower()}/'
            CREDENTIALS = (
            aws_key_id='{os.environ.get("AWS_ACCESS_KEY_ID")}',
            aws_secret_key='{os.environ.get("AWS_SECRET_ACCESS_KEY")}'
            )
