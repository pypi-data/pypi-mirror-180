...
class SoPSimulationGenerator:
    ...
    def dump(self, simulation_env: SoPMiddlewareElement):
        ...
        event_timeline.extend(
            [thing.dump_event(SoPEventType.THING_RUN, 0.0).dump() for thing in thing_list])
...
class SoPBasicSuperThing(SoPSuperThing):
    ...
    def __init__(self, name: str = None, service_list: List[SoPService] = [], alive_cycle: float = 60, is_super: bool = False, is_parallel: bool = True,
                 ip: str = None, port: int = None, ssl_ca_path: str = None, ssl_enable: bool = None, refresh_cycle: float = 10, append_mac_address: bool = True, log_name=None, log_mode=SoPPrintMode.FULL):
        super_function_list = \\
            [%s]
...
'''
    ...
    def dump(self, simulation_env: SoPMiddlewareElement, service_list: List[SoPServiceElement], base_thing_folder_path: str, remote_base_thing_folder_path: str):
        ...
        def make_thing_name(index: int, middleware: SoPMiddlewareElement):
        ...
        def dump_recursive(middleware: SoPMiddlewareElement):
            ...
            for i in range(base_thing_num):
                # TODO: parent_middleware 정보가 middleware에 없어도 정보를 추출해서 작성 해볼것
                thing_name = make_thing_name(i, middleware)
