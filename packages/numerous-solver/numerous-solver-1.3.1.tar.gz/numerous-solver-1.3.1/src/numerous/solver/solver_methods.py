...
from .interface import Interface, _JitHelper
...
class BDF(BaseMethod):
    ...
    def __init__(self, interface: Interface, use_jit: bool, **options):
        ...
        try:
            ...
            jacobian = _JitHelper.jithelper(UserDefinedJacobian, jit=self.use_jit)
            jac: UserDefinedJacobian = jacobian(interface=self.interface)
            self.J = jac.get_jacobian(0)

        except NotImplementedError:
            jacobian = _JitHelper.jithelper(NumericalJacobian, jit=self.use_jit)
