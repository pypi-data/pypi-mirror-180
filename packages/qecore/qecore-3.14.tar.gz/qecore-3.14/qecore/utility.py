...
def run_embed(context, command, *a, **kw):
    ...
    try:
        output = check_output(command, shell=True, stderr=STDOUT, *a, **kw, encoding="utf-8")
        return_code = 0
        error = None
    except CalledProcessError as error:
        output = error.output
...
def overview_action(action="hide") -> None:
    ...
    if len(activities_label) == 1:
        ...
        # Find Ancestor unfortunatelly takes only Predicates not lambdas alone, will use it since implementing it here would be too long.
        activities_toggle_button = activities_label[0].findAncestor(GenericPredicate(roleName="toggle button"))

        for _ in range(5):
            if (action == "hide" and activities_toggle_button.checked) or (action == "show" and not activities_toggle_button.checked):
