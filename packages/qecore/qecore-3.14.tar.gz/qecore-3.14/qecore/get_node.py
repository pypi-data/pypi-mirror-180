        ...
        try:
            found_node = self.root.findChild(
                lambda x:
                    ...
                    and ((not isinstance(self.name, re.Pattern)) or re.fullmatch(self.name, x.name))
                    and ((not isinstance(self.role_name, str)) or self.role_name == x.roleName)
                    and ((not isinstance(self.role_name, re.Pattern)) or re.fullmatch(self.role_name, x.roleName))
                    and ((not isinstance(self.description, str)) or self.description == x.description)
                    and ((not isinstance(self.description, re.Pattern)) or re.fullmatch(self.description, x.description))
...
def get_formated_error_message(error,
    ...
    return "".join(["\n\n{0}: {1}\n\n{2}: {3}:\n{4}\n{5}: {6}:\n{7}\n".format(
        ...
        colored("Items with name", "yellow", attrs=["bold"]) \
            if QE_DEVELOPMENT else "Items with name", node_name, same_name_items,
...
def get_error_message(node, error):
    ...
    if isinstance(node.name, str):
        nodes_with_name = node.root.findChildren(
            lambda x: node.name in x.name and (not (node.name != "") or x.name != "")
        )
