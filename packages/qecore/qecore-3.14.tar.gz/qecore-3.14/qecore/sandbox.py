...
class TestSandbox:
    def __init__(self, component, logging=False, context=None, kiosk=False):
        ...
        ##########################################################################################
        log.info(f"Accessibility is somehow turning off, making another check.")
        # First check: dogtail utility for accessibility check and enabling.
        if not isA11yEnabled():
            print("Accessibility not detected running. Enabling via dogtail.")
            enableA11y()
    ...
    def _set_up_embedding(self, context):
        ...
        def set_title(title, append=False, tag="span", **kwargs):
            for formatter in context._runner.formatters:
                if formatter.name == "html" and getattr(formatter, "set_title", None) is not None:
                    formatter.set_title(title=title, append=append, tag=tag, **kwargs)
                    continue
                elif formatter.name == "html-pretty" and getattr(formatter, "set_title", None) is not None:
            ...
            for feature in formatter_instance.features:
                for scenario in feature.scenarios:
                    ...
                    for tag in scenario.tags:
                        ...
                        # If the tag.link is not None, it was modified already, skip another attempt to modify it to link.
                        if tag.has_link():
                            break
                        ...
                        # No reason to not attempt to do link every single time, everything should be on git.
                            ...
                            if type(tag.behave_tag) is str:
                                tag.set_link(project_url_base + scenario.location.filename + "#L" + str(scenario.location.line))
                            else:
                                tag.set_link(project_url_base + scenario.location.filename + "#L" + str(tag.behave_tag.line))
