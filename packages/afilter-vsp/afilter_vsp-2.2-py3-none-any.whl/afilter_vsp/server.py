...
from afilter_vsp.utils.mask2polygon import get_annotation
...
def get_class_mask(defect):
    ...
    return class_mask, mask_images

def get_json(mask_images):
    return get_annotation(mask_images)

def get_OKNG(defect):
    class_mask, mask_images = get_class_mask(defect)
    if isinstance(class_mask, list) and len(class_mask) == 0:
        return afilter_vsp_pb2.OneReply(ok_ng=OK)
    else:
        json = get_json(mask_images)
        return afilter_vsp_pb2.OneReply(ok_ng=NG, defect=class_mask, json=json)
