...
class ZoomMenu(TransientMenu):
    def __init__(self, parent):
        ...
        self.radio_button_scaled = ttk.Radiobutton(value='Scaled', variable=self.mode_var)

        self.add_radiobutton(text='Fit to window', value='Fit', variable=self.mode_var, command=self.on_mode_change)
        self.add_radiobutton(text='Fill window', value='Fill', variable=self.mode_var, command=self.on_mode_change)
        self.add_spacer()
        self.add_radiobutton(text='50%', value='50', variable=self.mode_var, command=self.on_mode_change)
        self.add_radiobutton(text='100%', value='Original', variable=self.mode_var, command=self.on_mode_change)
        self.add_radiobutton(text='200%', value='200', variable=self.mode_var, command=self.on_mode_change)
        self.add_radiobutton(text='300%', value='300', variable=self.mode_var, command=self.on_mode_change)
        self.add_spacer()
        self.add_scale(length=100, from_=30, to=300, variable=self.scale_var, command=self.on_scale_change)
