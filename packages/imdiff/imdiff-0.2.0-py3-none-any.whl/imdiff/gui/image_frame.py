...
class ImageFrame(ttk.Frame):
    def __init__(self, parent, update_item=None, left_label='left', right_label='right'):
        ...
        self.button_zoom_100 = ttk.Radiobutton(
            self, text='1:1',
            variable=self.zoom_menu.mode_var, value='Original', command=self.zoom_original)
        self.button_zoom_fit_if_larger = ttk.Radiobutton(
            self, text='Fit if larger',
            variable=self.zoom_menu.mode_var, value='FitIfLarger', command=self.zoom_fit_if_larger)
        self.button_zoom_menu = ttk.Button(
            self, text='Zoom...',
            command=self.post_zoom_menu)
        self.button_high_contrast = ttk.Checkbutton(
            self, text='High constrast',
            variable=self.high_contrast, command=self.show_diff,)

        self.image_layout_style = tk.IntVar()
        self.image_layout_style.set(3)
        self.button_layout1 = ttk.Radiobutton(self, text='1', variable=self.image_layout_style, value=1, command=self.layout_images)
        self.button_layout2 = ttk.Radiobutton(self, text='2', variable=self.image_layout_style, value=2, command=self.layout_images)
        self.button_layout3 = ttk.Radiobutton(self, text='3', variable=self.image_layout_style, value=3, command=self.layout_images)
    ...
    def layout(self):
        ...
        for col, button in enumerate(buttons):
            button.grid(column=col, row=0, sticky='w')

        for col, button in enumerate(self.operate_buttons):
            button.grid(column=len(buttons) + 1 + col, row=0, sticky='e')

        self.image_frame.grid(column=0, row=1, columnspan=len(buttons) + 1 + len(self.operate_buttons), sticky='nsew')
    ...
    def bind_events(self):
        for key in self.images.keys():
            for otherkey in set(self.images.keys()) - {key}:
                self.images[key].bind('<Button1-Motion>', self.images[otherkey].on_motion, add='+')
                self.images[key].bind('<ButtonRelease-1>', self.images[otherkey].on_left_up, add='+')
    ...
    def minsize(self):
        ...
        height = (
            self.button_zoom_100.winfo_height()
          + 64
        )
    ...
    def post_zoom_menu(self):
        pos = Coordinates((self.button_zoom_menu.winfo_rootx(), self.button_zoom_menu.winfo_rooty()))
    ...
    def delete(self):
        if self.comparator:
            left_file, right_file = self.comparator.left_file, self.comparator.right_file
