...
class ImageScaling:
    ...
    def scaling_factor(self, value):
        self._scaling_factor = max(0., float(value))
    ...
    def scaled(self, original, frame):
        if self.mode == ImageScaling.Mode.Original:
            ...
            if (
                   (is_larger and self.shrink_if_larger)
                or (not is_larger and self.expand_if_smaller)
            ):
                if self.mode == ImageScaling.Mode.Scaled:
                    ...
                    if (
                           (frame_is_narrower and self.mode == ImageScaling.Mode.Fit)
                        or (not frame_is_narrower and self.mode == ImageScaling.Mode.Fill)
                    ):
